...
%                         ($(C.center) + (offset) + (1.3em,-0.4ex)$) to[out=-30, in=90] ($(XR) + (offset) + (1.6em,0ex)$)
...
%
\figstep{3}{\emph{Cartesian camera rotation.} We want to position the OpenGL camera at the origin of the sonar frame, $\udot{S}$, looking at (i.e. $\hat{z}_C$-axis pointing at) the origin of the image frame, $\udot{I}$. While the position is correct after step 2, we must apply the rotation of $\uvec{S}$ relative to $\uvec{C}$ with a \emph{Cartesian camera matrix} $\mat{T}_\textit{\tiny\!SI}$.
}\\[.5\baselineskip]
%
\figstep{4}{\emph{Radial camera coordinate conversion.} The $\hat{x}_C$ and $\hat{y}_C$ axes span an orthogonal viewing plane onto which the scene's points can be projected. However, to simplify the boundary specification of range and elevation in the next step, we convert the coordinate representation from Cartesian $(\hat{y}_C,\hat{z}_C)$ to cylindrical $(\hat{\phi}_R,\hat{r}_R)$ with a non-linear function T$_{RC}$.
}\\[.5\baselineskip]
%
\figstep{5}{\emph{Projection}. OpenGL only renders coordinates that fall in the range $[-1,1]$. We map the coordinate boundaries (for along-track, range and elevation) into this normalized range with a \emph{projection matrix} $\mat{T}_\textit{\tiny\!XR}$.
% 
...
%\coordinate(CS) at ($(O)!0.5!(S)$);
%\coordinate(RC) at ($(C)!0.5!(O)$);
%\coordinate(XR) at ($(X)!0.5!(C)$);
...
%%\node(project)[left = 5.6em of XR, anchor=center]{\footnotesize\textrm{Projective}};
%\draw[-{Latex[length=.5em,width=.5em]}]
%                          ($(C.west) - (.5em, 0ex)$) to[out=180, in=90] ($(XR) - (2.0em,0ex)$)
%      to[out=-90, in=180] ($(X.west) - (0.8em,0ex)$);
%\draw[-{Latex[length=.5em,width=.5em]},draw=red]                
%                          ($(O.west) - (.5em, 0ex)$) to[out=180, in=90] ($(RC) - (2.0em,0ex)$)
...
%      to[out=-90, in=180] ($(S.west) - (0.7em,0ex)$) to[out=180, in=90] ($(CS) - (2.0em,0ex)$)
...
%	\hline
%	\rowcolor{tabBlue}                     & $\mat{T}_{XR}$      & T$_{RC}$                & $\mat{T}_{CS}$      & $\mat{T}_{SI}$      & $\mat{T}_{IM}$             \\ \hline
%	$\mat{L}$ in (\ref{eq:T_definition})   & See (\ref{eq_L_XR})      & See (\ref{eq_cart2cyl}) & $\mat{R}_{CS}$      & $\mat{I}$           & $\mat{R}_{IM}\mat{S}_{IM}$ \\
%	$\dvec{t}$  in (\ref{eq:T_definition}) & $\boldsymbol{0}$ & See (\ref{eq_cart2cyl}) & $\dvec{p}_{CS}^{O}$ & $\dvec{p}_{SI}^{S}$ & $\dvec{p}_{IM}^{I}$ \\
...
%%\node(project)[left = 5.6em of XR, anchor=center]{\footnotesize\textrm{Projective}};
%\draw[-{Latex[length=.5em,width=.5em]}]
%                        ($(C.center) + (offset) + (0.5em,0ex)$) to[out=0, in=90] ($(XR) + (offset) + (1.6em,0ex)$)
%      to[out=-90, in=0] ($(X.center) + (offset) + (0.8em,0ex)$);
%\draw[-{Latex[length=.5em,width=.5em]},draw=black,densely dotted]                
%                        ($(O.center) + (offset) + (0.5em,0ex)$) to[out=0, in=90] ($(RC) + (offset) + (1.6em,0ex)$)
...
%	                                           Sonar & \t{S} & $S^1$                     &                         &                & 1 & \T{CS}  & Affine      & \Tr{CS}     & \SE                                             \\
%	                               Orthographic view & \t{O} &                           &                         &                & 0 & \TT{RC} & Non-linear  & \TT{RC}     &      \\
%	                                  Cylindric view & \t{C} &                           &                         &                & 0 & \T{XR}  & Affine      & \Tr{XR}     & \\
...
%	                                           Sonar & \t{S} & $\udot{S} = \udot{O}$ & $S^1$            &                &                & 1 & \T{CS}  & Affine     & \Tr{CS}     & \SE  \\
%	                               Orthographic view & \t{O} & $\udot{O} = \udot{C}$ &                  &                &                & 0 & \TT{RC} & Non-linear & \TT{RC}     &      \\
%	                                  Cylindric view & \t{C} &                       &                  & $\mathbb{E}^3$ &                & 0 & \T{XR}  & Affine     & \Tr{XR}     &      \\
...
\cmidrule{14-24}\rowcolor{tabBlue}
   ...
   \end{tabular}}} & \mc{1}{l}{\emph{$\mat{T}$}}                 & \mc{9}{O}{\hspace*{0\tabcolsep}\Emph{$\colon$ Mapping}} &  \\\arrayrulecolor{Green}\hline\arrayrulecolor{black}%cline{10-20}
	                                ...
	                                Radial camera & \t{R}             &\figstep{4}{}&                                             &                             & & &                 &  &                &           &                &   & \mc{11}{L}{\hspace*{-\tabcolsep}\begin{tabular}[c]{p{2.2em} @{ $\colon$} p{7em} @{ $\to$ } p{3.45em} @{ $\in$ } l} $\mathrm{T}_{RC}$ & $(x,y,z)   \in\mathbb{R}^3$          & $(x,\varphi,r)$ &  $\mathbb{R}S\mathbb{R}$ \end{tabular}} \\
	                                      Clipping & \t{X}             &\figstep{5}{}&                                             &                             & & &                 &  &                &           &                &   & \mc{11}{L}{\hspace*{-\tabcolsep}\begin{tabular}[c]{p{2.2em} @{ $\colon$} p{7em} @{ $\to$ } p{3.45em} @{ $\in$ } l} \Tr{XR}           & $(x,\phi,r)\in\mathbb{R}S\mathbb{R}$ & NDC$^{(3)}$  &  $[0,1]^3$ \end{tabular}} \\ \bottomrule
...
%	                                      Clipping & \t{X}             &                                             &                                               &  &                &           &                &   & \Tr{XR}           & \mc{8}{O}{\hspace*{0\tabcolsep}$\colon (x,\phi,r)\in\mathbb{R}S\mathbb{R}$}&\mc{2}{O}{\hspace*{-\tabcolsep}$\to [0,1]^3$}   \\ \bottomrule
%	                 \mc{10}{l}{$^{(1)}$Degrees of freedom} &  \\
%
...
\draw[-{Latex[length=.5em,width=.5em]}]
                        ($(R.center) + (offset) + (1.3em,-0.4ex)$) to[out=-30, in=90] ($(XR) + (offset) + (1.6em,0ex)$)
      to[out=-90, in=0] ($(X.center) + (offset) + (0.8em,0ex)$);
\draw[-{Latex[length=.5em,width=.5em]},draw=black,densely dotted]                
...
%	          Sonar & \t{S}      & \p{SV}{S} & \T{CS} & & Affine: \phantom{scale, }rotate\phantom{, translate}  \\
% Orthographic view & \t{O}      & \p{OV}{O} & \TT{RC}& & Non-linear: cartesian $\rightarrow$ cylindric         \\
%	 Cylindric view & \t{C}      & \p{CV}{C} & \T{XR} & & Affine: orthographic projection                       \\
...
Suppose a $N$-dimensional rigid model $\mathcal{M}$ has a reference frame $\ubar{M} \triangleq (\udot{M},\uvec{M})$ attached to it, where $\udot{M}\in\mathbb{A}$ is its absolute position and $\uvec{M}\triangleq\big(\hat{x}_{M},\hat{y}_{M},\dots\big)\in\mathbb{V}^N$ its absolute orientation (where $\hat{\cdot}$ denotes unit length). 
%
%
%
...
%
It has $N(N+1)/2$ degrees of freedom: $N$ for the translation, with Euclidean topology $\mathbb{R}^N$; and $N(N-1)/2$ for rotation, with angular topology $\{S^2\times{}S^1, S^1\}$ for $N=\{3,2\}$. The symbol $\times$, which denotes the cartesian product of sets, will be dropped and assumed implicit in all set-multiplications from now on. All frames and their configurations are listed in \Tab{IV_tab_frames_transitions}, along with an associated numeric representation. We describe this next.
...
As we dive into the specifics of each transformation, or step, look to \Tab{IV_tab_frames_transitions} for details on its configuration and coordinate representation, and to \Fig{IV_simulator_coordinate_system} for visual aid. Assume the world as three dimensional, and note that each step has a label in the figure, a line in the table, and a matching heading number in the upcoming text.
...
We previously found that the representation of $\mathcal{C}_{IM}$ in $\uvec{I}$ can be written $\boldsymbol{\mathcal{C}}_{IM}^I = (\dvec{p}_{IM}^I,\mat{R}_{IM})$. To allow resizing the model, we also add a scaling factor $s_{IM}$ (see \Tab{IV_tab_frames_transitions}). By inserting $\boldsymbol{\mathcal{C}}_{IM}^I$ into \eq{eq_homogeneous_affine_transform} we obtain the transformation $\vec{T}_{IM}\colon \ubar{M}\to\ubar{I}$ represented in $\ubar{I}$,
%
\begin{align}
\dvec{T}_{IM}^I &= 
\left[\begin{array}{c c}
...
An OpenGL camera has the $x$-axis pointing right, the $y$-axis pointing up and the $z$-axis pointing into the scene. Since this is a left-handed coordinate system---and we prefer right-handed ones---we define the $x$-axis as pointing left instead (we will flip it back later). A camera fixed to $\ubar{S}$ would have a correct position and $x$-axis, but look straight into the seafloor. What remains is to elevate it by the angle given by $\vec{p}_{SI}$. 
...
%
\begin{align}
\dvec{R}_{CS} &= R_x(\arctan\frac{\dvec{p}_{SI,y}^I}{\dvec{p}_{SI,z}^I})
\intertext{or equivalently}
\dvec{R}_{CS} &=
\left[\begin{array}{c c c}
 1 & 0                 & 0                 \\%\hline
 0 & \dvec{p}_{SI,z}^I & \dvec{p}_{SI,y}^I \\
 0 & \dvec{p}_{SI,y}^I & \dvec{p}_{SI,z}^I 
\end{array}\right] \in \mathrm{SO}(3) \label{eq_R_CS}
\end{align}
%


\subsubsection{Radian camera coordinate conversion T$_{\mathrm{RC}}$}
...
\subsubsection{Projection} %Normalization and clipping

\begin{align}
\dvec{R}_{IM} &= \dvec{R}_{\mathrm{Euler}}(\psi,\theta,\phi)\label{eq_R_IM}
\end{align}
\begin{align}
\dvec{T}_{XR}&\colon (x,\phi,r)\in\mathbb{R}^3 \to [r,l][b,t][n,f]
\end{align}

OpenGL clips any coordinate that ends up outside the normalized range $[-1,1]$. This means we need to offset and normalize each axis such that the pixels we want visible end up in this range.
...
\end{align}
%
Now we have all the information needed to normalize the scene in $\ubar{C}$, i.e. the contents of $\mat{T}_{XR}$:
%
\begin{align}
\mat{L}_{XR}
&= \diag\left(\boldsymbol{2}\oslash\dvec{s}_\Delta^C\right) \label{eq_L_XR} \\
\dvec{t}_{XR}
&= -2\dvec{p}_{CI}^C\oslash\dvec{s}_\Delta^C \label{eq_t_XR}
\end{align}
%
