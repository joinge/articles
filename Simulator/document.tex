...
\end{align}
%
The scaling factor shrinks the model if less than 1, \emph{vice versa} if greater. A definition of $\mat{R}_{\mathrm{Euler}}$ is provided in Appendix \ref{IV_transformation_matrices}. Note that $\mat{T}_{IM}^I \ne \mat{T}_{IM}^M$ whenever a translation is involved, so the representation must be specified to avoid ambiguity.
...
\end{align}
%
$\vec{R}_{CS} = \vec{R}(\hat{x}_C, \angle(\hat{z}_S,\vec{p}_{SI}))$ in $\ubar{C}$ by a regular rotation matrix,
%
\begin{align}
\dvec{R}_{\vv{C}{S}} &=
\left[\begin{array}{c c c}
...
There are different ways to project the scene's points onto a camera surface. Perspective projection, mimicking the human eye, scales all $x$ and $y$ coordinates inversely with $z$ to create a sense of perspective in the image. Orthographic projection, by contrast, collapses the depth dimension thereby preserving object size.  Our method of using an orthographic projection on the cylindrical data is a hybrid: $x$ is kept intact and $y$ is inversely scaled with $r$.
...
\end{align}
%
$\vec{L}_{NR} = \vec{I}\,\lVert\vec{p}_{IE}\cdot\hat{u}\rVert^{-1}$ in $\ubar{N}$ by a scaling matrix,
%
\begin{align}
\dvec{L}_{N\v{R}}^N &= \diag\left(\boldsymbol{1}\oslash\dvec{p}_{\dd{I}{E}}^{\v{R}}\right)
\end{align}
...
Rotations are special forms of linear operations that can be expressed in either free or decomposed form. We focus on the Euler-Rodrigues rotation formula, an axis-angle representation introduced by Euler in 1775 that still finds new areas of use e.g. in e.g. the study of coordinate free geometry, quaternions and Lie algebra~\cite{Dai2015}. 


\subsection{Free notation}\label{IV_sec:free_euler_rodrigues}

The Euler-Rodrigues formua has the following free form
%
\begin{align}\label{eq:p_rot_free}
...
&\triangleq %e^{j\overset{\times}{r}\theta}=\nn
 \vec{I} + \sin\theta \vec{S}(\vec{r}_{AB}) + (1-\cos\theta)\vec{r}_{AB}\vec{r}_{AB},
\end{align}
%
where $\vec{I}$ is the identity, and $S(\vec{r}_{AB})$ is the skew-symmetric dyadic form of $\vec{r}_{AB}$,
%
\begin{align}
\overset{\times}{r}
