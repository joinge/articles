...
\end{align}
%
$\vec{R}_{CS} = \vec{R}(\hat{x}_C, \angle(\hat{z}_S,\vec{p}_{SI}))$ in $\ubar{C}$ by a rotation matrix having $\vec{p}_{CI}$ as its bases,
%
\begin{align}
\dvec{R}_{\vv{C}{S}} &=
\left[\begin{array}{c c c}
...
%
\begin{align}\label{IV_eq_euler_rodrigues2}
\vec{R}(\hat{r},\theta) =  \vec{I} + \sin\theta{}S(\hat{r}) + (1-\cos\theta)S^2(\hat{r}),
\end{align}
%
...
If we define the rotation from $\uvec{A}$ to $\uvec{B}$, we denote this with subscripts: $\vec{R}_{AB}$, $\hat{r}_{AB}$ and $\theta_{AB}$.

\subsection{Decomposed notation}

Euler-Rodrigues' formula take on a very similar decomposed form,
%
\begin{align}
\mat{R}
&= \I + \sin\theta{}\mat{S}(\boldsymbol{\hat{r}}) + (1-\cos\theta)\mat{S}^2(\boldsymbol{\hat{r}})\label{IV_eq_euler_rodrigues_decomposed} \\
&= e^{\theta{}\mat{S}(\boldsymbol{\hat{r}})} \in\mathrm{SO}(3)\label{IV_eq_euler_rodrigues_exponential},
\end{align}
%
where $\mat{S}(\boldsymbol{\hat{r}})$ denotes the skew-symmetric matrix form of $\boldsymbol{\hat{r}}$,
%
\begin{align}
\mat{S}
&= \bmat{
...
\end{align}
%
That the complex matrix exponential in \eq{IV_eq_euler_rodrigues_exponential} equals the trigonometric formulation in \eq{IV_eq_euler_rodrigues_decomposed} may be verified by Taylor expansion about $\theta=0$. The result is interesting because it states that the axis-angle parameters are related to the rotation matrix by an exponential map. In Lie terms, it maps the Lie algebra $\mathfrak{so}(3)$ to the Lie group $\mathrm{SO}(3)$.
...
%
\begin{align}
\dvec{q} = \bmat{a & \boldsymbol\omega^T}
\end{align}
...
End-users often prefer Euler-angles over axis-angle parametrizations. These are yaw ($\phi$), pitch ($\theta$) and roll ($\psi$), representing clockwise rotations around the intrinsic $z$-, $y$- and $x$-axis, respectively. Rotation order follow the intrinsic Tait-Brian $Z$-$Y'$-$X''$ convention, in which roll is applied first, then pitch and finally yaw:
%
\begin{align}
...
\subsection{Active vs passive rotations}\label{IV_active_rotations_transforms} 
