...
% <<
% 
where vector subscripts define {\color{cBrightText}direction} and {\color{cBrightText}magnitude}, and superscripts define {\color{cDarkText}representation}. Stated differently, subscripts define geometric features while superscripts define numerical features.\todo{:-D}
...
The transformation matrix usage is similar to that of the rotation matrix, except it requires a homogeneous coordinate $\dvec{\rho}_{MV}^M = [w\dvec{p}_{MV}^M;w]\T\in\mathbb{P}$ as input, where $w\in\mathbb{R}$ (typically 1)\todo{Is this ambivalent},
%
\begin{align}\nonumber\\
...
\end{align}\todo{Chere: This is the color we chose previously.}
%
...
%
\begin{itemize}
\item \emph{Conventional (SIGMAS+).}\todo{Cherie: My lists throughout the article don't have the same style. Symmetry, remember?} Picks the best template out of a set precomputed with SIGMAS+. The templates are tailored to a 3\,m long cylinder; the most frequently occurring object in the scene.
...
\item \emph{Adaptive, buried (FFISim).}\todo{Coherent naming of methods} Finds an "optimal" template with FFISim, given the estimated parameters described above and a search space for burial depth---modeled as rotation by $\{-4,-2,0,2,4\}^\circ$ around the cross track axis $y_\textrm{\tiny$\uvec{I}$}$.
\end{itemize}
