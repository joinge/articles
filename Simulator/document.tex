...
\providecommand\c*{}\renewcommand*\c[2][]{%
%\ifthenelse{\equal{#1}{}}{#2}{%
%\typeout{---Shape provided}%
\ifx&#1&
#2
\else
\tikz[font=\scriptsize]{%
...
\providecommand\cd*{}\renewcommand*\cd[2][]{%
%\ifthenelse{\equal{#1}{}}{#2}{%
%\typeout{---Shape provided}%
\ifx&#1&
#2
\else
\tikz[font=\scriptsize]{%
\node[inner sep=0pt, text depth=0pt](#1){$\udot{#2}$};%
\begin{scope}[overlay]%
\coordinate(offsett) at (0.3em,1.2ex);%
\draw[-]($(#1.center) - (offsett)$) to ($(#1.center) + (offsett)$);%
\end{scope}%}%
...
\providecommand\cv*{}\renewcommand*\cv[2][]{%
%\ifthenelse{\equal{#1}{}}{#2}{%
%\typeout{---Shape provided}%
\ifx&#1&
#2
\else
\tikz[font=\scriptsize]{%
\node[inner sep=0pt, text depth=0pt](#1){$\uvec{#2}$};%
\begin{scope}[overlay]%
\coordinate(offsett) at (0.3em,1.2ex);%
\draw[-]($(#1.center) - (offsett)$) to ($(#1.center) + (offsett)$);%
\end{scope}%}%
...
\providecommand\cb*{}\renewcommand*\cb[2][]{%
%\ifthenelse{\equal{#1}{}}{#2}{%
%\typeout{---Shape provided}%
\ifx&#1&
#2
\else
\tikz[font=\scriptsize]{%
\node[inner sep=0pt, text depth=0pt](#1){$\ubar{#2}$};%
\begin{scope}[overlay]%
\coordinate(offsett) at (0.3em,1.2ex);%
\draw[-]($(#1.center) - (offsett)$) to ($(#1.center) + (offsett)$);%
\end{scope}%}%
...
%\colorlet{cBrightText}{cYellow}
%\colorlet{cDarkText}{cBrown}
%
...
%\providecommand\g*{}\renewcommand*\d[2][]{{\color{cBrightText}\tikzmark{#1}{$\udot{#2}$}}}
%%\providecommand\r*{}\renewcommand*\c[2]{\scriptsize\color{red}\tikzmark{#1}{$#2$}}
%\providecommand\b*{}\renewcommand*\v[2][]{{\color{cDarkText}\ifthenelse{\equal{#1}{}}{#2}{\tikzmark{#1}{$\ubar{#2}$}}}}

\providecommand\k*{}\renewcommand*\k[2][]{{\ifx&#1&#2\else\scriptsize\tikzmark{#1}{$#2$}\fi}}
\providecommand\d*{}\renewcommand*\d[2][]{{\color{cBrightText}\ifx&#1&\udot{#2}\else\scriptsize\tikzmark{#1}{$\udot{#2}$}\fi}}
\providecommand\dd*{}\renewcommand*\dd[3][]{{\color{cBrightText}\ifx&#1&\udot{#2}\udot{#3}\else\tikzmark{#1}{\scriptsize$\udot{#2}\udot{#3}$}\fi}}
\providecommand\v*{}\renewcommand*\v[2][]{{\color{cDarkText}\ifx&#1&\uvec{#2}\else\scriptsize\tikzmark{#1}{$\uvec{#2}$}\fi}}
\providecommand\vv*{}\renewcommand*\vv[3][]{{\color{cDarkText}\ifx&#1&\uvec{#2}\uvec{#3}\else\scriptsize\tikzmark{#1}{$\uvec{#2}\uvec{#3}$}\fi}}
\providecommand\b*{}\renewcommand*\b[2][]{{\color{Green}\ifx&#1&\ubar{#2}\else\scriptsize\tikzmark{#1}{$\ubar{#2}$}\fi}}
\providecommand\bb*{}\renewcommand*\bb[3][]{{\color{Green}\ifx&#1&\ubar{#2}\ubar{#3}\else\scriptsize\tikzmark{#1}{$\ubar{#2}\ubar{#3}$}\fi}}
...
The free form emphasizes geometrical reasoning, conceptual clarity and physical understanding, while the decomposed form ensures geometrical validity and clarity when constructing, manipulating and interpreting coordinates. Together they help the designer focus on higher level concepts rather than getting lost in implementation details, such as mixing up coordinates from different frames.%, such as ensuring that a vectors rather than ensuring that the points are decomposed in the same frame).
...
\end{align}
%
where $\vec{p}_{\td{M}\td{V}} = -\vec{p}_{\td{V}\!\td{M}}$. 

An image frame $\ubar{I}$ specifies the location and orientation of the seafloor area where we want to place the model. We express the displacement $\vec{T}_{\tb{I}\tb{M}}$ of $\ubar{M}$ relative to $\ubar{I}$,
%
\begin{align}
\vec{T}_{\b{I}\b{M}} \colon \ubar{M} &\to \ubar{I},\label{eq_rotation_dyadic}
\intertext{in terms of a translation $\vec{p}_{\d{I}\d{M}}$ that relates their positions,}
\vec{p}_{\d{I}\d{M}} \triangleq \udot{M} &- \udot{I},\label{eq_position_vector}
\intertext{and a rotation $\vec{R}_{\v{I}\v{M}}$ that relates their orientations,}
...
\end{align}
%
When $\dvec{R}_{IM}$ is left-multiplied as here we interpret it as the \emph{orientation} of $\uvec{M}$ \emph{represented} in $\uvec{I}$, and \emph{vice versa} if it is right-multiplied. The multiplication is associative, generally non-commutative and closed.  %Embee
...
%\eq{eq_homogeneous_affine_transform} contains six degrees 
...
\tikz[overlay,remember picture]{
  ...
  \path let \p1 = ($ (I2) - (1.0\baselineskip,0em) $),  \p2 = (R) in coordinate (_I2)  at (\x1,\y2);
  % Draw arrows
%    \draw[color=cDarkText]      ($ (I2.north) + (0em,\tpad) $)
%           to[out= 90, in=0]    (_I2)
%           to[out=180, in=0]    (_I1)
%           to[out=180, in=90]($ (I1.north) + (0em,\tpad) $);
}
...
%
\begin{align}
s_{\vv{I}{M}}\in\mathbb{R}^+,
\end{align}
%
that shrinks the model if less than 1, \emph{vice versa} if greater. A definition of $\mat{R}_{\mathrm{Euler}}$ is provided in Appendix \ref{IV_transformation_matrices}.
...
We can represent $\vec{T}_{SI}\colon \ubar{I}\to\ubar{S}$ in $\ubar{S}$ by
%
\begin{align}
\dvec{T}_{\bb{S}{I}}^{\v{S}} &= 
\left[\begin{array}{c c}
...
%
and $\dvec{p}_{SI}^S$ in $\uvec{I}$ by user-defined coordinates,
%
\begin{align}
...
To perceive the world as a sonar would, we let our camera share the sonar's position ($\udot{C}=\udot{S}$) and x-axis ($\hat{x}_C=\hat{x}_S$), and\todo{but/while/and?} direct it at the image center ($\hat{z}_C\parallel\vec{p}_{SI}$) through a rotation by $\angle(\hat{z}_S,\vec{p}_{SI})$. 
%
%
...
%
\begin{align}
\dvec{R}_{CS} &= \mat{R}_x(\mathrm{atan}\frac{p_{SI,y}^I}{p_{SI,z}^I}),\label{eq_R_CS_angle}
\end{align}
%
...
OpenGL discards any coordinate that ends up outside the normalized range $[-1,1]$. Since $\ubar{R}$ is the natural\todo{imprecise?} domain of the parameters, we can linearly map the valid range for along-track, cross-track and elevation (the volume in \Fig{IV_simulator_coordinate_system} marked by \checkicon{}\!s inside and \crossicon{}s outside) into this normalized range with the affine transform
%
\begin{align}
\dvec{T}_{NR}^N &= 
\left[\begin{array}{c c}
 \mat{L}_{NR}^N  & \dvec{t}_{NR}^N \\%\hline
 \dvec{0}^T  &  1
...
\dvec{T}_{NR}^N &= 
\left[\begin{array}{c c}
 \diag\left(\boldsymbol{1}\oslash\dvec{s}_\Delta^R\right)  & -\dvec{p}_{RI}^R\oslash\dvec{s}_\Delta^R \\%\hline
 \dvec{0}^T  &  1
...
Above 20\% FPR the results are less clear: the adaptive techniques performed similarly to the static method in 2012, worse 6 January 2019 and better 8 January 2009\todo{This is how IEEE JOE dates articles online. Does it read ok?}---with 5 and 3 false detections at 20\% FPR, respectively. From visual inspection we found the cause to be a combination of fish trawler tracks and poor highlight and shadow quality.
