...
\end{itemize}
%
A classification model should neither be too simple nor complex, but strike a balance between the two that captures statistically significant details of the object but not the noise. Lower FRP discards most poor quality objects and therefore raises template quality. the adaptive techniques improve their edge as image qualiperform better than the conventional ones   on higher quality objects.  the object detail level generally increase On average the detail level of  The conventional method seems too simple---also called high bias or underfitted---if its performance is unaffected with improves with more data, and too complicated---also called high variance or overfitted----if its performance data.




The results indicate that 
Image quality is the key here. The conventional method suffers from high bias and underfits the scene the suffers from high bias  underfitting our adaptive template matcher---when viewed as a form of classifier---underfits 
...
%
\begin{itemize}
\item \textit{On-the-fly simulation}, adapting to the scenario at hand;
...
\item \textit{Improved image quality}, because adaptivity 
\item \textit{Lowered false positive rate}, .
\end{itemize}

Unified notation promotes

\begin{itemize}
\item geometrical reasoning
