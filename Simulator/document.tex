...
\end{itemize}
%
A classification model should be neither too simple nor complex, but strike a balance between the two that captures statistically significant details of the object but not the noise. Lower FRP discards most poor quality objects which raises detail level. The conventional method appears too simple---also called high bias or underfitted---to fully benefit from this, lagging while  adaptive techniques excel at higher detail levels, but can easily become too complicated---also called high variance or overfitted---unless their adaptivity is constrained.
...
%
\begin{itemize}
\item \textit{On-the-fly simulation}, enabling full adaption to the scenario at hand and real-time applications adapting to the scenario at hand;
\item \textit{Faster processing speed}, exploring a wider and finer range of templates;
\item \textit{Increased flexibility}, allowing alternative simulation strategies for uncommon scenarios;
\item \textit{Improved image quality}, because adaptive methods ty 
\item \textit{Lowered false positive rate}, because .
\end{itemize}

Our unified notation system has a

\begin{itemize}
\item \emph{free form} promoting geometrical reasoning, conceptual clarity and physical clarity, and
\item \emph{decomposed form} ensuring geometrical validity and clarity when constructing, manipulating and interpreting coordinates,
\item \emph{unified way} of expressing transformations, linear or otherwise, in terms of
\item points denoted $\udot{P}$
