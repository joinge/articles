...
\end{align}
%
$\vec{R}_{CS}\colon \hat{z}_C \to \hat{p}_{CI}$ (subject to $\hat{x}_C=\hat{x}_S$) in $\ubar{C}$ by a regular rotation matrix,
%
\begin{align}
\dvec{R}_{\vv{C}{S}} &=
\left[\begin{array}{c c c}
...
%
%
We represent $\vec{T}_{NR}$ in $\ubar{N}$ by a special Euclidean matrix,
%
\begin{align}
\dvec{T}_{N\b{R}}^N &= 
\left[\begin{array}{c c}
...
\end{align}
%
$\vec{L}_{NR} = \frac{\vec{I}}{\lVert\vec{p}_{IE}\cdot\hat{u }\rVert}$ in $\ubar{N}$ by a scaling matrix,
%
\begin{align}
\dvec{L}_{N\v{R}}^N &= \diag\left(\boldsymbol{1}\oslash\dvec{p}_{\dd{I}{E}}^{\v{R}}\right)
\end{align}
