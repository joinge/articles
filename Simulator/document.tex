...
\IEEEPARstart{A}{utonomous} underwater vehicles (AUVs) are vital in the exploration of underwater frontiers. When equipped with a modern synthetic aperture sonar (SAS), they map seafloors, survey underwater biology, help localize wrecks, inspect oil pipes, fields and electricity lines, or help remove mines, biohazard waste or plastic. However, they struggle to deal with unforeseen events, such as poor imaging conditions, blocked paths, corrupted data or rare objects. In other words, their automatic target recognition (ATR) are insufficiently reliable and adaptive.
...
Typical conventional ATR isolates objects of interest, extracts their key features and classifies them accordingly~\cite{Duda2012}. The challenge is finding features that are unique and intrinsic, such as type, shape and size, independent of extrinsic ones such as alignment, burial depth, degeneration level or view angle and range. Inferring three-dimensional features from a two-dimensional projection is ambiguous, particularly from SAS images---despite their best-in-class resolution and dynamic range~\cite{Hayes2009}---due acoustical distortions such as layover, multiple scattering, refraction, diffraction and penetration~\cite{Jensen2011}.


Template matching---a model-based alternative to feature extraction---compares isolated objects with image templates of relevant classes and orientations, and assigns them the best fitting class. The difficulty is representing the objects properly, considering only a few degrees of freedom can be properly sampled with a manageable number of templates (curse of dimensionality). Therefore, practical implementations resort to a few sparsely sampled features, ultimately yielding inaccurate results~\cite{Midelfart2010}.

%The templates are typically simulated.
%Too many templates are needed to densely sample each degree of freedom.

Adaptive template matching---a combination of the two aforementioned solutions---extracts the most reliable statistics from the image, such as highlight, shadow and background characteristics\cite{Reed2004,Quidu2000,Fawcett2005,Fei2015}, then uses these to limit relevant template configurations~\cite{Midelfart2009,Midelfart2010,Myers2010}. This enables fine-grained search through more features, at reduced computational load and improved accuracy. We seek to improve this further, by simulating templates on-the-fly to eliminate the template library altogether.
...
This study describes and demonstrates a simulator designed from the ground up as part of a GPU-accelerated adaptive template matcher. It uses both OpenGL and the general-purpose programming framework OpenCL; two libraries that can inter-operate on the GPU. To describe the simulator, we extend a new navigational notation system with homogeneous coordinates and some ideas from modern robotics, to express complicated geometries in a rigorous, unified and simple way.
...
\cmidrule{14-24}\rowcolor{cBase}
   ...
   \end{tabular}}} & \mc{1}{l}{\emph{$\mat{T}$}}                 & \mc{9}{O}{\hspace*{0\tabcolsep}\Emph{$\colon$ Mapping}} &  \\\arrayrulecolor{cBaseMark}\hline\arrayrulecolor{black}%cline{10-20}
	                                ...
	                                Radial camera & \t{R}             &\figstep{4}{}&                                             &                             & & &                 &  &                &           &                &   & \mc{11}{L}{\hspace*{-\tabcolsep}\begin{tabular}[c]{p{2.2em} @{ $\colon$} p{7em} @{ $\to$ } p{3.45em} @{ $\in$ } l} $T_{RC}$ & $(x,y,z)   \in\mathbb{R}^3$          & $(x,\varphi,r)$ &  $\mathbb{R}S\mathbb{R}$ \end{tabular}} \\
	                                      Normalized coordinates & \t{N}             &\figstep{5}{}&                                             &                             & & &                 &  &                &           &                &   & \mc{11}{L}{\hspace*{-\tabcolsep}\begin{tabular}[c]{p{2.2em} @{ $\colon$} p{7em} @{ $\to$ } p{3.45em} @{ $\in$ } l} \Tr{NR}           & $(x,\phi,r)\in\mathbb{R}S\mathbb{R}$ & NDC$^{(3)}$  &  $[-1,1]^3$ \end{tabular}} \\ \bottomrule
...
The upcoming notation may at first seem strange, but we believe some long-term insights and benefits await those that persevere to understand. We begin with the core idea of marking symbols of positions, orientations and reference frames with dots, vectors and bars, respectively. 
%
...
The model in \Fig{IV_simulator_coordinate_system} is a rigid object with reference frame $\ubar{M} \triangleq (\udot{M},\uvec{M})$, where $\udot{M}\in\mathbb{A}$ is its absolute position and $\uvec{M}\in\mathbb{V}^N$ is its absolute orientation.
%
%
...
\end{align}
%
$\vec{R}_{IM}$ in $\ubar{I}$ by user-defined Euler angles $(\psi,\theta,\phi)\in[0,2\pi][0,\pi][0,2\pi]$,
%
\begin{align}
...
\end{align}
%
$\odot$ denotes Hadamard (elementwise) product and $\{N_x, N_\varphi, N_r\}$ are the user-defined number of samples for along-track, elevation and range, respectively. To form the sonar image $\dvec\gamma_n^R$, we accumulate all intensities in elevation that share a resolution cell in along-track and range. Pseudo-code for this can be written as
%
...
Improved classification accuracy and speed pave the way for 

heterogeneous clusters of AUVs ~\cite{Midtgaard2019}
