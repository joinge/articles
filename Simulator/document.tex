...
Template matching is a common technique for automatic classification of objects in synthetic aperture sonar (SAS) images. The principle is to correlate an image segment with a set of template images for typical object configurations, and assign it the class of the template yielding the highest correlation coefficient.
...
Once the simulator runs it produces thousands of sonar templates per second. However, loading OpenGL, OpenCL and 3D models from file consumes seconds per invocation. Therefore, the simulator is either run as preloaded server process or as batch run with a list of parameters.
...
% Speed + adaptivity -> better classification
% 
% Notation
...
%
\begin{itemize}
\item on-the-fly simulation, adapting to the scenario at hand;
\item faster processing speed, exploring a wider and finer range of templates;
\item increased flexibility, allowing alternative simulation strategies for uncommon scenarios;
\item improved image quality, because  
\item lowered FRP, because .
\end{itemize}

\begin{itemize}
\item simulating templates on-the-fly improves classification accuracy and flexibility compared to static template databases.
\item Processing speed follows from our GPU implementation using OpenGL and OpenCL, which is effective, flexible, portable and overwhelmingly fast.
\item Classification accuracy of the adaptive methods improves higher image quality, lower FRP, or both.  
