...
In the rendering process OpenGL discards any coordinate that ends up outside the range $[-1,1]$, called normalized device coordinates (NDC). The map into this range defines how points project onto the camera surface. Perspective projection, mimicking the human eye, scales coordinates inversely with depth to create a sense of perspective in the image. Orthographic projection, by contrast, is an affine transformation that preserves object size. Neither of these are a good fit for a ranged-sonar.\todo{meh}
...
The along-track, cross-track and elevation boundaries form a cylindrical slice, the volume in  \Fig{IV_simulator_coordinate_system} marked by \checkicon{}\!s inside and \crossicon{}s outside. Since the boundaries are non-linear in $\ubar{C}$, but linear in $\ubar{R}$, we can apply an affine transform similar to an orthographic projection, but on the cylindric representation instead of the Cartesian. 
% .  here It consists of a linear transform $\vec{L}_{NR}$ that scales the volume and a position vector $\vec{p}_{NR}$ that centers it, $\mathcal{C}_{NR} = \big(\vec{p}_{NR},\vec{L}_{NR}\big)$. .  
%
...
%
%
We skip free notation (it carries little merit when manipulating coordinates) and define $\dvec{T}_{NR}^N$ as a general linear (invertible) $\mathbb{R}^{4,4}$ matrix,
%
\begin{align}
\dvec{T}_{N\b{R}}^N &= 
\left[\begin{array}{c c}
...
\end{align}
%
where $\mat{L}_{NR}^N$ scales the axes to $\{[a-1,a+1]\colon a\in\mathbb{R}\}$,
%
\begin{align}
...
%
and $\dvec{p}_{NR}^N$ centers them to $[-1,1]$ (removes the $a$),
%
\begin{align}
...
% and diag($\cdot$) constructs a diagonal matrix from its input vector
