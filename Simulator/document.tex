...
% An OpenGL camera $\ubar{G}$ has $\hat{x}_G$ pointing right, $\hat{y}_G$ pointing up and $\hat{z}_G$ pointing into the camera. We can elevate the sonar frame by $\angle(\hat{z}_S,\vec{p}_{SI})$ 
...
%
This leaves no degrees of freedom for $\vec{T}_{CS}\colon \udot{S}\to\udot{C}$, which we represent in $\ubar{C}$ as\todo{Cherie: This paragraph still doesn't make sense.}
%
