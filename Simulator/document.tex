...
\end{align}
%
$\vec{R}_{CS} = \vec{R}(\hat{x}_C, \angle(\hat{z}_S,\vec{p}_{SI}))$ in $\ubar{C}$ by a rotation matrix where $\vec{p}_{CI}$ defines $\hat{y}_{CS}$ and $\hat{z}_{CS}$,
%
\begin{align}
\dvec{R}_{\vv{C}{S}} &=
\left[\begin{array}{c c c}
...
\dvec{q} = \bmat{a & \boldsymbol\omega} = \bmat{\cos\frac{\theta}{2} & \dvec\omega^T\sin\frac{\theta}{2}}
\end{align}
% 
...
\end{align}
%
where $\dvec{q}^*$ negates the three last coefficients ($\dvec\omega$). We may use our notation system by wrapping \eq{IV_eq_quaternion_usage} into a function,
%
...
\end{align}
%
Quaternions can reduce computational complexity, improve numerical stability and facilitate efficient spherical interpolations. 
