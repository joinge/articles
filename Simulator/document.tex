...
\providecommand\g*{}\renewcommand*\g[2]{{\scriptsize\color{cBrightText}\tikzmark{#1}{$#2$}}}
%\providecommand\r*{}\renewcommand*\r[2]{\scriptsize\color{red}\tikzmark{#1}{$#2$}}
\providecommand\b*{}\renewcommand*\b[2]{{\scriptsize\color{cDarkText}\tikzmark{#1}{$#2$}}}
...
\end{align}
%
where $\r{M}{M}$ denotes cancellation of the intermediate frame $\ubar{M}$ and arrows emphasize composition of the output vector.
...
% <<
% 
where vector subscripts define {\color{cBrightText}direction} and {\color{cBrightText}magnitude}, and superscripts define {\color{cDarkText}representation}. Stated differently, subscripts define geometric features while superscripts define numerical features.
...
We previously found that the representation of $\mathcal{C}_{IM}$ in $\uvec{I}$ can be written $\boldsymbol{\mathcal{C}}_{IM}^I = (\dvec{p}_{IM}^I,\mat{R}_{IM})$. To allow resizing the model, we now also add a scaling factor $s_{IM}$ (see \Tab{IV_tab_frames_transitions}). By inserting this into \eq{eq_homogeneous_affine_transform} we obtain the transformation $\vec{T}_{IM}\colon \ubar{M}\to\ubar{I}$ represented in $\ubar{I}$,
%
\begin{align}%\nonumber\\
\dvec{T}_{\g{IM1}{IM}}^{\b{I1}{I}} &= 
\left[\begin{array}{c c}
 \k{C}{}\mat{R}_{\g{IM2}{IM}}  & \dvec{p}_{\g{IM3}{IM}}^{\b{I2}{I}} \\%\hline
...
\end{array}\right]
% = \mat{T}_{\g{II2}{I}\r{MM1}{M}}^{\b{II3}{I}}\boldsymbol{\rho}_{MV}^M.
\\\nonumber
\tikz[overlay,remember picture]{
  % Set up some general bounding box markers
%  \coordinate(_MV)  at ($   (V.south)    + (-1.0\baselineskip,-1.0\baselineskip) $);
...
\end{align}
%
and the scaling factor by a positive number, $s_{IM}\in\mathbb{R}^+$, that shrinks the model if it is less than 1, and \emph{vice versa} if it is greater. These user-defined parameters are unconstrained, thus add seven degrees of freedom to the system. For a definition of $\mat{R}_{\mathrm{Euler}}$, see Appendix \ref{IV_transformation_matrices}.
...
Above 20\% FPR the results are less clear: the adaptive techniques performed similarly to the static method in 2012, worse January 6. and better January 8. 2009---with 5 and 3 false detections at 20\% FPR, respectively. From visual inspection we found the cause to be a combination of fish trawler tracks and poor highlight and shadow quality.
...
With good image quality or low FRP, or both, the adaptive methods did better. However, when this was not the case, it was less clear who the winner was. The conventional method of using a static template library generally did well here.\todo{Meh. Fix fix fix}
