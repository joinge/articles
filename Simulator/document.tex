...
We previously found that the representation of $\mathcal{C}_{IM}$ in $\uvec{I}$ can be written $\boldsymbol{\mathcal{C}}_{IM}^I = (\dvec{p}_{IM}^I,\mat{R}_{IM})$. To allow resizing the model, we also add a scaling factor $s$ (see \Tab{IV_tab_frames_transitions}). The transform that applies this configuration, $\vec{T}_{IM}\colon \ubar{M}\to\ubar{I}$, can be represented in $\ubar{I}$ by inserting $\boldsymbol{\mathcal{C}}_{IM}^I$ into \eq{eq_homogeneous_affine_transform},
%
\begin{align}
\dvec{T}_{IM}^I &= 
\left[\begin{array}{c c}
...
\end{align}
%
A transform that scales an object but otherwise preserves it is called a similarity transform (Sim). Note that, unlike the rotation matrix, we generally have that $\mat{T}_{IM}^I \ne \mat{T}_{IM}^M$, so here the representation must be specified.
...
\end{align}
%
and the scaling factor by a positive number that shrinks or enlarges the model depending on whether it is less or greater than 1, respectively. For a definition of $\mat{R}_{\mathrm{Euler}}$, see Appendix \ref{IV_transformation_matrices}.




\subsubsection{Sonar translation $\dvec{T}_{\!IM}$}

The sonar generally does not share the orientation of the to the same orientation of the image region, $\uvec{S}=\uvec{I}$, but has a different position, so the configuration $\mathcal{C}_{SI}$ has three degrees of freedom. This is identical to $\mathcal{C}_{IM}$ except for the lack of a scaling factor. We represent the displacement $\vec{T}_{SI}$ in $\uvec{S}$ as,
%
\begin{align}
