...
We describe the implementation with a new navigational notation system by extending it with topological ideas from robotics and homogeneous coordinates. This ensures geometrical clarity and validity while aiding the designer in focusing on higher level concepts, such as manipulating rigid objects. Supplementary Python scripts and media files showcase the notation and implementation in practice.
\end{abstract}
%  
%
%, 
...
\quotesave{IV_text_introduction}{
\IEEEPARstart{A}{utonomous} underwater vehicles (AUVs) are vital in the exploration of underwater frontiers. When equipped with modern synthetic aperture sonars (SAS), they map seafloors, survey underwater biology, help localize wrecks, inspect oil pipes and electricity lines, and help remove mines, biohazard waste or plastic. However, they struggle to deal with unforeseen events, such as poor imaging conditions, blocked paths, corrupted data or rare objects; their automatic target recognition (ATR) seems insufficiently reliable and adaptive.
...
%objects isolated, features extracted and class estimated 
%
%estimates object class from features extracted from 
%and estimates their classes


Typical conventional ATR isolates objects of interest, extracts their key features and classifies them accordingly~\cite{Duda2012}. The challenge is finding features that are unique and intrinsic, such as type, shape and size, independent of extrinsic ones such as alignment, burial depth, degeneration level, view angle and range. Inferring three-dimensional features from a two-dimensional projection is ambiguous, particularly from SAS images---despite their best-in-class resolution and dynamic range~\cite{Hayes2009}---due to acoustical distortions such as layover, multiple scattering, refraction, diffraction and penetration~\cite{Jensen2011}.

%
Template matching---a model-based alternative to feature extraction---compares isolated objects with image templates of relevant classes and orientations, and assigns them the best fitting class. Representing the objects with templates is difficult, considering only a few degrees of freedom can be properly sampled with a manageable number of templates (the dimensionality curse). Therefore, practical implementations resort to a few sparsely sampled features, ultimately yielding inaccurate results~\cite{Midelfart2010}.


Adaptive template matching---a combination of the two aforementioned solutions---extracts the most reliable statistics from the image, such as highlight, shadow and background characteristics~\cite{Reed2004,Quidu2000,Fawcett2005,Fei2015}, then uses these to limit relevant template configurations~\cite{Midelfart2009,Midelfart2010,Myers2010}. This enables fine-grained searching through more features, at reduced computational load and improved accuracy. We seek to improve this further, by simulating templates on-the-fly to eliminate the template library altogether.

% normalized shadow-echo matching (NSEM)
...
%frequency domain, finite difference, finite element / boundary element, ray tracing, rasterization


Most simulators for high frequency sonar imagery prioritize modeling accuracy over execution speed~\cite{Bell1995,Hunter2006}, usually by solving wave equations~\cite{Elston2004} or performing ray-tracing (e.g. Heriot-Watt~\cite{Bell1997}, Limerick~\cite{Riordan2005}, SIGMAS~\cite{Coiras2009}, SWAT~\cite{Sammelm2003}). Fast sonar simulators typically adapt rasterization techniques for full acceleration by graphics processing units (GPUs)~\cite{Cerqueira2017}, such as the SIGMAS+ simulator~\cite{Coiras2009a} from NATO Centre for Maritime Research and Experimentation (CMRE). Aligning with our goals, it assumes Lambertian scattering and renders sonar images from 3D models with the OpenGL computer graphics library. However, it includes stochastic effects such as ambient noise, which we deem irrelevant for templates, and lacks general programmability, which limits adaptivity.}
...
This study describes and demonstrates a simulator designed as part of a GPU-accelerated adaptive template matcher. It uses both OpenGL and the general-purpose programming framework OpenCL; two libraries that inter-operates on the GPU. To describe the simulator, we extend a new navigational notation system with homogeneous coordinates and some ideas from modern robotics, to express complicated geometries in a rigorous, unified and simple way.

Section \ref{IV_sec_methods} describes the simulator concept, underlying geometry and notation system. Section \ref{IV_sec_general_notation} and \ref{IV_sec_decomposed_notation} details the notation without and with coordinates, respectively. Section \ref{IV_sec_results} presents classification results on experimental data of a few thousand objects from Jesus Bay, Norway, recorded by the HISAS1030 100\,kHz SAS mounted on a HUGIN AUV from Kongsberg Maritime in 2009 and 2012. Section~\ref{IV_sec_results_selected_objects} verifies the design through qualitative analysis of SAS, simulation and segmentation images of selected objects, and Section~\ref{IV_sec_results_all_objects} validates the design through quantitative analysis of receiver operating characteristic (ROC) curves. Section~\ref{IV_sec_conclusion} concludes with a list of key merits for the design and notation. 

\section{Methods}\label{IV_sec_methods}
...
\begin{tabular}{r c >{\hspace*{\numhspace}} C >{\hspace*{0.5em}} O Q >{\hspace*{-.5em}} Q Q Q C O C C C L >{\hspace*{-2\tabcolsep}}L O O O >{\hspace*{\tabcolsep}} O O O >{\hspace*{\tabcolsep}}  O O @{\hspace*{\tabcolsep}\shiftdown{$\in$}\hspace*{\tabcolsep}} L}
   ...
   \cmidwrap{\cmidrule(lr){1-2}\cmidrule(lr){3-8}\cmidrule(lr){9-13}\cmidrule(lr){14-24}}\rowcolor{cBase} 
	\emph{Description} & \emph{Symbol} & \Emph{} & \Emph{$\vec{T}$} & \mc{4}{O}{\Emph{$\colon$ Mapping}} & \Emph{$\mathcal{C}($} & \Emph{$\vec{p}$\addtimes} & \Emph{$\vec{R}$\addtimes} & \Emph{$s)$} &  \Emph{DoF$^{(1)}$} &  \mc{1}{l}{\emph{$\mat{T}$}} & \Emph{$($} & & \Emph{$\dvec{p}$} & \Emph{$\phantom{z},$}  & & \Emph{$\dvec{R}$} & \Emph{$\phantom{\varphi},$} &  \Emph{$s$} & \mc{1}{>{\hspace{-1.75\tabcolsep}}l}{$)$} & \Emph{Group} \\%\Emph{Sum} \\
	\midline
	                                         Model & \t{M}             &&                                             &          &            &       &                           &  &                &           &                &   & \mcc{1}{L}{}      &                                                                      &      &      &      &         &           &         &     & \mcc{1}{C}{} &      \\
	                                         Image & \t{I}             &\figstep{1}{}& \T{IM}                                      & $\colon$ & $\ubar{M}$ & $\to$ & $\ubar{I}$                &  & $\mathbb{R}^3$ & $S^2 S$ & $\mathbb{R}^+$ & 7 & \Tr{IM}           & $($                                                                  & $x,$ & $y,$ & $z,$ & $\psi,$ & $\theta,$ & $\varphi,$ & $s$ & $)$          & \SIM \\
...
\cmidrule{14-24}\rowcolor{cBase}
   ...
   \end{tabular}}} & \mc{1}{l}{\emph{$\mat{T}$}}                 & \mc{9}{O}{\hspace*{0\tabcolsep}\Emph{$\colon$ Mapping}} &  \\\arrayrulecolor{cBaseMark}\hline\arrayrulecolor{black}%cline{10-20}
	                                ...
	                                Radial camera & \t{R}             &\figstep{4}{}&                                             &                             & & &                 &  &                &           &                &   & \mc{11}{L}{\hspace*{-\tabcolsep}\begin{tabular}[c]{p{2.2em} @{ $\colon$} p{7em} @{ $\to$ } p{3.45em} @{ $\in$ } l} $T_{RC}$ & $(x,y,z)   \in\mathbb{R}^3$          & $(x,\varphi,r)$ &  $\mathbb{R}S\mathbb{R}$ \end{tabular}} \\
	                                      Normalized coordinates & \t{N}             &\figstep{5}{}&                                             &                             & & &                 &  &                &           &                &   & \mc{11}{L}{\hspace*{-\tabcolsep}\begin{tabular}[c]{p{2.2em} @{ $\colon$} p{7em} @{ $\to$ } p{3.45em} @{ $\in$ } l} \Tr{NR}           & $(x,\varphi,r)\in\mathbb{R}S\mathbb{R}$ & NDC$^{(3)}$  &  $[-1,1]^3$ \end{tabular}} \\ \bottomrule
...
\end{tabular}
%	                                Cylindric view & \t{C}             &                                             &                                               &  &                &           &                &   & $\mathrm{T}_{RC}$ & \mc{8}{O}{\hspace*{0\tabcolsep}$\colon (x,y,z)\in\mathbb{R}^3$}&\mc{1}{O}{$\to (x,\varphi,r)\in$}&$\mathbb{R}S\mathbb{R}$   \\
%	                                      Clipping & \t{X}             &                                             &                                               &  &                &           &                &   & \Tr{NR}           & \mc{8}{O}{\hspace*{0\tabcolsep}$\colon (x,\varphi,r)\in\mathbb{R}S\mathbb{R}$}&\mc{2}{O}{\hspace*{-\tabcolsep}$\to [0,1]^3$}   \\ \bottomrule
%	                 \mc{10}{l}{$^{(1)}$Degrees of freedom} &  \\
%
...
\caption{Reference frames and their transformations. The upper half is an affine domain (metric) where frames relate through poses, each defined by a specific configuration and coordinate representation. The lower half contains non-linear and non-metric domains, their coordinate transformations described with the same uniform notation. All set-multiplications are Cartesian.}\label{IV_tab_frames_transitions}
\end{table*}
...
\Fig{IV_simulator_coordinate_system} and \Tab{IV_tab_frames_transitions} detail the geometry and processing steps associated with the upcoming implementation details, with supplementary source code and media files described in Appendix \ref{IV_sec_source_code}. The first steps detail the transformations a vertex $\udot{V}$ undergoes---from being represented in the reference frame of a model, $\ubar{M}$, to that of an OpenGL camera, $\ubar{C}$---then discuss rendering and post-processing details. The mathematical syntax comes from a rigorous navigational notation system and rule set developed by K.~Gade~\cite{Gade2018}.
...
Gade's notation has a coordinate-free form that promotes geometrical reasoning, conceptual clarity and physical understanding~\cite{Derose1992}, and a decomposed form that facilitates geometrical clarity and validity when implementing coordinates numerically. Combined, they help the designer focus on higher level concepts rather than getting lost in implementation details, such as mixing coordinates from different frames. 

%, such as ensuring that a vectors rather than ensuring that the points are decomposed in the same frame).
%
...
We adhere to Gade's notation for affine geometry, extend it to handle homogeneous coordinates and projective geometry, and suggest a slight tweak for arbitrary functions. This allows representing  transitions between reference frames in a unified and geometrically valid way. Inspired by modern robotics, we also touch on topology and Lie group theory of rigid objects~\cite{Lynch2017,Murray1994}.
...
\subsection{Free notation}\label{IV_sec_general_notation}

The upcoming notation may at first seem strange, but we believe long-term insights and benefits await those that persevere to understand. This section explains Gade's core idea of marking symbols of positions, orientations and reference frames with dots, vectors and bars, respectively. We relate positions by translations, orientations by rotations and reference frames by poses. Section \ref{IV_sec_decomposed_notation} adds coordinates to translations and rotations, while Section \ref{IV_sec_homogeneous_coordinates} extends Gade's notation to poses and non-linear operations. This results in a cascade of uniformly looking, standardized transforms that fully describes the simulator geometry.
...
%The rigid object in \Fig{IV_simulator_coordinate_system}, we denote its position as $\udot{M}\in\mathbb{A}$, orientation as 
Let the model in \Fig{IV_simulator_coordinate_system} be a rigid object with reference frame $\ubar{M} \triangleq (\udot{M},\uvec{M})$, where $\udot{M}\in\mathbb{A}$ is its absolute position and $\uvec{M}\in\mathbb{V}^N$ is its absolute orientation.
%
%
...
%
%
We relate positions with position vectors, such as $\vec{p}_{\td{M}\td{V}}\in\mathbb{V}$ going from $\udot{M}$ to $\udot{V}$,
%
\begin{align}\label{eq_p_MV_free}
\vec{p}_{\udot{M}\udot{V}} &\triangleq \udot{V} - \udot{M},
\end{align}
%
where $\vec{p}_{\udot{M}\udot{V}} = -\vec{p}_{\udot{V}\udot{M}}$. The subscript dots explicitly state the position vector's sole dependence on positions.  %, for geometric clarity. We can also say things like $   %Dropping the annotations leads to implicit notation, but comes at no cost of generality. %To demonstrate both forms we will use implicit notation in the text and explicit notation in the standalone equations, color coded for added clarity.

The image frame $\ubar{I}$ in \Fig{IV_simulator_coordinate_system} specifies the location and orientation of the seafloor area where we want to place the model. We express the \emph{pose} (or displacement) $\vec{T}_{\ubar{I}\ubar{M}}$ of {\color{cFrame}frame} $\ubar{M}$ relative to {\color{cFrame}frame} $\ubar{I}$,
%
\begin{align}
...
%
It has $N(N+1)/2$ degrees of freedom: $N$ for the translation, with Euclidean topology $\mathbb{R}^N$; and $N(N-1)/2$ for rotation, with angular topology $\{S^2\times{}S^1, S^1\}$ for $N=\{3,2\}$. The symbol $\times$, which denotes the Cartesian product of sets, will be dropped and assumed implicit in upcoming set-multiplications. We also drop subscript and superscript accents and colors from symbols in the text, but keep it for comparison in the standalone equations. All frames and their configurations are listed in \Tab{IV_tab_frames_transitions}, along with an associated numeric representation. We describe this next.
...
\subsection{Decomposed notation}\label{IV_sec_decomposed_notation}
...
\subsection{Homogeneous transformations}\label{IV_sec_homogeneous_coordinates}

Any affine transformation in $\mathbb{R}^N$-space can be represented by a transformation matrix $\dvec{T} \in \mathbb{R}^{N+1,N+1}$~\cite{Klein2004}. In the affine case, it has the form of a Special Euclidean matrix $\dvec{T}\in\mathrm{SE}(N)$,
%
\begin{align}
\dvec{T} &= 
...
\end{align}
%
$\vec{R}_{IM}$ by user-defined Euler angles $(\psi,\theta,\varphi)\in[0,2\pi][0,\pi][0,2\pi]$,
%
\begin{align}
\mat{R}_{\vv{I}{M}}    &= \mat{R}_{\mathrm{eu}}(\psi,\theta,\varphi) \in SO(3),
\end{align}
%
...
\end{align}
%
The scaling factor shrinks the model if less than 1, \emph{vice versa} if greater. Appendix \ref{IV_sec_euler_angles} defines the Euler rotation matrix $\mat{R}_{\mathrm{eu}}$. Since $\mat{T}_{IM}^I \ne \mat{T}_{IM}^M$ whenever a translation is involved, the representation of transformation matrices must be specified to avoid ambiguity.
...
Suppose the image lies some distance from the sonar ($\udot{S}\ne\udot{I}$) but shares its orientation ($\uvec{S}=\uvec{I}$), resulting in the configuration $\mathcal{C}_{SI} = (\vec{p}_{SI},\vec{I})$ where $\vec{I}$ denotes identity (no change).
%
...
\end{align}
%
and $\vec{p}_{SI}$ in $\uvec{S}$ by user-defined coordinates,
%
\begin{align}
...
\end{align}
%
$\vec{R}_{CS}$ by a rotation matrix where $\vec{p}_{CI}$ defines $\hat{y}_{CS}$ and $\hat{z}_{CS}$ (alternatively the Euler-Rodrigues rotation $\dvec{R}_\mathrm{er}(\hat{x}_C, \angle(\hat{z}_S,\vec{p}_{SI}))$ defined in Appendix \ref{IV_sec_euler_rodrigues_decomposed}),
%
\begin{align}
\dvec{R}_{\vv{C}{S}} &=
\left[\begin{array}{c c c}
...
%
\begin{align}
\dvec{p}_{\dd{C}{S}}^{\,\v{C}} &= \boldsymbol{0} \in \mathbb{R}^3.
\end{align}


\subsubsection{Camera coordinate conversion \texorpdfstring{T$_{RC}$}{T\_RC}}\label{IV_sec_radial_camera}

%\figstep{4}{\emph{Radial camera coordinate conversion.} The $\hat{x}_C$ and $\hat{y}_C$ axes span an orthogonal viewing plane onto which the scene's points can be projected. However, to simplify the boundary specification of range and elevation in the next step, we convert the coordinate representation from Cartesian to radial (cylindrical) with a non-linear function T$_{RC}\colon (\hat{x}_C,\hat{y}_C,\hat{z}_C) \mapsto (\hat{x}_R,\hat{\varphi}_R,\hat{r}_R)$ subject to $\hat{x}_R=\hat{x}_C$.
%}\\[.5\baselineskip]

%T$_{RC}\colon (\hat{x}_C,\hat{y}_C,\hat{z}_C) \mapsto (\hat{x}_R,\hat{\varphi}_R,\hat{r}_R)$ subject to $\hat{x}_R=\hat{x}_C$.
...
To prepare for this computation and ease specification of spatial system boundaries, we change the representation from Cartesian to cylindrical ($\ubar{R}$ for "ranged-device") with a non-linear function $T_{RC}\colon (\hat{x}_C,\hat{y}_C,\hat{z}_C) \mapsto (\hat{x}_R,\hat{\varphi}_R,\hat{r}_R)$ subject to $\hat{x}_R=\hat{x}_C$,
%
...
\end{align}
%
$T_{RC}$ needs no superscript since this is a pure change of reference.
...
% 
% \begin{align}
% \dvec{R}_{IM} &= \dvec{R}_{\mathrm{Euler}}(\psi,\theta,\varphi)\label{eq_R_IM}
% \end{align}
% \begin{align}
% \dvec{T}_{NR}&\colon (x,\varphi,r)\in\mathbb{R}^3 \to [r,l][b,t][n,f]
% \end{align}
...
\end{align}
%
where $\mat{L}_{NR}^N$ scales the volume to $[\dvec{p}_{RI}^R\pm\dvec{1}]$,
%
\begin{align}
\dvec{L}_{N\v{R}}^N &= \left[\begin{array}{c c c}
p_{\dd{I}{E},x}^{\,\v{R}} & 0 & 0 \\
0 & p_{\dd{I}{E},\varphi}^{\,\v{R}} & 0 \\
0 & 0 & p_{\dd{I}{E},r}^{\,\v{R}}
\end{array}\right]^{-1}
\end{align}%\boldsymbol{1}\oslash\dvec{p}_{\dd{I}{E}}^{\,\v{R}}
%
and $\dvec{p}_{NR}^N$ centers it to $\mathcal{V}^N\in[-1,1]^3$,
%
\begin{align}
...
%
\begin{align}\nonumber\\
\dvec{\rho}_{\tikzmark{NV}{$\scriptstyle{}N\color{cPoint}\udot{V}$}}^{\,\n[N1]{\scriptstyle{}N}} &= \mat{T}_{\n[N2]{\scriptstyle{}N}\b{R}}^{\,\n[N3]{\scriptstyle{}N}} \;T_{\vv{R}{C}}(\mat{T}_{\bb{C}{S}}^{\v{C}}\;\mat{T}_{\bb{S}{I}}^{\v{S}}\;\mat{T}_{\bb{I}{M}}^{\v{I}} \dvec{\rho}_{\d{M}\d[V1]{V}}^{\v{M}} ),
%\left[\begin{array}{c}
%\dvec{p}_{\cd[M2]{M}\d[V1]{V}}^{\cv[M3]{M}} \\ \tikzmark{One}{1}
%\end{array}\right].
...
The rendered intensity image $\dvec\alpha_n^R$ and depth image $\dvec\beta_n^R$ represent the sonar's view of our defined volume. To form the sonar image $\dvec\gamma_n^R$, we accumulate all intensity values in elevation that share a resolution cell in along-track and range. Listing \ref{IV_listing_pseudo_code} supplies pseudo-code for this,
%
...
%\begin{minipage}{\linewidth}
%\begin{table}
%\lstset{language=, caption=Pseudo-code for combining the OpenGL intensity and depth\\ image into a sonar-line image., label=IV_listing_pseudo_code}
\begin{lstlisting}[belowskip=-0.8\baselineskip, float=h, caption=Pseudo-code for combining the OpenGL intensity image\newline and depth image into a sonar-line image., label=IV_listing_pseudo_code]
$\text{for all } (n_x^{\v{R}}, n_\varphi^{\v{R}})\text{ in }[0,N_x][0,N_\varphi] $
   $ \rlap{ \(r\) }\phantom{\dvec\gamma^{\v{R}}[n_x^{\v{R}}, \lfloor{}r\rfloor+1]} := (\dvec\beta^R[n_x^{\v{R}}, n_\varphi^{\v{R}}]] - p_{\dd{R}{I},r}^{\v{R}} + p_{\dd{I}{E},r}^{\v{R}}) / \Delta_r^{\v{R}} $
   $\dvec\gamma^{\v{R}}[n_x^{\v{R}}, \rlap{\(\lfloor{}r\rfloor\)}\phantom{\lfloor{}r\rfloor+1}]  := (1 - r - \left\lfloor r\right\rfloor)\dvec\alpha^{\v{R}}[n_x^{\v{R}}, n_\varphi^{\v{R}}]] $
   $\dvec\gamma^{\v{R}}[n_x^{\v{R}}, \lfloor{}r\rfloor+1]                        := (\phantom{1 - r}\llap{\(r\)} - \left\lfloor r\right\rfloor)\dvec\alpha^{\v{R}}[n_x^{\v{R}}, n_\varphi^{\v{R}}]] $
\end{lstlisting}
%\end{float}
%\end{table}
%\end{minipage}
% \end{minipage}
%
where $:=$ and $\lfloor\cdot\rfloor$ are assignment and floor operators, $\{N_x, N_\varphi, N_r\}$ are user-defined number of samples for along-track, elevation and range, and $\Delta_r^{\!\v{R}}$ is the size of a resolution cell,
%
\begin{align}
\Delta_r^{\!\v{R}} = 2\,p_{\dd{I}{E},r}^{\v{R}} / N_\varphi.
\end{align}
%
The last two lines mitigate round-off artifacts by distributing the intensity value linearly between the two neighboring cells of a range value.  

We perform this computation in OpenCL which provides general purpose programming and seamless interoperation with OpenGL. The improved flexibility allows the GPU to tackle tasks like template matching, classification and advanced effects without involving the central processing unit (CPU).
...
Templates are finally matched to the SAS image by isolating the highlight and shadow regions in each, cross-correlating them, and assigning the mean correlation coefficient to the match score~\cite{Midelfart2011}. 

Improved feature filters~\cite{Fei2015} and object models~\cite{Pailhas2010} would likely improve performance here.

%
%\cite{Fawcett2005}. Computer-aided classification for a database of images of minelike objects. Sidescan. Image library. Kernel-based nearest-neighbour. TPR+TNR 90\% at 20\% FPR.
%
%\cite{Cetin2003}. Nonquadratic optimization. Improved performance (low data quality and quantity) of three classifiers: templates, likelihood based on Gaussian reflectivity models, locations of dominant scatterers. 
%
%%   Speckle and noise etc. makes optical similarity metrics such as sum of absolute difference and normalized cross-correlation fail.
%\cite{Myers2010} (+Fawcett). A Template Matching Procedure for Automatic Target Recognition in Synthetic Aperture Sonar Imagery
%   
%   Approaches that segment pixels into highlight, shadow and background to reduce the effect of noise:
%
%   - \cite{Reed2004}. Detection with Markov random field model (good in noise) using prior knowledge. Segmentation with statistical snake (assumes statistical separability). Classification with Dampster-Shafer, comparing features with synthesized ones from sidescan model. Hausdorff distance.
%   
%   - \cite{Quidu2000}. Mine classification from raw sonar data. Fourier descriptors, statistical models, genetic algorithms. Skips segmentation.
%   
%   - \cite{Fawcett2005}.
%
%Similarity metrics:
...
%
%When separated into shadow/highlight:
%Reed: Maximum distance between shadow regions
%Fawcett: Fraction of pixels in highlight and shadow regions matching corresponding pixels in the image
%
%Groen
...
Once the simulator runs it generates thousands of sonar templates per second. However, loading OpenGL, OpenCL and 3D models from files takes seconds per invocation. Therefore, the simulator runs as a preloaded server process or in bath mode with a list of parameters.
...
\section{Results \& Discussion}\label{IV_sec_results}
...
\subsection{Classification - selected objects}\label{IV_sec_results_selected_objects}

We contrast simulation and classification performance on three typical objects from the 2012 data: two roughly 3\,m long mines and a 5\,m long torpedo, each with unique immersion depths and aspect angles. SAS image segments and templates for these objects are shown in \Fig{IV_fig_images_sonar_simulation}, with hand-drawn red lines superimposed to reference the objects' contours. Segmentation performance and correlation scores are illustrated in \Fig{IV_fig_image_simulation}. Observe the following:
%
\begin{itemize}
...
\item \emph{Template accuracy} improves when using the adaptive technique; it excels at generating accurate highlight regions, but is less convincing in shadow regions. The less common the object, the less accurate its representation by the static database, the greater the advantage of the adaptive method.
\end{itemize}
...
\subsection{Classification - all objects}\label{IV_sec_results_all_objects}
...
%
\begin{itemize}
\item \emph{Below 20\% FPR} the adaptive techniques outperformed the static method in every run. This benefits typical mine counter measures (MCM) operations with thousands of detections, as only some of these can be visually inspected up close, transmitted via underwater modem, or both.
...
\end{itemize}
%
A classification model should be neither too simple (high bias or underfitted) nor too complex (high variance or overfitted), but strike a balance between the two that captures statistically significant details of the object but not the noise. Higher detail-to-noise ratios favor more complex models and occur at lower FPR, which discards poor quality objects. The added complexity of the adaptive techniques seems beneficial here, yielding greater advantage at higher detail levels without suffering at lower ones. The greatest edge is held by the adaptive technique that also adjusts burial depth. The results suggest significant potential to improve performance by raising adaptivity in this scenario.
...
\section{Conclusion}\label{IV_sec_conclusion}
...
Improved classification speed and accuracy paves the way for heterogeneous clusters of AUVs ~\cite{Midtgaard2019}.
...
\subsection{Decomposed notation}\label{IV_sec_euler_rodrigues_decomposed}
...
\end{align}
%
At $\theta=\frac{\pi}{2}$ we have $\varphi=-\psi$, and at $\theta=-\frac{\pi}{2}$ we have $\varphi=\psi$. This effect, in which one degree of freedom is lost, is known as Gimbal lock. Any three-parameter specification of an orientation in three-dimensional space has this problem~\cite{Stuelpnagel1964}. Axis-angle representations such as Euler-Rodrigues formula or quaternions avoids this issue by adding a fourth parameter and one constraint: one angle and three parameters for the rotation axis subject to unit length. 
...
depending on whether the free or decomposed form is wanted, and where $\vec{R}\colon \mathbb{V}^N \to \mathbb{V}^N$ and $\dvec{R}^M\colon \mathbb{R}^N \to \mathbb{R}^N$. Be aware that the rotation matrix can be identical---both notationally and numerically---in the active and passive case, for example, if $\dvec{R}^M$ was replaced by $\dvec{R}_{IM}$ in \eq{IV_eq_active_rotation}. Thus, the geometrical intent must be inferred from the position vector notation; either the point changes, or the representation does, but never both.


\section{Supplementary material}\label{IV_sec_source_code}

\ifPhdDoc{}The link below \else{}IEEExplore \fi{}provides the following supplementary material:
%
\begin{itemize}
\item a Python script to demonstrate the notation and methods in practice, using OpenGL and PyQt5 to render a simplified version of the scene in \Fig{IV_simulator_coordinate_system} and display the Cartesian and radial viewport, respectively;
\item computer models to test the script with; and
\item animations showing the script and fully-featured simulator in action.
\end{itemize}

\ifPhdDoc
\url{https://github.com/joinge/sas-simulator-notation}
\fi
...
%%& & &   \\
%%& \mat{R}_{IM}(\psi,\theta+\pi,\varphi)\;\mat{S}_{IM} & & \dvec{p}_{IM}^{I} \\
%%& & &  \\\hline
%% 0 &  0  &  0  &  1
%%\end{array}\right]
%%\end{align}
...
%	$\Phi_y$           & 0                                                                                  & 0                   & $\pi+\theta$        \\
%	$\Phi_z$           & 0                                                                                  & 0                   & $\varphi$              \\
%	$\mat{L}$          & $\mat{R}(\uvec{I}_{\,z}^S, \dvec{p}_{SI}^{S})$                                     & 0                   & $\mat{R}(\psi,\theta,\varphi)\mat{S}$              \\
%   $\mat{S}$
%	$\s_x$             & 1                                                                                  & 1                   & 1                   \\
%	$\s_y$             & 1                                                                                  & 1                   & 1                   \\
%	$\s_z$             & 1                                                                                  & 1                   & 1                   \\
