...
We perform this computation in OpenCL using a computing kernel (similar to OpenGL shaders). It allows general purpose programming on GPUs and interoperates with OpenGL effectively. This allows the GPU to perform full simulations without involving the central processing unit (CPU).
...
Once the simulator runs it produces thousands of sonar templates per second. However, loading OpenGL, OpenCL and 3D models from file consumes seconds per invocation. Therefore, the simulator is either run as preloaded server process or as batch run with a list of parameters. %  or ensure that it did not have to be shut down for every invocation. 
...
Classification performance on all objects is demonstrated with receiver operating characteristic (ROC) curves in \Fig{IV_fig_roc_curves}. Observe the following:
%
\begin{itemize}
\item \emph{Below 20\% false positive rate (FPR)} the adaptive techniques outperformed the static method in every run. This benefits e.g. mine counter measures (MCM) operations where only a few detections out of 3000 will be visually inspected up close.
...
One way to automatically classify objects in synthetic aperture sonar (SAS) is to compare them with relevant object templates. Since it is only feasible to precompute these for most common object configurations, classification accuracy deteriorates for less common  objects. We solve this by simulating templates on-the-fly using a graphics processing unit (GPU). 


% However, this only works well when  The challenge is to come up with a representative set of template images covering the relevant configurations of object and seabed.
...
%
\begin{itemize}
\item Simulating templates on-the-fly improved flexibility and classification accuracy compared to static template databases. Implementation on a GPU using OpenGL and OpenCL proved efficient, flexible, portable and surprisingly fast. Classification accuracy improved with higher image quality, lower FRP, or both.  
I with unrivaled speed  is an  way to do so. 
\item A good notation system promotes geometrical understanding, conceptual clarity and 
\end{itemize}

%
%The best way to classify objects 
%
%We conclude that the best way to classify SAS images is....



A final post-processing step is performed in OpenCL, which allows general-purpose GPU programming. It interoperates effectively with OpenGL and improves simulation flexibility. This is valuable as the simulator is still being actively developed and will likely see new features that can not be easily implemented in OpenGL alone.
