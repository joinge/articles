...
\caption{Simulator principle. A model of the seafloor and object is loaded into OpenGL. All points in the imaged seafloor region that are visible from the sonar are orthogonally projected onto a view plane. }\label{simulator_coordinate_system}%
\end{figure*}
...
We have tested the template matching techniqiues on experimental data from the HISAS1030 synthetic aperture sonar (SAS) attached to the HUGIN autonomous underwater vehicle (AUV), both developed by Kongsberg Maritime. Three objects from Jesusbukta (Jesus Bay) in Norway will be presented; two roughly 3\,m long cylinders imaged a couple days apart in Janyary 2009, and a roughly 5\,m long torpedo imaged in June 2012. 


The resulting classification performance of the template match ing techniques 

Data from Jesusbukta

Cylinder 1: 2.9m (short side towards sonar)
Cylinder 2: 3.05m (diagonal)
Torpedo: 5.06m (looks good)

\begin{figure*}[!tbp]\centering%
\graphicsAI[width=\linewidth]{gfx/fig_images_sonar_simulation.pdf}%
\label{fig_sonar_simulation}%
\caption{SAS images of 2 cylinders and a torpedo located in the Jesus Bay, Norway. These detections are just 3 objects out of 1000+ detected in this run.}
\end{figure*}

\begin{figure*}[!tbp]\centering%
\graphicsAI[width=\linewidth]{gfx/fig_rocs.pdf}%
\label{fig_sonar_simulation}%
\caption{Receiver operating characteristics curves.}
\end{figure*}
...
\graphicsAI[drawing,width=.48\linewidth]{gfx/overlay_cylinder_submerged.svg}\label{overlay_cylinder}%
}%
\hfill%
\subfloat[Adapted template. Template simulated with adapted parameters. Good fit in both highlight and shadow segments.]{%
\graphicsAI[width=.48\linewidth]{gfx/overlay_cylinder_submerged_adaptive.svg}%
}\caption[LoF entry]{\raggedright Simulation image overlaid the segmented SAS image. Segmentation overlay color description:
