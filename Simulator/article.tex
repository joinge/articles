...
\graphicsAI[width=\linewidth]{gfx/scene_coordinate_system.svg}%
...
%
\begin{align*}
\bmat{x\\y\\0\\1}_\text{image} = \P \cdot \V \cdot \M \cdot \bmat{x\\y\\z\\1}_\text{object},
\end{align*}
%
...
%
\begin{align}
\M = \bmat{x_t \\ y_t \\ z_t \\ w}_\text{\parbox{1cm}{\setlength\baselineskip{0.3cm}transformed\\model}} &= \boldsymbol{T} \cdot \R \cdot \boldsymbol{S} \bmat{x \\ y \\ z \\ w}_\text{model},
\end{align}
%
...
\graphicsAI[width=\linewidth]{gfx/fig_images_sonar_tagged.svg}\label{fig_image_sonar}%
\caption{SAS images of 2 cylinders and a torpedo located in the Jesus Bay, Norway. Most of the 1000+ detected objects this run are similar to these.}
\end{figure*}

\begin{figure*}[!tbp]\centering%
\graphicsAI[width=\linewidth]{gfx/fig_images_simulator.pdf}\label{fig_image_simulation}%
\caption{SAS images of 2 cylinders and a torpedo located in the Jesus Bay, Norway. Most of the 1000+ detected objects this run are similar to these.}
\end{figure*}

\begin{figure*}[!tbp]\centering%
\graphicsAI[width=\linewidth]{gfx/fig_rocs.pdf}\label{fig_roc_curves}%
\caption{Receiver operating characteristics (ROC) curves.}
\end{figure*}

\begin{figure*}[!tbp]\centering%
\graphicsAI[width=\linewidth]{gfx/fig_overlay.pdf}%
...
We present data from three different HUGIN runs from the Jesus Bay in Norway. The first two are taken a couple days apart in January 2009, the last in June 2012. There are roughly 3000 detections in each, most representing by cylinder shaped objects such as barrels, pipe mines and torpedoes. Therefore, the standard template library were created with SIGMAS+ using a basic  cylinder model with a set of geometrical properties commonly found in the scene\todo{Should be more precise. How many templates? How finely were the parameters sampled?}. For a fair comparison, FFISim was restricted to the same cylinder model as well.

In \Fig{fig_image_sonar} we display SAS images of three typical objects in the 2012 run. All are cylindric; the length two first being estimated to roughly 3\,m and the last 5\,m, likely representing cylinder mines and a torpedo, respectively. 
...
The classification performance is quantified with receiver operating characteristics (ROC) curves from all runs in \Fig{fig_roc_curves}. As expected, the 2009 runs proves a greater challenge than that from 2012, with 5 and 3 false positive detections at FPR=20\% in the January 6. and 8. run, respectively. This can partly be explained by incorrect estimation of orientation for a few objects in close proximity to fish trawler tracks, and partly by poor quality in highlight, shadow, or both. However, for FPR<20\% the adaptive techniques do much better than the static template library. For e.g. a mine counter measures (MCM) operation, this is the only region of interest because only a few objects out of the 3000 detections will be visually inspected up close.
...
% \graphicsAI[width=0.5\linewidth]{gfx/fig_images_sonar_simulation.pdf}%
...
% \graphicsAI[drawing,width=0.5\linewidth]{gfx/sim_cylinder_submerged_adaptive.svg}%
% \label{sim_cylinder}}%
...
% \graphicsAI[drawing,width=0.5\linewidth]{gfx/data_cylinder_submerged.svg}%
% \label{data_cylinder}}%
% \subfloat[Cylinder simulation with parameters estimated from SAS image: Length 2.6\;m, burial depth 0.263\;m, aspect angle 105$^\circ$.]{
% \graphicsAI[drawing,width=0.5\linewidth]{gfx/sim_cylinder_submerged_adaptive.svg}%
% \label{sim_cylinder}}%
