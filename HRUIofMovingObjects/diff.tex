...
%DIF 402a402
\newcommand{\degree}{\ensuremath{^\circ}} %DIF > 
%DIF -------
...
z[n] = \sum_{m = 0}^{M-1}\DIFdelbegin %DIFDELCMD < \vec{w}%%%
\DIFdel{_m^*}%DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{w_m^*x}\DIFaddend _m[n - \Delta_m[n]] = \vec{w}^H\vec{x}[n],
\end{align}
...
The Capon beamformer \DIFdelbegin \DIFdel{produce }\DIFdelend \DIFaddbegin \DIFadd{produces }\DIFaddend a weight vector, like the one in (\ref{eq:das}), based on the impinging signal and an optimization criterion. The problem is as follow \cite{Capon1969}:
\begin{align}
...
The required lateral sampling in an ultrasound image for a given center frequency, $f_c$, and bandwidth, $B$, is given by the aperture size, $D$, and the wavelength of the center frequency, $\lambda_c = c/f_c$, where $c$ is the speed of sound. The required Nyquist beam spacing is then\DIFdelbegin %DIFDELCMD < \cite{Hergum2007}%%%
\DIFdelend :
\begin{align}
\DIFdelbegin \DIFdel{\Delta }\DIFdelend \DIFaddbegin \DIFadd{\delta\theta }\DIFaddend < \DIFdelbegin \DIFdel{\frac{\lambda_c}{qpD}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\lambda_c}{2qD}}\DIFaddend , \label{eq:resolution}
\end{align}
where the \DIFdelbegin \DIFdel{oversampling factor $p$ is }\DIFdelend \DIFaddbegin \DIFadd{factor two is the effect of a two-way acquisition }\cite{Hergum2009}\DIFadd{, and where $q$ is an oversampling factor which can be }\DIFaddend applied to reduce folding of frequencies between $f_c$ and $f_c + B/2$. \DIFdelbegin \DIFdel{A $p$-factor of two has been used throughout this paper . The oversampling factor $q$ will be used to oversample }\DIFdelend \DIFaddbegin \DIFadd{In this paper we will use the $q$-factor to perform oversampling }\DIFaddend prior to Capon beamforming\DIFdelbegin \DIFdel{and is one if not stated otherwise}\DIFdelend .
...
The LSV-plots in this paper are displayed as a contour plot with contours on -1, -2, -3, -6, -12 and -24 dB. The maximum for each beam profile is also marked with a black dot, and the amplitude variation among these points is presented in a subplot next to the LSV-plot. The LSV-plots based on simulations \DIFdelbegin \DIFdel{has }\DIFdelend \DIFaddbegin \DIFadd{have }\DIFaddend been up-interpolated to 128 samples in each direction using cubic interpolation if the number of receive beams is less than 128.
...
Figure \ref{fig:das_shiftinvariant} presents such a LSV-plot for an imaging system using the DAS beamformer with a beam spacing equal to (\ref{eq:resolution}). Dotted vertical lines indicate position of transmit-receive beam pairs. The point scatterer response has been simulated using Field II \cite{Jensen1992, Jensen1996} \DIFdelbegin \DIFdel{with a probe setup equal to the M5S probe by GE Vingmed Ultrasound, Horten, Norway. The probe }\DIFdelend \DIFaddbegin \DIFadd{using a 96 element phased array with a 22 mm azimuth aperture, which }\DIFaddend was excited with a 2.5 MHz and 1.5 cycles long pulse. \DIFdelbegin \DIFdel{The transmit }\DIFdelend \DIFaddbegin \DIFadd{Transmit }\DIFaddend focus in azimuth and elevation were configured to be equal and the point scatterer was moved through this point. \DIFaddbegin \DIFadd{The transmit focus is where the ultrasound beam is at its narrowest, hence shift variance effects will appear at this location first. The beam density was calculated  based on (\ref{eq:resolution}) with a speed of sound equal to 1540. }\DIFaddend We see that the contours in Fig.\,\ref{fig:das_shiftinvariant} are approximately diagonal (low P-MAE) and that the amplitude variation among the maximum points is minimal (low A-MAE). We therefore conclude that the system is locally laterally shift-invariant. %In this, and the following LSV-plots based on simulations, the scan sequence always has a beam at zero degrees and the point scatterer is located in the center of this beam at one time instance.
...
A straight forward way of obtaining sufficient sampling in an active imaging system \DIFdelbegin \DIFdel{is by }\DIFdelend \DIFaddbegin \DIFadd{with an equal number of transmit (tx) and receive (rx) beams is to }\DIFaddend increasing the number of \DIFdelbegin \DIFdel{transmit beams (}\DIFdelend tx-beams \DIFdelbegin \DIFdel{) }\DIFdelend until the system is locally laterally shift-invariant. In Fig.\,\ref{fig:capon_oversampling_a} and Fig.\,\ref{fig:capon_oversampling_b} we have increased the tx-beam density by a $q$-factor of four and sixteen respectively. As expected the geometric distortion and scalloping loss decreases with increased lateral sampling. Note that Fig.\,\ref{fig:capon_oversampling_b} is zoomed compared to Fig.\,\ref{fig:capon} in order to better see the beam-to-beam variation.

From Fig.\,\ref{fig:capon_oversampling_b} we see that sixteen times oversampling reduces the scalloping loss significantly. Even though high frequency variations are reduced, the maximum is still larger than 1 dB. An even higher level of oversampling is therefore required in order to have a completely local lateral shift-invariant Capon beamformer when a moving point scatterer is simulated without noise and with the same parameters as in Fig\DIFaddbegin \DIFadd{.}\DIFaddend \,\ref{fig:capon}. %$1/100$ in diagonal loading. 
...
\begin{align}
\theta_{max} = \DIFdelbegin \DIFdel{\alpha p \Delta}\DIFdelend \DIFaddbegin \DIFadd{2\alpha \delta\theta}\DIFaddend ,
\end{align}
where $\alpha$ is the pulse length in $\lambda$'s, and \DIFdelbegin \DIFdel{$p$ and $\Delta$ }\DIFdelend \DIFaddbegin \DIFadd{$2$ and $\delta\theta$ }\DIFaddend is the oversampling factor and beam spacing \DIFdelbegin \DIFdel{respectively as defined in }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend (\ref{eq:resolution}). For our simulations \DIFdelbegin \DIFdel{$\theta_{max} = 3\Delta$}\DIFdelend \DIFaddbegin \DIFadd{$\theta_{max} = 3\delta\theta$}\DIFaddend .
...
\end{bmatrix}
\end{align}
where $x_i$ is the element position and $\theta$ is swept from \DIFdelbegin \DIFdel{$-\Delta/2$ to $\Delta/2$}\DIFdelend \DIFaddbegin \DIFadd{$-\delta\theta/2$ to $\delta\theta/2$}\DIFaddend . Note that the steering vector must be calculated for an $L$ element symmetric subarray, even though the full array has $M$ elements. This subarray should have the same element pitch, \DIFdelbegin \DIFdel{$x_i - x_{i-1}$}\DIFdelend \DIFaddbegin \DIFadd{$|x_i - x_{i-1}|$}\DIFaddend , as the full array. %DIF < The steering error introduced by this steering vector for a frequency band $B$ is less than $B/2 \times \Delta/2$.
%DIF > The steering error introduced by this steering vector for a frequency band $B$ is less than $B/2 \times \delta\theta/2$.
...
Figure \ref{fig:capon_oversampling_pr} shows the phase rotation method applied on the same scan sequence as in Fig\DIFaddbegin \DIFadd{.}\DIFaddend \,\ref{fig:capon} with four and sixteen times oversampling \DIFaddbegin \DIFadd{($q$-factor) on receive }\DIFaddend respectively. Note that the figure is zoomed compared with Fig\DIFaddbegin \DIFadd{.}\DIFaddend \,\ref{fig:capon}. As with oversampling on transmit there is some variation when the point moves in and out of focus. The maximum scalloping loss is therefore large. The extreme local variations are however reduced significantly. Comparing Fig\DIFaddbegin \DIFadd{.}\DIFaddend \,\ref{fig:capon_oversampling} with Fig\DIFaddbegin \DIFadd{.}\DIFaddend \,\ref{fig:capon_oversampling_pr}, the errors introduced by the phase rotation method are best seen in sidelobes.  
...
\begin{figure*}[!t]
	...
	\caption{Capon beamforming as described in Fig\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL \,\ref{fig:capon} with oversampling based on phase rotation. The figure is zoomed compared with Fig\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL \,\ref{fig:capon} to better see the beam-to-beam variation. a) Four times oversampling \DIFaddbeginFL \DIFaddFL{on receive ($q=4$)}\DIFaddendFL . b) Sixteen times oversampling \DIFaddbeginFL \DIFaddFL{on receive ($q=16$)}\DIFaddendFL .}
	\label{fig:capon_oversampling_pr}
\end{figure*}

\section{In-vitro phantom data}\label{sec:res}
In Fig.\,\ref{fig:LSV_invitro} we present the result of applying the Capon beamformer on \textit{in-vitro} data with and without oversampling by phase rotation. The data was acquired using a modified Vivid E9 high-end cardiac ultrasound scanner by GE Vingmed Ultrasound, Horten, Norway, equipped with a \DIFdelbegin \DIFdel{M5S }\DIFdelend \DIFaddbegin \DIFadd{M5S-D phased }\DIFaddend transducer operating in harmonic mode \DIFdelbegin \DIFdel{with a receive frequency of 3.7 }\DIFdelend \DIFaddbegin \DIFadd{at 3.34 }\DIFaddend MHz. The transmit beam density was set to match (\ref{eq:resolution}) based on this frequency\DIFaddbegin \DIFadd{, and 111 tx-beams where therefore distributed on a 65}\degree \DIFadd{sector. Transmit focus is located at 8 cm range}\DIFaddend . Note that in harmonic imaging the generated transmit beam at the harmonic frequency will be wider and have lower sidelobes than the beam resulting from transmitting at the harmonic frequency \cite{Fedewa2004}. The lateral sampling is therefore increased compared with our simulations. A tissue mimicking phantom containing wire targets and anechoic cysts was imaged, and the data was acquired while the probe was moved across the phantom manually. Fig.\,\DIFdelbegin \DIFdel{\ref{fig:image_das} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:phantom_das_image} }\DIFaddend shows the first frame out of 14 in the acquired loop \DIFaddbegin \DIFadd{processed with DAS beamforming}\DIFaddend . %Movies of the whole loop processed with DAS and Capon beamforming with and without ten times oversampling by phase rotation can be found in the supplementary material \multimedia{Videos}.

One point scatterer in Fig\DIFaddbegin \DIFadd{.}\DIFaddend \,\DIFdelbegin \DIFdel{\ref{fig:image_das} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:phantom_das_image} }\DIFaddend has been selected [white box], and then used to generate LSV-plots. \DIFdelbegin \DIFdel{Figure \ref{fig:phantom_das} }\DIFdelend \DIFaddbegin \DIFadd{The point scatterer is located close to the transmit focus. Figure \ref{fig:phantom_das_LSV} }\DIFaddend shows the resulting LSV-plot when using the DAS beamformer. The scalloping loss is less than 1 dB, and the system is by that locally laterally shift-invariant. As a consequence point scatterers have a consistent appearance when subjected to motion \multimedia{Media Movie 1}.  
...
%DIFDELCMD < \subfloat[]{\includegraphics[width=0.5\linewidth]{\img fig6a.eps}\label{fig:image_das}}
%DIFDELCMD < \subfloat[]{\includegraphics[width=0.5\linewidth]{\img fig6b.eps}\label{fig:phantom_das}}\\
%DIFDELCMD < \subfloat[]{\includegraphics[width=0.5\linewidth]{\img fig6c.eps}\label{fig:phantom_capon_LSV}}
%DIFDELCMD < \subfloat[]{\includegraphics[width=0.5\linewidth]{\img fig6d.eps}\label{fig:phantom_capon_LSV_10PR}}
...
\DIFaddendFL \caption{LSV-plots generated from \textit{in-vitro} phantom data where the probe has been manually dragged across the phantom. See the supplementary material for videos of the whole loop processed with the different beamformers\DIFdelbeginFL %DIFDELCMD < \multimedia{Videos}%%%
\DIFdelendFL . Capon beamforming has been performed with the same settings as in Fig.\,\ref{fig:capon1}. P-MAE is not shown due to a lack of probe movement control. a) First frame in the recorded loop \DIFaddbeginFL \DIFaddFL{processed with delay-and-sum beamforming  }\multimedia{Media Movie 1}\DIFaddendFL . A white box indicates the point scatterer used to construct the LSV-plots.  b) \DIFdelbeginFL \DIFdelFL{Delay-and-sum }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{LSV-plot for delay-and-sum }\DIFaddendFL beamforming. c) \DIFaddbeginFL \DIFaddFL{LSV-plot for }\DIFaddendFL Capon beamforming without oversampling. d) \DIFaddbeginFL \DIFaddFL{LSV-plot for }\DIFaddendFL Capon \DIFaddbeginFL \DIFaddFL{beamforming }\DIFaddendFL with \DIFdelbeginFL \DIFdelFL{ten }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{sixteen }\DIFaddendFL times oversampling using phase rotation.}
...
\caption{\DIFaddFL{First frame from Fig.}\,\DIFaddFL{\ref{fig:phantom_das_image} processed with Capon beamforming. a) Without oversampling }\multimedia{Media Movie 2}\DIFaddFL{. b) With sixteen times oversampling using phase rotation }\multimedia{Media Movie 3}\DIFaddFL{. Notice how the point scatterers appear brighter in the image with oversampling.}}
\label{fig:img_invitro}
\end{figure*}

\DIFaddend %\subsection{In-Vivo Images}
...
Oversampling by phase rotation can therefore introduce similar block artifacts as parallel receive beamforming (PRB). \DIFdelbegin \DIFdel{These artifacts should be possible to remove in the same way as for PRB by using the synthetic transmit beamforming (STB) technique. If we }\DIFdelend \DIFaddbegin \DIFadd{To remove these artifacts we could }\DIFaddend distribute our steering \DIFdelbegin \DIFdel{angles }\DIFdelend \DIFaddbegin \DIFadd{vectors }\DIFaddend in an overlapping scheme [like rx-beams are spread out for STB]\DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{. We then }\DIFaddend get a phase rotation version of STB where e.g. eight phase rotated beams are averaged into four. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{If the }\DIFaddend geometric distortion, or stripes, seen at the bottom of the sector in the video with sixteen times oversampling \multimedia{Media Movie 3} \DIFdelbegin \DIFdel{should be remove }\DIFdelend \DIFaddbegin \DIFadd{is removed }\DIFaddend by such technique \DIFaddbegin \DIFadd{is left for future work}\DIFaddend .

When oversampling is applied it becomes evident that the high resolution settings used in this paper might be \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{too }\DIFaddend extreme, both with respect to the level of required oversampling and the point scatterer's appearance. Traditionally in medical ultrasound imaging the point spread function is always wider laterally than in range. With the Capon beamformer this is now changed. A more symmetric point spread function while preserving speckle statistics is obtained with the parameters $L=M/4$ and $K=0$ \cite{Synnevag2007a}. The level of required oversampling and the computational complexity are also reduced. %Another option is to perform pulse inversion if a spiky symmetric point spread function is desirable.
...
%DIF < \begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{photos/jon_petter2.eps}}]{Jon Petter \AA{}sen}
%DIF < (S12) was born in Porsgrunn, Norway in 1986. He received the B.Sc. and M.Sc. degree in computer science from the University of Oslo, Norway, in 2008 and 2010. He is currently pursuing his Ph.D. degree in medical ultrasound technology at the Norwegian University of Science and Technology (NTNU) Medical Imaging Lab (MI-Lab), Trondheim, Norway. 
\DIFaddbegin \begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{photos/jon_petter2.eps}}]{\DIFadd{Jon Petter \AA{}sen}}
\DIFadd{(S12) was born in Porsgrunn, Norway in 1986. He received the B.Sc. and M.Sc. degree in computer science from the University of Oslo, Norway, in 2008 and 2010. He is currently pursuing his Ph.D. degree in medical ultrasound technology at the Norwegian University of Science and Technology (NTNU) Medical Imaging Lab (MI-Lab), Trondheim, Norway. 
}\DIFaddend 

%DIF < He has industry experience from GE Vingmed Ultrasound, Horten, Norway, were he completed three summer internships from 2008 to 2010. His research interests include image and signal processing, adaptive ultrasound processing techniques and acceleration of ultrasound algorithms using Graphics Processing Units (GPUs). 
%DIF < \end{IEEEbiography}
\DIFaddbegin \DIFadd{He has industry experience from GE Vingmed Ultrasound, Horten, Norway, were he completed three summer internships from 2008 to 2010. His research interests include image and signal processing, adaptive ultrasound processing techniques and acceleration of ultrasound algorithms using Graphics Processing Units (GPUs). 
}\end{IEEEbiography}
\DIFaddend 
