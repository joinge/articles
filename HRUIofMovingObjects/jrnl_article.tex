...
\IEEEPARstart{W}{ith} the introduction of a fast implementation of the Capon adaptive beamformer, a stream of images can now be processed at interactive frame rates \cite{Asen}. A natural question then is how a moving object appear after Capon beamforming has been applied. Earlier work on Capon beamforming for medical ultrasound imaging has investigated the method's capabilities in single frames only, and temporal behaviour and effects has therefore been neglected. 

An important property of an ultrasound imaging system is its local shift invariance. Local shift invariance means that an object’s appearance should be preserved when the object, or the ultrasound probe, is moved slightly. Local shift invariance is what makes techniques like doppler and flow imaging and speckle tracking possible \comment{(need citation)} . 

Capon beamforming has the ability to provide increased lateral resolution and contrast in an ultrasound image \cite{Synnevag2009, Chen2011}. However, it is also sensitive to errors like perturbations of the array element, correlated signals and errors in the assumed steering vector.

\comment{Add intro to other work.}

In this paper we analyze the shift invariance property of the Capon beamformer by studying simulated and in vivo images of moving objects. We show that when imaging moving point scatterers using the Capon beamformer with the same beam spacing as used for delay-and-sum, artifacts arise in the image, manifested as oscillating point scatteres. \comment{(TODO: Process csound data to see if we need over sampling on in vivo data, How often do we have bright point scatterers in vivo? Might improve contrast further)}.
...
\end{align}
where $\mat{\hat{R}}$ is the sample covariance matrix. Hence, the resulting weight vector will minimizes the output power while maintaining unit gain along the steering direction $\vec{a}$. %The covariance matrix is usually unknown and has to be estimated from the element data. The covariance matrix in (\ref{eq:capon_optimization_criteria}) is therefore substituted with the sample covariance matrix.
...
\end{align}
where $K = M-L+1$ typical is proportional to the pulse length, $N_K = 2K + 1$, $N_L = M-L+1$, and $\vec{x}_l$ is the $l\text{th}$ subarray $[x_l[n], \dotso x_{l+L}[n]]$ of length $L$. Finally $\mat{\hat{R}}$ is loaded with a diagonal factor for numerical stability and increased robustness, 
\begin{align}
...
Note that one matrix has to be constructed and inverted for each data vector $\vec{x}$ received by the system. This is indeed computational demanding, and has for long limited the methods accessibility. Following the innovation in GPU computing, this has now changed.
...
The lateral sampling of an ultrasound image is given by the aperture size on transmit ($D_{tx}$) and receive ($D_{rx}$) combined and the center frequency ($f_c = c/\lambda$), where $c$ is the speed of sound in the media and $\lambda$ is the wavelength. The required beam spacing is then:
\begin{align}
\Delta < \frac{\lambda}{D_{tx} + D_{rx}}.
\end{align}
If this requirement is not met, aliasing effects will appear in the image and the system will not be lateral shift invariant.
