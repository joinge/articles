...
In previous literature on Capon beamforming for medical ultrasound imaging in single frames this effect has been passed over in silence by either oversampling on transmit, or carefully position point scatterers on transmit-receive beam pairs, or ignorance. In this paper we show that when imaging a moving point scatterer with ultrasound using the Capon beamformer with the same beam spacing used for DAS the same scallop loss is observed, manifested as oscillating point scatteres. The question is then, what can we do in order to improve on the situation.
...
In order to visualize the local shift-invariance of a given imaging system, we will use lateral shift invariance plots, introduced by Hergum \textit{et al} \cite{Hergum2007}.
