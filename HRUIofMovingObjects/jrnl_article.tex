...
\begin{align}
\theta_{max} = \alpha q \delta\theta,
\end{align}
where $\alpha$ is the pulse length in $\lambda$'s, and $q$ and $\delta\theta$ is the oversampling factor and beam spacing respectively as defined in (\ref{eq:resolution}). For our simulations $\theta_{max} = 3\delta\theta$.
