...
\IEEEPARstart{I}{ntroduction} to Capon adaptive beamforming and temporal artifacts.

With the introduction of a fast implementation of the Capon adaptive beamformer, a stream of images can now be processed at interactive frame rates. A natural question then is how a moving object behaves throughout the stream of images. An important property of an ultrasound imaging system is its local shift invariance. An object’s appearance should be preserved when moved slightly, or if the ultrasound probe moves. The Capon beamformer has previously only been investigated intra-frame; it is therefore unknown how it works when applied in a real-time ultrasound setting.

In this paper we analyze the shift invariance property of the Capon beamformer by studying simulated and in vivo images of moving objects. We show that when imaging moving point scatterers using the Capon beamformer with the same beam spacing as used for delay-and-sum, artifacts arise in the image, manifested as oscillating point scatteres. (TODO: Process csound data to see if we need over sampling on in vivo data, How often do we have bright point scatterers in vivo?).
...
The Capon beamformer produce a weight vector, like the one in (\ref{eq:das}), based on the impinging signal and an optimization criteria. The weights are found by solving the following minimization problem:
\begin{align}
...
\begin{align}
\mat{\breve{R}} = \frac{1}{N_LN_K}\sum_{n'=n-K}^{n+K} \sum_{l=0}^{N_L-1} \vec{x}_l[n']\vec{x}_l[n']^H,
\end{align}
where $K = M-L+1$, $K\sim \tau/T_s$ ($\tau$ is the imaging pulse length and $T_s$ is the sampling period.) and $\vec{x}_l$ is the $l\text{th}$ subarray $[x_l[n], \dotso x_{l+L}[n]]$ of length $L$. Finally $\mat{\hat{R}}$ is loaded with diagonal factor for numerical stability and increased robustness, 
\begin{align}
\epsilon &= d*\text{trace}\{\mat{\breve{R}}\}\\
...
Note that one matrix has to be constructed and inverted for each raw data pixel in the image. This is indeed computational demanding, and has for long limited the methods accessibility. Following the innovation in GPU computing, this has now started to change.
