...
To investigate this aspect we derived exact complexity formulas for each step in the MVDR process. Then these were evaluated for the entire range of possible subarray sizes from $L=1\dots M$, with temporal averaging assessed with values $K=\{0,1,2\}$ and the number of channels set to $M=\{8,16,24,32\}$. The results are shown in Fig. \ref{mvdr_complexity}. Note how the computation of $\eR$ completely dominates at smaller subarray sizes, and that solving $\eRi\1$ only plays a notable role when the element number and subarray sizes are higher. There is also a very notable penalty in performing temporal averaging, but this can be a
