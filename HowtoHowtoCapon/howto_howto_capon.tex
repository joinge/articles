...
In attempts to improve image quality, data driven methods has been introduced in various imaging fields over the years. One such method, the Minimum Variance Distortionless Response (MVDR) beamformer, has been reported to produce images with improved contrast and resolution compared to conventional non-adaptive methods, e.g. in radar~\cite{Benitz1997}, ultrasound~\cite{Synnevag2007}, and passive sonar. Recently has its feasibility in active sonar also been demonstrated~\cite{Blomberg2012a,Blomberg2011,Dursun2009,Lo2004}.
...
Previous work have shown that the robustness issue can be solved by means such as temporal and spatial averaging~\cite{Kailath1985}, and regularisation. To reduce computational complexity, some studies have demonstrated the feasibility of enforcing a Toeplitz-structure on the covariance matrix to simplify its inversion, or performing MVDR beamforming in beamspace (frequency domain)~\cite{Nilsen2009}. Some low complexity approximations to the MVDR beamformer is also available, such as the Low Complexity Adaptive (LCA) beamformer, 

In this work, we show that the computation time can be reduced by 1-2 orders of magnitude when implemented wisely on a Graphics Processing Unit (GPU). These chips have in recent years emerged as a flexible and highly potent parallel technology well suited for many DSP tasks. Driven by the gaming industry, these devices are engineered to maximize the number of floating point operations per second (Flop/s) while keeping cost and power consumption at a minimum. Compared to many-core CPUs, a GPU core offer modest computational capabilities, but in return provide up to thousands of them\todo{misplaced?}. 
...
Comparable work may be found, e.g. in ultrasound, where Chen \emph{et al.} has demonstrated a GPU implementation of the MVDR that operates on real data~\cite{Chen2011,Chen2011a}. While promoting impressing performance, such a constraint does not allow the creation of asymmetric and shifted responses, and hence prevents the MVDR's potential to be fully reached. The complexity issue has been targeted in various ways. 
...
His industry experience includes the European Organization for Nuclear Research (CERN), Geneva, Switzerland (2007-2008), and the Norwegian Defence Research Establishment, Kjeller, Norway (2009). He has lectured in digital signal processing at the Gj\o{}vik College University (2009), and at the University of Oslo (2010-2012). His research interests include adaptive beamforming, digital image reconstruction, high performance computing, intelligent detector design and embedded Linux.
\end{IEEEbiography}
