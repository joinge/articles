...
Our implementation is tested on data from a 100\;kHz active sonar system with 30\;kHz bandwidth. Its range resolution is around 2.5\;cm and hence we can not rely on sufficient sample support from the temporal domain. here is a very little temporal sample support. we have adapted MVDR to a near single snapshot scenario as a single snapshot With a range resolution of around there is very little sample support to which provides very little   basebanded wideband data. In related work, we investigated using the same MVDR GPU implementation in cardiac ultrasound imaging~\cite{Asen2012,Asen2013}, and in active sonar imaging we have compared the performance of this implementation to comparable adaptive methods~\cite{Buskenes2013a}.
