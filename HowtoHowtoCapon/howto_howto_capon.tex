...
\item Processes $\eR$ along its diagonals.
\end{itemize}

\begin{itemize}
\item Pixels processed one by one.
\item Single thread per pixel.
\item Creates $\eR$ in a two-step procedure 
\end{itemize}

\begin{itemize}
\item Minimized arithmetic operations in the computation of the covariance matrix $\eR$.
\item 
\end{itemize}

% \rowcolor{tabBlue} & \multicolumn{1}{>{\columncolor{tabBlue}}c}{\bf B$_\text{arith}$} & \multicolumn{1}{>{\columncolor{tabBlue}}c}{\bf B$_\text{mem}$} & \multicolumn{2}{>{\columncolor{tabBlue}}c}{\bf
