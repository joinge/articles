...
To investigate this aspect we derived exact complexity formulas for each step in the MVDR process\todo{except patial pivoting, diagonal loading}. Then these were evaluated for the entire range of possible subarray sizes from $L=1\dots M$, with temporal averaging assessed with values $K=\{0,1,2\}$ and the number of channels set to $M=\{8,16,24,32\}$. The results are shown in Fig. \ref{mvdr_complexity}.
...
\caption{Per-pixel computational complexity of the steps in MVDR beamforming (prior to any optimizations). To avoid signal cancellation in an active sonar system we usually set $L<\frac{M}{2}$, in which region the computation of $\eR$ dominates in terms of arithmetic  complexity.}\label{mvdr_complexity}
\end{figure}
...
An important feature of MVDR beamforming, and with beamforming in general, is that each pixel can be computed independently. Considering that sonar images often size up in the order of Mpixels, this makes a compelling case for investigating technologies that support massive data parallelism. Perhaps the prominent technology in this application domain is the Graphics Processing Unit (GPU). These are chips 

A GPU fundamentally differs from 
