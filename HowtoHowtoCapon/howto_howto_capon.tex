...
We show that for active sonar systems up to 32 channels, the computation time can be significantly reduced by performing arithmetic optimizations, and by implementing the MVDR on a Graphics Processing Unit (GPU). On a quad-core Intel Xeon system with a high-end Nvidia GPU, we found that the GPU rendered more than 1\;Mpixels/s. This represents a speedup of more than two orders of magnitude over our intial implementation in C. We envision this performance used for real-time processing of sonar data, and for making the MVDR a viable alternative to conventional methods in practical systems. 
...
In attempts to improve image quality, data driven methods has been introduced in various imaging fields over the years. One such method is the Minimum Variance Distortionless Response (MVDR) beamformer. Its use has led to reports of improved image contrast and resolution compared to conventional non-adaptive methods, e.g. in radar~\cite{Benitz1997}, ultrasound~\cite{Synnevag2007}, and active sonar~\cite{Blomberg2012a,Blomberg2011,Dursun2009,Lo2004}.
...
It has been shown that the robustness issue can be solved by means such as temporal and spatial averaging, and regularisation, in e.g. \cite{VanTrees2002,Kailath1985}. To reduce computational complexity, one possibility is to assume data stationarity which allows the formation of a Toeplitz-structured covariance matrix that are simpler to invert~\cite{Asl2012a,Jakobsson2000}. Another possibility is to perform MVDR beamforming in beamspace (the frequency domain)~\cite{VanTrees2002,Nilsen2009}.
...
In this work, we show that such approximations may not be necessary since the computation time can be reduced by 1-2 orders of magnitude when implemented wisely on a Graphics Processing Unit (GPU). This can in part be attributed to the GPU itself, which is designed up of a large number of computing cores that combined to achieve a maximum number of floating point operations per second (Flop/s), while keeping cost and power consumption at a minimum. This is perfect for image 
