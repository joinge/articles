...
\end{figure}
Consider a backscattered wavefield being recorded by an $M$ element uniform linear array, and assume that the signature of the transmitted signal has been removed by a matched filter (Fig. \ref{beamforming}). Further assume that the array channels are digitally delayed to create focus at a pixel at azimuth angle $\theta$ and range sample $n$, and let the delayed data from the $m$'th channel be $x_m[\theta,n]$. To simplify notation, the dependence on $\theta$ will be made implicit from now on.

The beamformer output $z[n]$ is defined as the weighted sum of all the delayed data samples:
\begin{align}
