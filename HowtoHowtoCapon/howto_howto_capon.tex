...
We target the complexity issue by investigating the feasibility of accelerating the MVDR method using Graphics Processing Units (GPUs). These chips have in recent years emerged as a flexible and highly potent parallel technology well suited for many DSP tasks. Driven by the gaming industry, these devices are engineered to maximize FLOPS while keeping cost and power consumption at a minimum. Compared to many-core CPUs, a GPU core offer modest computational capabilities, but in return there are up to thousands of them. 
...
To investigate this aspect we derived exact complexity formulas for each step in the MVDR process\todo{except patial pivoting, diagonal loading}. Then these were evaluated for the entire range of possible subarray sizes from $L\in[1,M]$, with temporal averaging assessed with values $K\in\{0,1,2\}$ and the number of channels set to $M\in\{8,16,24,32\}$. The results are shown in Fig. \ref{mvdr_complexity}.

Note how the computation of $\eR$ completely dominates at smaller subarray sizes, and that solving $\eRi\1$ only plays a notable role when the element number and subarray sizes are higher. Also notice how temporal averaging comes with a high computational penalty. This can partly be attributed to the fact that building $\eR$ is is heavy on complex multiplications, and a lot of these are repeated unneccessary.
...
Due to the simple reason of availability we have targeted the nVidia GeForce GTX 480 in our study. This is by now a three generation old high-end Compute Unified Device Architecture (CUDA) enabled device from nVidia, which we instructed to carry out the general purpose tasks in the MVDR beamformer using nVidia's ``C for CUDA'' framework. We believe that a GPU from AMD would have been equally attractive, and that GPUs from either vendors might as well be targeted through the open and portable framework OpenCL from the Khronos group, but comparing these alternatives are beyond the scope of this study. Instead we aim to investigate how to efficiently map the MVDR beamformer to the GeForce GTX 480.

Using nVidias own terminology, the GeForce GTX 480 is comprised of 15 ``streaming'' multiprocessors (SMs), each having 32 CUDA cores that execute a common program called a kernel. That make for a total of 480 CUDA cores which we need to keep busy if we want to harness the full potential. This is achieved by balancing the load evenly on all cores, and by trying to avoid that some cores are forced to idle due to a pending data transfer or thread synchronization. The best way to do this is completely different for each step in the MVDR process, so we will launch a different kernel and configuration for each of them. 
...
As observed in Fig. \ref{mvdr_complexity} a direct implementation of (\ref{spatialR}) and (\ref{finalR}) is very compuationally complex. Furtunately, we can dramatically improve matters by performing arithmetic optimizations. As illustrated illustrated in Fig. \ref{mvdr_build_R}, we can exploit the fact that $\eR$ is Hermitian positive semi-definite to compute only one half of it, we can avoid unneccessary multiplications, and the spatial sum can be implemented in a sliding window fashion. The order of summations will also play a role here.
...
\caption{Step 1: Building $\eR$. This is an example of how we would build $\eR$ in a case with $M=5$ sensors, with subarray size $L=3$ and temporal averaging set to $K=1$. $\eR_{l}$ is the sample covariance matrix for the $l$'th subarray, and $\eR_s$ is the average of all these.}\label{mvdr_build_R}
\end{figure}
...
Before proceeding we should make a quick note on the GPU memory. Note that to obtain the best possible performance, we need to make sure that CUDA threads find the data they need in cache or registers most of the time. These two memory types have the sufficient bandwidth to promote maximum utilisatin of all CUDA cores. Unfortunately, they are a sparse resource. The GTX 480 CUDA architecture is of compute capability 2.0, which translates to a maximum of 48kB L1 cache and 128kB of registers per SM~\cite{Nvidia2012}, the equivalent of 12k and 32k complex numbers stored with single precision, respectively. This memory is shared by all active threads currently residing on that SM. To completely hide latency in even worst case scenarios this number should be no less than 768 (on compute capability 2.0)~\cite{Nvidia2012a}. Divide the SM memory on these, and each thread can store no more than 8\todo{$\frac{2^{15}+2^{14}}{8*768}$} complex numbers in shared memory\todo{introduce memory structure -shared for data shared between threads}.

This makes it apparent that computing one pixel per thread is not a feasible solution. More pixels need to cooperate in building $\eR$. An idea of how this can  We need to decompose the steps illustrated in Fig. \ref{mvdr_build_R} into independent tasks that can be computed by 
