...
In previous work it has been shown that the robustness issue can be solved by means such as temporal and spatial averaging, and regularisation, in e.g. \cite{VanTrees2002,Kailath1985}. To reduce computational complexity, one possibility is to assume data stationarity which allows the formation of a Toeplitz-structured covariance matrix that is simpler to invert~\cite{Asl2012a,Jakobsson2000}. Another possibility is to perform MVDR beamforming in beamspace (the frequency domain)~\cite{VanTrees2002,Nilsen2009}.
...
In this work, we show that such approximations may not be necessary since the computation time can be reduced by 1-2 orders of magnitude by implementing MVDR on a Graphics Processing Unit (GPU). We will show how we achieved this by altering and carefully mapping the algortithm to effectively make use of the vast parallel processing power present in such devices. The final implementation is then measured in terms of absolute performance, the results are compared to the theroetical peak, and some resoahow close to the peak performance it is, and the reasons for performance relative to the teoretical peak, and design optim We will explai will detail the challenges met, show how to make efficient use of the GPUs resources this can be done in the case of the MVDR method, provide details on challenges and solutions, was adapted  design is very well suited for performing parallel reconstruction of images, but in the case of the MVDR
...
Image processing favours such designs, because each pixel is often pixels can be processed independently, as well as many of the tasks on a subpixel level.
...
Similar work may be found in e.g. medical ultrasound imaging, where Chen \emph{et al.} has demonstrated a GPU implementation of the MVDR that operates on real data~\cite{Chen2011,Chen2011a}. They report a significant reduction in computation time, but the real data constraint does not allow the creation of asymmetric and shifted responses, and hence prevents the MVDR's potential to be fully reached. We have also investigated using MVDR on a GPU in cardiac ultrasound imaging~\cite{Asen2012,Asen2013}, and in active sonar imaging we have compared the performance of this implementation to other adaptive methods that is based on the same optimisation criterion~\cite{Buskenes2013a}.

This article is outlined as follows: In section \ref{methods} we introduce the concept of adaptive beamforming and provide details on the MVDR method. Then we investigate the complexity issue, discuss means for reducing arithmetic complexity, and show how to properly map this algorithm to a GPU in section \ref{maptogpu}. Then we present results in \ref{benchmarks}, discuss them in \ref{discussion}, and finally conclude in \ref{conclusion}.
...
\section{Images and Benchmarks}\label{benchmarks}
