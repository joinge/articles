...
\author{Jo~Inge~Buskenes, %
        Jon~Petter~$\overset{\tiny_{\boldsymbol{\circ}}}{\text{A}}$sen, % The Norwegian letter Å was not recognized by latex
...
\begin{gather}
\eR_\text{s,t}[n] =  \frac{1}{N_K N_L} \sumb{n'=n-K}{n+K}\sumb{l=0}{M-L} \x_l[n]\x_l\H[n] \in\mathbb{C}^{L,L},\label{spatialR}
\end{gather}
...
As observed in Fig. \ref{mvdr_complexity} a direct implementation of (\ref{spatialR}) and (\ref{finalR}) is very computationally complex. Furtunately, we can dramatically improve matters by performing arithmetic optimizations. As illustrated illustrated in Fig. \ref{mvdr_build_R},
we can exploit the fact that $\eR$ is Hermitian positive semi-definite to compute only one half of it, we can avoid unnecessary multiplications, and the spatial sum can be implemented in a sliding window fashion (iterative summing). Moreover, the order of the temporal or spatial sum also play a role here.

Which solution is best\todo{introduce them}? We assessed this by comparing each solutions individual complexity\todo{per pixel} to the reference implementation in Fig. \ref{mvdr_complexity} and (\ref{spatialR}). The result is shown in Fig. \ref{mvdr_build_R}\todo{figure is cropped}. Note\todo{overused} how the optimizations truly makes a difference, especially at lower subarray sizes and when using temporal averaging. As expected, the recursive solution proved an overall winner \todo{don't just say it, let it be evident}. 
...
\caption{Arithmetic optimization of computing $\eR$: Relative reduction in arithmetic complexity (higher is better). The reference is at 1(TODO: show it!) } 
\label{implementation}
\end{figure}
...
Take as example the case in Fig \ref{mvdr_complexity} in case where $M=32$, $L=16$ and $K=1$. The figure shows a that 300k floating point operations need to be performed per pixel. Furthermore, the total size of global reads is $(M+L)*8$bytes$ = 384 $bytes per px, and the total number of writes is $L^2*8$bytes = 2048 bytes, leading to a total of 2432 bytes. This means less than 1 bytes need to be read from or written to global per every 1000 floating point instructions. 
