...
% \begin{tabular}[c]{l r r r@{}  l}\hline
\begin{tabular}[c]{l p{.29\linewidth} p{.29\linewidth}}\hline
\rowcolor{tabBlue} & \bf GPU & \bf CPU \\\hline
Pixel processing & In batches & One by one \\
Threads per pixel & Multiple & One \\
1. Build $\eR$  & & \\
\ \ Values computed & \mbox{$\bullet$ Computing everything} \mbox{$\bullet$ Minimized memory} & \mbox{$\bullet$ Only upper triangular} \mbox{$\bullet$ Minimized instructions} \\
2. Compute $\eRi\1$ & With a batch linear equation solver & With custom inline Cholesky solver \\
