...
Recently\todo{maybe not so recent anymore?} the Minimum Variance Distortionless Response (MVDR) adaptive beamformer's ability to improve image quality in active sonar imaging has been demonstrated. However, it is based on the estimation and inversion of a large number of spatial covariance matrices. The computational complexity involved and the required means for making the estimate suited for MVDR may discourage its use unneccessarily.
