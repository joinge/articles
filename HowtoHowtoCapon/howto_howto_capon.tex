...
Which of these steps are mainly responsible for the MVDR beamformers complexity? This may not be immediately apparent. From the litterature we may find that computing a simple covariance matrix has a computational complexity of O($M^2$), but in our case this will largely be affected by i.e. the values of $L$ and $K$. Step 2 is also largely affected by the size of $L$, since $\eR$ is a quadratic matrix of size $L$ and inverting will be of O($L^3$). But these are approximate figures, and does not take into account the fact that the underlying data is complex, not whether the algorithms are suited for implementation on paralell hardware.
...
To assess the this aspect, complexity curves were computed for the entire range of possible subarray sizes from $L=1\dots M$. Temporal averaging was set to $K=\{0,1,2\}$, which should cover the range of values used in most applications. Data were assumed to be complex with a rectangular representation, with complex addition corresponding to 2 real additions, and a complex multiplication corresponding to 4 multiplications and 2 additions. The result is shown Fig. \ref{mvdr_complexity} \todo{move to results}. 
...
Whether we are successful in keeping all cores busy depends on a few critial factors. First we need to make sure they actually have something to do, by decomposing the algorithm into  be exposed in the algorithm, and the computational load distributed evenly on all cores. 
