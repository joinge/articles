...
\end{figure}
Consider a backscattered wavefield being recorded by an $M$ element uniform linear arraa, and assume that the signature of the transmitted signal has been removed by a matched filter (Fig. \ref{beamforming}). Further assume that the channels are digitally delayed to create focus at azimuth angle $\theta$ and range $n$, and let the delayed data from the $m$'th channel be $x_m[\theta,n]$. To simplify notation, the dependence on $\theta$ will be made implicit from now on.
...
\end{align}
where $w_m$ is the weight factor assigned to channel $m$. If these weights static, this would be referred to as the conventional delay-and-dum (DAS) beamformer. Depending on the weighting pattern, lateral resolution to be traded for improved noise suppression (contrast), but one always ends up with a compromise between the two~\cite{Harris1978}.
