...
The Minimum Variance Distortionless Response (MVDR) adaptive beamformer can improve active sonar image quality compared to conventional methods\todo{argh!!}. Unfortunately, it is also very computationally complex because a spatial covariance matrix must be estimated and inverted for each image pixel. %Furthermore, to a few several parameters need to be tuned to obtain sufficient estimation accuracy.\todo{henger i lufta...}

We show that for active sonar systems up to 32 channels, the computation time can be significantly reduced by performing arithmetic optimizations, and by implementing the MVDR on a Graphics Processing Unit (GPU). On a quad-core Intel Xeon system with a high-end Nvidia GPU, our GPU implementation renders more than 1\;Mpixels/s. This represents a speedup of more than two orders of magnitude over our intial implementation in C. We envision this performance used for real-time processing of sonar data, and for making the MVDR a viable alternative to conventional methods in practical systems. 
...
In attempts to improve image quality, data driven methods has been introduced in various imaging fields over the years. One such method is the Minimum Variance Distortionless Response (MVDR) beamformer. When compared to conventional methods the MVDR can often offer improved image contrast and resolution, as shown in e.g. radar~\cite{Benitz1997}, ultrasound~\cite{Synnevag2007}, and active sonar~\cite{Blomberg2012a,Blomberg2011,Dursun2009,Lo2004}.
...
% -  
To ensure MVDR robustness the litterature suggests combining means such as temporal and spatial averaging, and regularisation~\cite{VanTrees2002,Kailath1985}. Several strategies for reducing computational complexity have also been suggested. Some studies assume data stationarity which allows the formation of a Toeplitz-structured covariance matrix that is simpler to invert~\cite{Asl2012a,Jakobsson2000}. Other studies perform MVDR beamforming in beamspace (the frequency domain) to allow MVDR to be used in high channel systems~\cite{VanTrees2002,Nilsen2009}.
...
\end{align}
where $\I$ is an identity matrix, $\tr\{\cdot\}$ represents the matrix trace operation, and $\tr\{\breve{\R}[n]\}$ is an estimate of the energy received by this pixel.

Note how subarray averaging led to a size reduction of $\eR$ from $\mathbb{C}^{M,M}$ to $\mathbb{C}^{L,L}$, and hence will produce an $L$-element weight set when substituted into (\ref{z}). This weight set can be applied to on all the subarrays, prior to computing the beamformer output as in (\ref{z}). Or, equivalently, we may apply the weight set to the sum of all the subarrays:
\begin{align}
...
To visualise these relations, and include the effects of using complex numbers, we derived exact complexity formulas for each step in the MVDR process\todo{except partial pivoting, diagonal loading}. Then these were evaluated for the entire range of possible subarray sizes from $L\in[1,M]$, with temporal averaging set to $K\in\{0,1,2\}$, and the number of channels set to $M\in\{8,16,32\}$. The results are shown in Fig. \ref{mvdr_complexity}.
...
\cite{Owens2007} \cite{Lee2010}\todo{Find a good spot for these}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{gfx/plot_holmengraa_L16_Navg1.eps}
\caption{HISAS sidescan sonar (SSS) image of the shipwreck Holmengraa. Imaging parameters was $M=32$, $L=16$, $K=1$ and $d=1\%$. Prior to display the image was linearly upinterpolated by a factor 2 in azimuth. Note how the MVDR image improves edge definition and reduces noise in shadow regions.}\label{holmengraa}
\end{figure}
...
\caption{\protect MVDR benchmarks from Boston HPC centre. The exact same scenario and code as in Fig. \ref{benchmarks} was used here.}\label{benchmarks_boston}
\end{figure}
...
The computational performance of our implementation was assessed on a test system with a quad-core Intel Xeon E5620, 64\;GB of RAM and an nVidia Quadro 6000 card. In in \Fig{benchmarks}   is assessed thorugh various benchmarks. The , our GPU implementation of MVDR was able to process just over 1\;Mpx/s. A single-thread C implementation that was added as reference was outperformed by around 2 orders of magnitude. However, note that while the GPU design was tuned to make efficient use of L1 cache, similar optimisation work was not carried out on the CPU design. Also note that the computation times are reported without including the data transfer time from CPU to GPU, which account for 2-20\% of the total processing time. This is because we the required data amount to transfer will be reduced by a factor $M$ if we compute channel delays on the GPU instead of the CPU, which assume will be the case in a full beamforming system. Then the transfer time will be way less than the computation time, and can be easily hidden using asynchronous data transfer.
...
As Fig. \ref{benchmarks} demonstrates, performing arithmetic reductions and designing for using GPUs can significantly improve processing speed. If we look at the total runtimes for a subarray size of $L=16$ and temporal averaging at $K=2$, the GPU implementation is able to process the test image in roughly 40\,ms\todo{?}. This translates to a processing throughput of 1.4\,Mpixels/s. For the same scenario the C implementation clocked in at roughly 3\,s and Matlab at 17\,s, which is 75 and 435 times slower than the GPU implementation, respectively.\todo{rephrase}
