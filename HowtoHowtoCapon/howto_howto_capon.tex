...
The first thing note is the difference in complexity, and how this complexity changes depending on the parameters chosen.  Estimating a covariance matrix has a complexity of O(M$^2$), solving a linear equation O($L^3$)\todo{explain why}, and computing the beamformer output is at O($M$). How can we speed this process up?


\subsection{GPU Computing Model}

Two main parallelisation strategies competes when a modern processor is made. One is to make each thread run as fast as possible, by running independent instructions in parallel. The other is to support the parallel execution of as many threads as possible. This is typically realized in two levels. On a top level a few fully functional processor cores can run threads that executes different pieces of code. These cores may in turn be able to execute the same instruction on multiple data (SIMD). While CPUs represent a balanced compromise, GPUs are all out on the SIMD design.
 
The nVidia GeForce GTX 680 is a good example of such a design. Using nVidias own terminology, it is comprised of 8 ``streaming'' multiprocessors (SMX) each having 192 CUDA\todo{introduce} cores. This is a SIMD design where all the CUDA cores within an SMX share the same instruction unit, and a common pool of register and cache memory.

. Hence, granted that all threads run the exact same function (called a kernel), and does not  The threads also have to share a pool of register and cache memory available on each SMX. 
