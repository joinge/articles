...
To demonstrate the imaging capability of the MVDR beamformer, we have processed experimental datasets from the $M=32$ element Kongsberg Maritime HISAS1030 sonar mounted on a HUGIN AUV~\cite{Hansen2009}. HISAS1030 is a high resolution synthetic aperture sonar with an array length of 1.2\;m, operating frequency of 100\;kHz, and bandwidth of 30\;kHz. The elements are evenly spaced along the array at 2.5$\lambda$ separation with hardly any inter-apterture gap. To produce the image shown in \Fig{holmengraa} it was operated in sidescan mode. The studied object is the 1500 dwt oil tanker wreck Holmengraa. It is 68\;m long and 9\;m wide, and lies at a slanted seabed at 77\;m depth outside of Horten, Norway. The 1\;MP MVDR image were here processed with parameters $L=16$, $K=1$, and $d=1\%$.
...
GPU
\begin{itemize}
\item Pixels processed in batches.
\item Multiple threads per pixel.
\item Computation of $\eR$ performed directly from raw data to minimize memory footprint.
\item Inversion step performed by Nvidias batch linear equation solver.
\item Design makes efficient use of cache and data transfers are kept to a minimum.
\end{itemize}

CPU
\begin{itemize}
\item Pixels processed one by one.
...
As observed in \Fig{benchmarks} the combination of minimizing arithmetic operations and implementing the MVDR on a GPU lead to a significant improvement in processing speed. Compared to a MATLAB and single thread C implementation a speedup of 2-3 orders of magnitude was achieved, but we do not consider this comparison fair. In fact, the theoretical peak throughput of this GPU is roughly 20 times higher than that the CPU in question, meaning the potential of the CPU was far from reached in our initial implementations. This is mainly because the entire GPU design is focused on minimizing data transfers and maximize the use of fast memory cache, while no similar optimization work was carried out on the CPU implementations. Like the GPU, the CPU can only reach its potential if batches of pixels are computed at a time. Using library functions from e.g. the Math Kernel Library (MKL) per pixel turned out slower in our in house tests.  Unfortunately we are not aware of any batch solvers for the CPU, and the covariance code would have to be heavily altered to be suited to the CPU. 
