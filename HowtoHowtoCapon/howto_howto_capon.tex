...
\end{figure}
To summarize, the MVDR method is applied to each pixel independently, by
\begin{enumerate}
\item computing the sample covariance matrix $\eR$ (in \ref{finalR}),
\item computing $\eRi\1$ (in \ref{weights}), and
\item computing the beamformer output $z$ (in \ref{finaleR}).
\end{enumerate}
This is illustrated in Fig. \ref{implementation}.
\begin{figure}[!t]\centering
...
Which of these steps are mainly responsible for the MVDR beamformers complexity? This may not be immediately apparent. In the first step, computing a simple covariance matrix has a computational complexity of O($M^2$), but this is largely affected by the values of $L$ and $K$. Step 2 is also largely affected by the size of $L$, since $\eR$ is a quadratic matrix of size $L$ and inverting will be of O($L^3$). But these are approximate figures, and does not account for e.g. the fact that the underlying data is complex.

To assess the this aspect, complexity curves were computed for the entire range of possible subarray sizes from 1 to $M$. Temporal averaging was set to $K=\{0,1,2\}$, which should cover the range of values used in most applications. Data were assumed to be complex with a rectangular representation, with complex addition corresponding to 2 real additions, and a complex multiplication corresponding to 4 multiplications and 2 additions. The result is shown Fig. \ref{complexity} \todo{move to results}. 

We notice that for small subarray sizes $L$ building the covariance matrix dominates. Since this is the most common scenario, we will have a thorough look at how it can be optimized.
...
\end{figure}
where $r_{ij} = x_i[\p]x_j^*[\p]$, and $\eR_{l}$ is the covariance matrix for the $l$'th subarray. Since $r_{ji} = r^*_{ij}$, and the subarrays only span $L$ elements, we only compute an upper triangular band of R (marked \Green{green}). $\R_s$ can then be found as the mean of all $\eR_l$ (\ref{spatialR}):
...
\begin{align*}
\eR_\text{s,t}[n] &= \eR_\text{s}[n] + \eR_\text{s}[n] + \eR_\text{s}[n+1]
\end{align*}%


Hence, whenever $N_\text{avg}>1$ the sliding window approach will demand less instructions at the expense of a slightly higher memory consumption. The next thing to realise is that not all $r_{ij}[n]$ need to be computed. 
...
Keep delay step on CPU, as this is a common step in all beamforming
...
% Todays modern multicore systems accelerate algorithms using both instruction level and data level parallelism, or ILP and DLP for short. 
