...
In this study we show that for active sonar systems up to 32 channels, the computation time can be significantly reduced by performing arithmetic optimizations, and by implementing the MVDR on a Graphics Processing Unit (GPU). We point out important hardware limitations for these devices, and assess the design in terms of how efficiently it is able to use the GPU's resources. On a quad-core Intel Xeon system with a high-end Nvidia GPU, our GPU implementation renders more than a megapixel per second (1\;MP/s). Compared to our initial CPU implementation the optimizations described herein led to a speedup of more than 2 orders of magnitude, or an expected 5-10 times improvement had the CPU received similar optimization effort. This throughput enables real-time processing of sonar data, and makes the MVDR a viable alternative to conventional methods in practical systems.
...
\subsubsection{Minimizing Arithmetic Operations}\label{minimizing_arithmetics}
...
To study the effect of these optimizations we altered the complexity formulas to take them into account. Exploiting symmetry and performing iterative summing (step 1 and 3) is always desirable, but avoiding redundant multiplications (step 2) comes at the cost of increased memory consumption. When all optimizations are applied we will call it a \emph{minimized instructions} solution, and when step 2 is left out we call it a \emph{minimized memory} solution. The formulas may be found in appendix \ref{mvdr_formulas}, and Fig. \ref{mvdr_complexity_optimized} compares the two solutions. Here the complexity curves are relative to the reference implementation in Fig. \ref{mvdr_complexity}. Both solutions reduce the complexity considerably, and recomputing the multiplications does not make that much of a difference. We selected the memory minimized version since our algorithm is severely memory bound, as we will see next.
...
\item $\eR$ is computed directly from the raw data to \emph{minimize memory}.
\item Inversion step is performed by Nvidia's batch linear equation solver.
\item Design makes efficient use of cache and data transfers are kept to a minimum.
\end{itemize}

This will be compared to our CPU implementations, which have the following features:
\begin{itemize}
\item Pixels are processed one by one.
\item Single thread per pixel.
\item $\eR$ is computed using the \emph{minimized instructions} solution. Only upper half of $\eR$ is computed (see next point).
\item Inversion step is performed by a custom inline Cholesky based solver that only requires the upper triangular of $\eR$.
\item Designs have generally not been optimized for efficient use of memory.
\end{itemize}
These designs are discussed further in the next two sections.
...
\fi%
\caption{\protect MVDR benchmarks. A 1\;MP image from a $M=32$ channel array was processed for all $L$, and for $K\in\{0,1,2\}$.\newline \emph{Top:} The time the GPU spent on building $\eR$, solving $\eRi\1$, and computing $z$. Note the major speedup of building $\eR$ when compared to the complexity plot in Fig. \ref{mvdr_complexity}. The total GPU processing time can be derived by summing the time spent on the respective steps.\newline
...
As observed in \Fig{benchmarks} the combination of minimizing arithmetic operations and implementing the MVDR on a GPU lead to a significant improvement in processing speed. Compared to a single thread C implementation an order 2 speedup was achieved, but we are not considering this comparison to be fair. In fact, the theoretical peak throughput of this GPU is roughly 20 times higher than that the CPU in question, meaning the potential of the CPU was far from reached in our initial implementations. We can infer the reason from the curious observation that in \Fig{benchmarks} the two CPU implementations have run-times that are linear with $L$. This is because these designs are severely memory bound due to processing pixels one by one. Our GPU design, on the other hand, processes pixels in batches, which allows us to minimize data transfers and maximize the use of fast memory cache. In other aspects the designs are similar. Both CPU implementations compute $\eR$ efficiently, and they make use of either an optimized custom Cholesky based solver or one from the Intel Math Kernel Library (MKL). Unfortunately we had little time to write a custom solver and covariance builder for the CPU that processes pixels in batches, but we expect the speed difference of 5-10 times in favor of the GPU if we had compared with such a design.

Note from \Fig{benchmarks} how the building of $\eR$ now only takes up a fraction of the total processing time, and recall that it was the other way around when we presented the theoretical complexity curves in \Fig{mvdr_complexity}. Also, the benchmark curves for building $\eR$ now seem linear. The main reason for this is that the optimization step negated much of the extra complexity introduced by averaging, i.e. reduced the complexity from O($N_KN_LL^2$) to less than O($L^2$). Another reason for the run-time to appear more like O($L$) is likely that our design makes better use of the GPU's resources when there is more processing involved per pixel. The inversion step, on the other hand, gains less from being implemented on a GPU. This is because its nature is less data parallel. In particular, the back substitution step involved in its computation is mostly sequential. This difference be can observed in \Fig{mvdr_complexity}. In theory we could gain another factor two if we had a batch based Cholesky solver for the GPU, since then we would only need to build and invert one triangular of $\eR$.
...
Even with all our efforts we were only able to obtain a computation efficiency of 40\;MFlop/s on average in the desirable range of subarray sizes (\Fig{code_assess_flops}). This is mainly due to the inversion step, since we for the most part obtained more than 100\;MFlop/s when building $\eR$. While these numbers are likely slightly underestimated, they are still far away from the Quadro 6000's peak of 1\;Flop/s. Again, we believe this is due to the design being bound by memory bandwidth. This belief is further supported by the test results we got from running the code on the new K20 Kepler card (\Fig{benchmarks_boston}), where we saw a modest factor 1.5-2 speedup. Although the Kepler card peaks at 3.5-4\;Tflop/s, its shared memory bandwidth is only approximately twice that of the Quadro 6000 which matches the observed speedup. However, it is likely that the Kepler card would perform better had we optimized our design for it. For scientific computing the Kepler boards are usually considered more attractive, with the introduction of features such as kernels calling kernels, more registers, more shared memory, and better mechanisms for hiding or removing data transfers.
...
\begin{align}
\text{B}_\text{gmem} &= \frac{2\cdot 3\;\text{GHz}\cdot384\;\text{bit}}{8\;\text{bit/byte}}\nn
...
\begin{align}
\text{B}_\text{smem} &= \frac{\frac{1148}{2}\;\text{MHz/s}\cdot32\;\text{bit/bank}\cdot 32\;\text{banks/SM}\cdot 14\;\text{SMs}}{8\;\text{bit/byte}}\nn
