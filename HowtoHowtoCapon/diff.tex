...
%DIF -------
\newif\ifPeerReview\PeerReviewtrue              % Whether to create the PeerReview version or %DIF > 
%DIF -------
                                                % Journal version
%DIF 8a8-9
\newif\ifOnlineColor\OnlineColorfalse            % Compile online color version? %DIF > 
 %DIF > 
%DIF -------
...
%DIF 14a16-21
\ifOnlineColor %DIF > 
   \newcommand\figPostfix{_online}              % Colored images online %DIF > 
\else %DIF > 
   \newcommand\figPostfix{_bw}                  % Black and white in journal %DIF > 
\fi %DIF > 
 %DIF > 
%DIF -------
%%%%%%%%%%%%                                    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
%DIF 164a173-175
\else %DIF > 
   \definecolor{tabBlue}{HTML}{CCCCCC} %DIF > 
\fi %DIF > 
%DIF -------
...
%DIF < 
%DIF < \renewcommand\d[1]{\scriptstyle\kw{\;d\parm{#1}}\displaystyle}
%DIF < 
%DIF < % These commands are mutually exclusive. Remember to "renew" in v2.
...
% \usepackage{yfonts}
 %DIF > 
\graphicspath{submission/final/gfx/} %DIF > 
...
%DIF <   Furthermore, by implementing it on a modern \gls{GPU} we have gained a speedup of around 2 orders of magnitude compared to an optimized single-thread implementation in C.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  These devices have in recent years emerged as a flexible and highly potent parallel technology well suited for many DSP tasks, and are available off-the-shelf at reasonable prices\todo{skip this?}.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  These devices have in recent years emerged as a flexible and highly potent parallel technology well suited for many DSP tasks, and are available off-the-shelf at reasonable prices.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % Yay or nay? $\Rightarrow$ GPU ultrasound (\cite{So2011,Chen2011}). Complex data, all robustification techniques.



%DIF <  - Adaptive beamformers offers image quality edge, but are usually computationally complex.
%DIF <  - Who's tried before?
%DIF <  - We: Active sonar MVDR, GPU, high performance.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Methods}\label{methods}
...
\begin{figure}
[H]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\linewidth]{gfx/buske1.eps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\linewidth]{gfx/buske1\figPostfix.eps}
\DIFaddendFL \else
\begin{figure}
[!t]\centering
...
\begin{gather}
\vec w[n] = \DIFdelbegin \DIFdel{\frac{\Ri[n]\1}{\1\T\Ri[n]\1}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\Ri[n]\a}{\a\T\Ri[n]\a}}\DIFaddend ,\label{weights}
\end{gather}
where \DIFdelbegin \DIFdel{$\1$ is a row vector of ones that represents broadside steering , }\DIFdelend \DIFaddbegin \DIFadd{$\a$ is a steering vector }\DIFaddend and $\R=E\{\x[n]\x\H[n]\} \in\mathbb{C}^{M,M}$ is the spatial covariance matrix for the full array. Since we pre-steer our data to every pixel in the image we \DIFdelbegin \DIFdel{will always look to broadside, but }\DIFdelend \DIFaddbegin \DIFadd{simplify }\DIFaddend (\ref{weights}) \DIFdelbegin \DIFdel{can be easily generalized by substituting $\1$ with a suitable steering vector . Since }\DIFdelend \DIFaddbegin \DIFadd{by substituting $\a$ with a row vector $\1$ that represents broadside phase-steering. To estimate }\DIFaddend $\R$ \DIFdelbegin \DIFdel{is unknown, we estimate it by computing }\DIFdelend \DIFaddbegin \DIFadd{we compute }\DIFaddend a sample covariance matrix $\eR$. In this computation we perform some degree of:
\begin{itemize}
...
\DIFdelbegin \DIFdel{To form $\eR$ we will first compute }\DIFdelend \DIFaddbegin \DIFadd{We perform \emph{temporal} and \emph{spatial averaging} first and put the result in }\DIFaddend an intermediate sample covariance matrix $\breve{\R}$\DIFdelbegin \DIFdel{, which we form using temporal and spatial averaging}\DIFdelend . To do this we need to segment our array into subarrays. If we let $x_l[n]$ represent the data vector from subarray $l$,
\begin{gather}
...
\begin{figure}
[!t]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{gfx/buske2.eps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]{gfx/buske2\figPostfix.eps}
\DIFaddendFL \else
\begin{figure}
[!t]\centering
...
%DIF <  \includegraphics[width=\linewidth]{gfx/algorithm_structure.eps}
%DIF <  \caption{MVDR beamforming. First a spatial covariance matrix is estimated from the delayed data (\ref{spatialR}-\ref{finalR}), then the weights are computed (\ref{weights}) and finally applied to the delayed channel data (\ref{z}).}
%DIF <  \label{implementation}
%DIF <  \end{figure}
...
\end{enumerate}
%DIF <  This is illustrated in .
%DIF <  \begin{figure}[!t]\centering
%DIF <  \includegraphics[width=\linewidth]{gfx/buildR-complex_data-average.eps}
%DIF <  \caption{.}\label{complexity}
%DIF <  \end{figure}
...
\begin{figure}
[!t]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{gfx/buske3.eps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]{gfx/buske3\figPostfix.eps}
\DIFaddendFL \else
\begin{figure}
[!t]\centering
...
\end{figure}
To visualize these relations, and include the effects of using complex numbers, we set up complexity formulas that account for the total number of arithmetic operations for each step in the MVDR process (see appendix \ref{mvdr_formulas}). \DIFdelbegin \DIFdel{We only excluded diagonal loading performed in the computation of $\eR$, and }\DIFdelend \DIFaddbegin \DIFadd{The only excluded operation was }\DIFaddend partial pivoting used in the inversion step, \DIFdelbegin \DIFdel{since these operations }\DIFdelend \DIFaddbegin \DIFadd{but this should }\DIFaddend contribute marginally to the end result. The entire range of possible subarray sizes from $L\in[1,M]$ was finally evaluated, with temporal averaging set to $K\in\{0,1,2\}$, and the number of channels set to $M\in\{8,16,32\}$. The results are shown in \Fig{mvdr_complexity}.

\DIFdelbegin \DIFdel{We note }\DIFdelend \DIFaddbegin \DIFadd{Note }\DIFaddend how the computation of $\eR$ completely dominates at smaller subarray sizes, that solving $\eRi\1$ only plays a notable role for larger array and subarray sizes, and that the computation of $z$ has a negligible impact on computation time. Also notice how temporal averaging comes with a high computational penalty. This is because building $\eR$ is heavy on complex multiplications, which require 3 times as many arithmetic instructions as a complex addition, and a lot of these are repeated unnecessarily.




%DIF <  Not corrected for fused multiply accumulate
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Also, the need for partial pivoting has been ignored, as we ensure a numerically well conditioned $\eR$ by applying diagonal loading. Since this is the most common scenario, we will have a thorough look at how it can be optimized.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  and does not take into that does not  feasibility parallel decomposition  memory consumption.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Mapping the MVDR to a GPU}\label{maptogpu}


%DIF <  \begin{itemize}
%DIF <  \item Keep delay step on CPU, as this is a common step in all beamforming
%DIF <  \item Increasing matrix size reduces the number of threads per block, since the entire matrix must be in shared memory. 1 matrix 72x72 complex (8 bytes) max per block - 72 threads. same as 207 5x5 matrixes, 5*207=1036 threads. max threads in block 512.
%DIF <  \end{itemize}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  An important feature of MVDR beamforming, and with beamforming in general, is that each pixel can be computed independently and in an identical fashion. Considering that sonar images often end up having millions of pixels, this makes a compelling case for investigating technologies that support massive data level parallelism. The technology that does this best is perhaps Graphics Processing Units (GPUs). These chips contain up to thousands of small computation cores, which combined deliver superior peak floating point performance compared multi-core systems such as CPUs.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend An important feature of MVDR beamforming, as with beamforming in general, is that each pixel can be computed independently and in an identical fashion. Furthermore, a typical sonar image may contain millions of pixels. This represents a level of data parallelism that appears very well suited for a massively parallel architecture such as the GPU.

We decided to investigate the feasibility of running MVDR on a GPU by mapping it to a Nvidia GeForce Quadro 6000, which \DIFdelbegin \DIFdel{performance-wise is or par with or perhaps slightly inferior to the Geforce }\DIFdelend \DIFaddbegin \DIFadd{performs similarly to the consumer grade Geforce GTX }\DIFaddend 480. This is a high-end Compute Unified Device Architecture (CUDA) enabled GPU based on Nvidia's Fermi architecture, which as now been superseded by the Kepler architecture. The code was written in Nvidia's ``C for CUDA'' framework, which adds a few extra features to the C language to specify how to distribute work across a large number of threads. It would have been possible to use GPUs from AMD and the cross-platform OpenCL framework from the Khronos group instead, but CUDA has a batch linear equation solver~\cite{Nvidia} that no OpenCL library provides. As will be discussed later, this is a key component in our design.
...
\begin{figure}
[t]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{gfx/buske4.eps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]{gfx/buske4\figPostfix.eps}
\DIFaddendFL \else
\begin{figure}
[t]\centering
...
\end{figure}
%DIF <  \todo{Another shitty GPU transition}
%DIF <  Before proceeding we should make a quick note on the GPU memory. Note that to obtain the best possible performance, we need to make sure that CUDA threads find the data they need in cache or registers most of the time. Only these two memory types have a high enough bandwidth to promote sustained maximum utilisation of all CUDA cores. Unfortunately, they are a sparse resource. 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  This makes it apparent that computing one pixel per thread is not a feasible solution. \todo{...}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  When optimizing an algorithm for speed, it is if of importance to know whether an algorithm is bound by computations or memory. One way to get an approximate idea of this is to compute the ratio of peak arithmetic throughput to peak memory throughput of the in the target platform, and compare this ratio to the ratio of arithmetical operations to memory operations in the algorithm. 
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  follow collective access patterns to memory.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  At first glance, it might seem like a good idea to compute one pixel per CUDA core. Each pixel is computed using the same instructions, and without any exchange of data. However, this approach is unfeasible due to the limited cache and register memory available to each thread. One could resolve to using the slower GDDR5 memory, but this would largely mitigate the performance gain and defeat the purpose of using a GPU in the first place.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Bandwidth becomes crucial, as getting data in to the cores and out again must happen i a frantic pace. %the feeding all these cores with data and harvesting the result
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  To effectively harness the computing potential of a GPU we need to make sure all computing cores stays busy most of the time. 
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % Memory latency can be hidden in two ways. One is to keep a lot of threads queued up on each SM at all times, such that when some threads perform memory operations other threads can be scheduled to run instead. This is referred to as data level parallelism (DLP). Alternatively, or preferrably in addition, the designer may promote instruction level parallelism (ILP) by letting subsequent instructions in a thread be independent. While ILP can be very effective when done correctly, DLP is generally simpler to design for.
%DIF <  
%DIF <  In the CUDA framework, the threads on an SM is managed through a software abstraction call a compute block. The block is a 3 dimensional grid where each element correspond to a thread that is scheduled to be executed on the SM at some point.
...
\begin{figure}
[!t]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{gfx/buske5.eps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]{gfx/buske5\figPostfix.eps}
\DIFaddendFL \else
\begin{figure}
[!t]\centering
...
%DIF <  \begin{table}[b]\centering%\normalsize
%DIF <  \begin{tabular}[c]{l r@{:}  l}\hline
%DIF <  \rowcolor{tabBlue}\bf Memory type & \multicolumn{2}{>{\columncolor{tabBlue}}c}{\bf BW$_\text{memory}$/BW$_\text{arithmetic}$} \\\hline
%DIF <  Global memory &\hspace{30pt} 1 & 30 \\
%DIF <  Shared memory & 1 & 4 \\
%DIF <  Registers & $>$3 & 2~\cite{Vasilyy}
%DIF <  \end{tabular}
%DIF <  \caption{Nvidia Quadro 6000 bandwidth ratios.}\label{tabbandwidth}
%DIF <  \end{table}
\DIFdelbegin %DIFDELCMD < 
...
\begin{figure}
[!t]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{gfx/buske6.eps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]{gfx/buske6\figPostfix.eps}
\DIFaddendFL \else
\begin{figure}
[!t]\centering
\includegraphics[width=\linewidth]{gfx/mvdr_implementation.eps}
\fi%
\caption{MVDR implementated on a GPU. We do this in 3 steps, where each step process the full image before moving on to the next step. \emph{Step~1}: The sample covariance matrix $\eR$ is formed by threads running along its diagonals. This allows spatial averaging to be implemented in a computationally efficient manner and minimizes inter-thread communication. \emph{Step~2}: $\eRi\1$ is computed using the heavily optimized batched linear equation solver from Nvidia~\cite{Nvidia}. \emph{Step~3}: The beamformer output $z$ is computed in a straight forward fashion using $L$ threads that first sum the subarrays and then apply the MVDR weighting function. A single thread finally sum all the channels up.}\label{mvdr_implementation}
\end{figure}
...
%DIF <  What about the GPU implementation? This one was tricky. Balance load, avoid inter-thread dependencies when performing iterative summations, keep thread memory consumption at a minimum, and promote collective access patterns (coalesced reads/writes) to global memory.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In Fig. \ref{mvdr_implementation} we illustrate how we get around these challenges. First we make each SM compute entire range lines of pixels, then load the subset of $\x$ that all these pixels depend upon into shared memory. This lets us perform temporal averaging without having to read additional channel data.

\DIFdelbegin %DIFDELCMD < \ifPeerReview\else
%DIFDELCMD < \begin{figure}[!t]\centering
%DIFDELCMD < \includegraphics[width=\linewidth]{gfx/plot_holmengraa_L16_Navg1.eps}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{HISAS sidescan sonar (SSS) image of the shipwreck Holmengraa that lies on a slanted seabed at 77}%DIFDELCMD < \;%%%
\DIFdel{m depth outside of Horten, Norway. The image was processed with $M=32$, $L=16$, $K=1$ and $d=1\%$. Prior to display the image was linearly upinterpolated by a factor 2 in azimuth making its total size 1.46 megapixels (MP). Note how MVDR improves edge definition and reduces noise in shadow regions.}}%DIFAUXCMD
%DIFDELCMD < \label{holmengraa}
...
%DIFDELCMD < %%%
\DIFdelend Second, we assign $L$ threads per pixel to traverse the diagonals of $\eR$. On the top row a full computation is carried out, then that row is saved back to global memory following a coalesced access pattern that maximizes global memory bandwidth. For subsequent rows the threads move along the diagonals while performing iterative summations; the result from the previous element on the diagonal is updated by adding and removing the correlation coefficients that enters and exits the sum, respectively. To minimize memory consumption, we compute the coefficients again every time we need them. When a thread has finished up a diagonal, we have them wrap around to compute one of the diagonals in the lower triangular of $\eR$. Since $\eR$ is conjugate symmetric, the values in the leftmost column is obtained by a complex conjugate copy of the relevant value in the first row. This is slightly non-optimal as it causes divergent branching, but is \DIFdelbegin \DIFdel{only }\DIFdelend needed because the solver is Gauss Jordan based and thus require a full $\eR$. Combined these steps balance the load evenly on all threads, is almost completely free of arithmetic redundancy, and consumes less memory.


%DIF <  Hence, whenever $N_\text{avg}>1$ the sliding window approach will demand less instructions at the expense of a slightly higher memory consumption.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  However, an advantage with this approach is that the summations can be carried out in any order, and hence this form exhibits great deal of parallelism. Can we optimize the algorithm without sacrificing parallelism?
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  - Order of summations - beamspace capable
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  
%DIF <  In the upcoming results, we have beamformed pre-delayed data. This is a large dataset, and the latency experienced when copying it from the \gls{CPU} side to the \gls{GPU} was significant. However, in a practical system only the channel data should be transferred as the delaying of data can effortlessly be performed by the \gls{GPU}. The remaining latency can be hidden as long as the \gls{GPU} is kept busy, hence the data transfer times will not be reported in the upcoming results.
%DIF <  
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \begin{itemize}
%DIF <  \item Arrange order of summations to reduce data dimensions as early as possible.
%DIF <  \item Sliding window in time/space to reduce instructions at the cost of extra memory consumption. Only pays off to do this over subarrays.
%DIF <  \end{itemize}
%DIF <  
%DIF <  \item Between GPU and CPU: Several concurrent blocks (contexts) per SM.
%DIF <  \item Job queue (how does this work)
%DIF <  \item Register pressure and shared memory. Thread context must fit in local memories.
%DIF <  \item Warp computational intensity should be high to make up for the memory cost.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  increasing performance hit several walls:
%DIF <  \begin{itemize}
%DIF <  \item memory couldnt keep track - remedy cache.
%DIF <  \item instruction parallelism - complex to identity
%DIF <  \item power consumption cubic with frequency
%DIF <  GPUs:
%DIF <  \begin{itemize}
%DIF <  \item lower frequency, less power
%DIF <  \item no instruction parallelism
%DIF <  \item massive amounts of lightweight cores
%DIF <  \end{itemize}\end{itemize}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % \cite{Owens2007}\todo{Find a good spot for these}

\subsection{Solving $\hat{\boldsymbol{R}}$$\;\!^{-1}$$\mathbf{1}$}

As intuition might suggest the matrix product $\eRi\1$ can be carried out by first inverting the matrix $\eR$, then performing the inner product with $\1$. However, one can also solve the linear equation $\hat{\boldsymbol{R}}\boldsymbol{b} = \1$ for $\boldsymbol{b}$, where $\boldsymbol{b} = \eRi\1$. This is the preferred approach since \DIFdelbegin \DIFdel{one vector In terms of arithmetic count the latter is preferred , and when comparing direct implementations, the latter appears to be the preferred option}\DIFdelend \DIFaddbegin \DIFadd{the solution is obtained directly and without the added computational cost of the final inner product}\DIFaddend .

An important thing to note, however, is that unlike the problems most GPU libraries tries to solve we do not attempt so solve large linear equations or invert large matrices; our matrices are small, but we have a very large number of them. Fortunately Nvidia recently released a highly optimized batched linear equation solver tailored for this particular task. It is Gauss Jordan based and supports partial pivoting and complex numbers. The only downside to using it in our application is that it does not exploit the Hermitian property of our sample covariance matrix. A better choice \DIFdelbegin \DIFdel{might }\DIFdelend \DIFaddbegin \DIFadd{would }\DIFaddend be a solver based on Cholesky decomposition. These are designed for Hermitian positive semi-definite matrices \DIFaddbegin \DIFadd{such as our covariance matrix}\DIFaddend , and require \DIFdelbegin \DIFdel{roughly }\DIFdelend \DIFaddbegin \DIFadd{only }\DIFaddend half the arithmetic operations \DIFdelbegin \DIFdel{compared to }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend a Gauss Jordan solver.



%DIF <  \emph{Computing} $\eRi[n]\1$ (from (\ref{weights})). While intuition may suggest that this step is carried out by inverting $\eR$, it is better to solve the linear equation $\R\beta = \1$ for $\beta$ instead, which gives us $\beta = \eRi\1$ directly. We have tested various solvers for this task, both in-house and proprietary implementations, and achieved the best performance by using an unofficial batch solver from nVidia. Inverting $\eR$ is by far the most computationally intensive task, and a key area of focus for further improvements.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \ifPeerReview\else
%DIFDELCMD < \begin{figure}[!t]\centering
%DIFDELCMD < \includegraphics[width=\linewidth]{gfx/code_assessment.eps}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Execution time of an arithmetic-only and a memory-only version of the MVDR code. A dataset from an $M=32$ array was processed for all $L$ using $K=1$, and the mean execution time for a 1}%DIFDELCMD < \;%%%
\DIFdel{MP image was used here. From this plot we can infer that the kernel building $\eR$ is memory bound, as the time the kernel spends performing memory transactions is higher than the corresponding time it spends carrying out arithmetical operations. Furthermore, when the total runtime is larger than the restricted kernels this can largely be attributed to latency, which we can see that building $\eR$ suffers from with the chosen parameters.}}%DIFAUXCMD
%DIFDELCMD < \label{code_assessment}
%DIFDELCMD < \end{figure}
%DIFDELCMD < \fi%%%
%DIF < 
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Computing $z$}
...
\subsection{\DIFdelbegin \DIFdel{Summary of GPU implementation}\DIFdelend \DIFaddbegin \DIFadd{Implementation summary}\DIFaddend }

\DIFdelbegin \DIFdel{The }\DIFdelend %DIF >  The GPU implementation can be summarized as follows:
%DIF >  \begin{itemize}
%DIF >  \item Pixels are processed in batches.
%DIF >  \item Multiple threads per pixel.
%DIF >  \item $\eR$ is computed directly from raw data to minimize memory footprint.
%DIF >  \item Inversion step is performed by Nvidias batch linear equation solver.
%DIF >  \item Design makes efficient use of cache and data transfers are kept to a minimum.
%DIF >  \end{itemize}
%DIF >  
%DIF >  We compare this to our CPU implementations, where:
%DIF >  \begin{itemize}
%DIF >  \item Pixels are processed one by one.
%DIF >  \item Single thread per pixel.
%DIF >  \item Computation of $\eR$ is optimized for minumum number of instructions, as in \ref{mvdr_build_R}. Only upper half of $\eR$ is computed (see next point).
%DIF >  \item Inversion step carried out by a custom inline Cholesky based solver that only requires the upper triangular of $\eR$.
%DIF >  \item Design has generally not been optimized for efficient use of memory.
%DIF >  \end{itemize}
\DIFaddbegin 

\DIFadd{The GPU design is summarized and compared to our CPU designs in table \ref{implementation_summary}.
}
\begin{table}
[h]\centering%DIF > \normalsize
%DIF >  \begin{tabular}[c]{l r r r@{}  l}\hline
\begin{tabular}[l]{l >{\raggedright}p{.27\linewidth} p{.28\linewidth}}\hline
\rowcolor{tabBlue} & \bf \DIFaddendFL GPU \DIFdelbeginFL \DIFdelFL{implementation can be summarized as follows:
}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item %%%
\DIFdelFL{Pixels are processed in batches.
}%DIFDELCMD < \item %%%
\DIFdelFL{Multiple threads per pixel.
}%DIFDELCMD < \item %%%
\DIFdelFL{$\eR$ is computed directly from raw data to minimize memory footprint.
}%DIFDELCMD < \item %%%
\DIFdelFL{Inversion step is performed by Nvidias batch linear equation solver.
}%DIFDELCMD < \item %%%
\DIFdelFL{Design makes efficient use of cache and data transfers are kept to a minimum.
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelFL{Compared to our CPU implementations, where:
}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item %%%
\DIFdelFL{Pixels are processed one by one .
}%DIFDELCMD < \item %%%
\DIFdelFL{Single thread per pixel .
}%DIFDELCMD < \item %%%
\DIFdelFL{Computation of }\DIFdelendFL \DIFaddbeginFL & \bf \DIFaddFL{CPU }\\\hline
\DIFaddFL{Pixel processing }& \DIFaddFL{In batches }& \DIFaddFL{One by one }\\
\DIFaddFL{Threads per pixel }& \DIFaddFL{Multiple }& \DIFaddFL{One }\\
\DIFaddFL{1. Building }\DIFaddendFL $\eR$\DIFdelbeginFL \DIFdelFL{is optimized for minumum number of instructions, as in \ref{mvdr_build_R}. Only upper half of $\eR$ is computed (see next point).
}%DIFDELCMD < \item %%%
\DIFdelFL{Inversion step carried out by a custom inlineCholesky based solver that only requires the upper triangular of $eR$.
}%DIFDELCMD < \item %%%
\DIFdelFL{Design has generally not been optimized for efficient use of memory.
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{:  }& & \\
\ \ \ \DIFaddFL{Values computed }& \DIFaddFL{All }& \DIFaddFL{Only upper triangular }\\
\ \ \ \DIFaddFL{Optimization    }& \DIFaddFL{Minimized memory consumption }& \DIFaddFL{Minimized number of instructions }\\
\DIFaddFL{2. Computing $\eRi\1$: }& & \\
\ \ \ \DIFaddFL{Method }& \DIFaddFL{Nvidia's batch}\newline \DIFaddFL{Gauss Jordan solver }& \DIFaddFL{Custom inline}\newline \DIFaddFL{Cholesky solver }\\
\DIFaddFL{Cache utilization }& \DIFaddFL{Efficient }& \DIFaddFL{Not efficient }\\
\DIFaddFL{Data transfers }& \DIFaddFL{Minimum }& \DIFaddFL{Moderate
}\end{tabular}
\caption{\DIFaddFL{Implementation summary.}}\label{implementation_summary}
\end{table}
\DIFaddend 


%DIF <   The subarray data can, as here, be reduced to coincide in length with the subarray weights, or the weights can be extended to $M$ in size and applied directly on $\x[n]$ and summed.  
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \item \emph{Computing the spatial covariance matrix} $\eR[n]$ (\ref{spatialR}-\ref{finalR}). A group of threads are created that slide along the diagonals of $\eR$. In this way we exploited the fact that entries on the diagonals overlap across subarrays and time, and keep the numbers of both data reads and writes at a minimum.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % There are essentially two ways to accelerate an algorithm on parallel hardware. One is to identify independent instructions for each thread, and run these in in parallel, and the other is to support the parallel execution of as many threads as possible. This is referred to as instruction level parallelism (ILP) and data level parallelism (DLP), respectively. Most general purpose processors, such as a CPUs, support both DLP by featuring multiple cores with single-data multiple-data (SIMD) capabilities, and ILP by through means such as branch prediction and out-of-order execution.
...
%DIF <  In short, designing for a GPU involves finding the optimal balance between hiding memory latency (high occupancy) and resource utilization.
%DIF <  
%DIF <  blocks in grid - high enough so that all SMs have atleast one block to execute. Optimally in the thousands to scale for future devices, and to allow an SM to switch between blocks to keep hardware busy when running synchronizing threads.
%DIF <  
%DIF <  threads in block - min 64. 128-256 better starting point.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Instead of having a few complex core that can boost massive single thread performance, designers chose to create hundreds of lightweight cores that each is much powerful than a single CPU core, but combined boasts a massive artithmetical performance.
%DIF <  
%DIF <  \begin{itemize}
%DIF <  \item Lots of cores, little memory for each.
%DIF <  \item Few data transfers, lots of computations.
%DIF <  \item Data transfers are moved around by hardware that can operate asynchronously from the compute units.
%DIF <  \item Memory latency hidden as long as it takes longer to compute than to move data around.
%DIF <  \item Tasks can be queued on SM (blocks) and on CPU to hide internal and external memory latencies.
%DIF <  \end{itemize}
%DIF <  
%DIF <  Vast amount of processing power, which are easily accessible but hard to fully harness. This is because in the pursuit of an ever increasing amount of arithmetical computing cores, the 
%DIF <  
%DIF <  Unlike a CPU, a GPU 
%DIF <  
%DIF <  If we compare a GPU with a CPU, 
%DIF <  
%DIF <  When implementing an algorithm on a GPU, it is important to be aware
%DIF <  
%DIF <  People friendly:
%DIF <  \begin{itemize}
%DIF <  \item More and more compute units, but less memory for each.
%DIF <  \item Need to keep each compute unit busy.
%DIF <  \item Few data transfers, lots of computations.
%DIF <  \item Data transfers are moved around by hardware that can operate asynchronously from the compute units.
%DIF <  \item Memory latency hidden as long as it takes longer to compute than to move data around.
%DIF <  \item Tasks can be queued on SM (blocks) and on CPU to hide internal and external memory latencies.
%DIF <  \end{itemize}
%DIF <  
%DIF <  The ``fast'' memory of the GPU is the registers and cache. All threads executed 
%DIF <  
%DIF <  
%DIF <  \begin{itemize}
%DIF <  \item Hiding memory access.
%DIF <  \begin{itemize}
%DIF <  \item Between GPU and CPU: Several concurrent blocks (contexts) per SM.
%DIF <  \item Job queue (how does this work)
%DIF <  \end{itemize}
%DIF <  \item Register pressure and shared memory. Thread context must fit in local memories.
%DIF <  \item Warp computational intensity should be high to make up for the memory cost.
%DIF <  \item Optimized for small L, high M, hardly any time averaging. A typical sonar scenario.
%DIF <  \end{itemize}
%DIF <  - Split program into equally sized portions - load balancing
%DIF <  - Some portions depend on other potions already being carried outer - sequential dependencies
%DIF <  - Effort timed in order to finish at the same time - synchronization
%DIF <  
%DIF <  - task level parallelism - compute images independent of each other
%DIF <  - data level parallelism - compute pixels independent of each other
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  - Compute bound vs. bandwidth bound
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Images and Benchmarks}\label{images_and_benchmarks}

\ifPeerReview
\begin{figure}
[!t]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{gfx/buske7.eps}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{HISAS sidescan sonar (SSS) image of the shipwreck Holmengraa that lies on a slanted seabed at 77}%DIFDELCMD < \;%%%
\DIFdelFL{m depth outside of Horten, Norway. The image was processed with $M=32$, $L=16$, $K=1$ and $d=1\%$. Prior to display the image was linearly upinterpolated by a factor 2 in azimuth making its total size 1.46}%DIFDELCMD < \;%%%
\DIFdelFL{MP. Note how MVDR improves edge definition and reduces noise in shadow regions.}}%DIFAUXCMD
%DIFDELCMD < \label{holmengraa}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]{gfx/buske7\figPostfix.eps}
\else
\begin{figure}
[!t]\centering
\includegraphics[width=\linewidth]{gfx/plot_holmengraa_L16_Navg1.eps}
\DIFaddendFL \fi%
\DIFaddbeginFL \caption{\DIFaddFL{HISAS sidescan sonar (SSS) image of the shipwreck Holmengraa that lies on a slanted seabed at 77}\;\DIFaddFL{m depth outside of Horten, Norway. The image was processed with $M=32$, $L=16$, $K=1$ and $d=1\%$. The image was linearly upinterpolated by a factor 2 in azimuth making its total size 1.46 megapixels (MP). Note how MVDR improves edge definition and reduces noise in shadow regions.}}\label{holmengraa}
\end{figure}
\DIFaddend 
...
\begin{figure}
[!t]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{gfx/buske8.eps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]{gfx/buske8\figPostfix.eps}
\DIFaddendFL \else
\begin{figure}
[!t]\centering
...
\begin{figure}
[!t]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{gfx/buske10.eps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]{gfx/buske10\figPostfix.eps}
\DIFaddendFL \else
\begin{figure}
[!t]\centering
...
\begin{figure}
[!t]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{gfx/buske11.eps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]{gfx/buske11\figPostfix.eps}
\DIFaddendFL \else
\begin{figure}
[!t]\centering
...
As seen in the run-time comparisons presented in \Fig{benchmarks}, the GPU method \DIFdelbegin \DIFdel{is now }\DIFdelend \DIFaddbegin \DIFadd{became }\DIFaddend 2-3 orders of magnitude faster than the C implemtation we started out with. The \DIFaddbegin \DIFadd{remaining }\DIFaddend bottleneck in the final design is \DIFdelbegin \DIFdel{now }\DIFdelend the inversion step, which is typically 5 times slower than the build step. In most cases the processing speed of the Quadro 6000 is above 1\;MP/s, and this was only improved by a factor 1.5-2 when run on the new K20 GPU. The benchmarks of our memory-only and arithmetic-only kernels show that the kernels spend roughly the same time on both these tasks, so optimising only one of these further will have marginal \DIFdelbegin \DIFdel{effect}\DIFdelend \DIFaddbegin \DIFadd{impact}\DIFaddend . All GPU kernels were compiled with \texttt{nvcc} at optimization level \texttt{O2}. Excluded from these benchmarks is the data transfer time from CPU to GPU, which account for 2-20\% of the total processing time. We believe it makes little sense to include them since it keeps getting easier \DIFaddbegin \DIFadd{to }\DIFaddend perform these data transfers in parallel by offloading the task to the Direct Memory Access (DMA) controllers present on modern GPUs. Furthermore, the data rates in active sonar are relatively low compared to the bandwidth available for these transfers.
%DIF <  
...
We have shown that for systems \DIFdelbegin \DIFdel{containing }\DIFdelend \DIFaddbegin \DIFadd{consisting of }\DIFaddend up to 32 channels the problem can be largely mitigated by building the covariance matrix in a clever way, and by making use of the massive computational power available in modern GPUs. We were able to improve upon the run-time of a single thread C-implementation by roughly two orders of magnitude. For most choices of parameters the GPU was able to create images at $\sim$1\;MP/s at an average data processing rate of \DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{$\sim$}\DIFaddend 40\;GFlop/s. This is less than 5\% of the peak performance of the GPU, but we believe it to be near optimal given the constraints of memory bandwidth and the sequential nature of some parts of the MVDR implementation.
...
%DIF <  \begin{figure}[t]
%DIF <  \centering
%DIF <  \includegraphics[width=\linewidth]{gfx/benchmark_tagged.eps}
%DIF <  \caption{Realtime requirement for sectorscan imaging.}\label{real-time_sectorscan}
%DIF <  \end{figure}
\DIFdelbegin %DIFDELCMD < 
...
%DIF <  \begin{itemize}
%DIF <  \item UFFC(?)
%DIF <  \item Ultrasound. Arrays up to 128 channels. Processing power vital. What's the best way to implement 
%DIF <  \item Other articles evaluates Capon image quality, and some have shown that speedups can be achieved by reducing the complexity of the traditional Capon algorithm or by implementing specific versions on a GPU.
%DIF <  \end{itemize}
%DIF <  }
%DIF <  
%DIF <  \begin{itemize}
%DIF <  \item As briefly as possible, introduce beamforming.
%DIF <  \item Adaptive beamformer's potential lies in its ability to suppress interference power
%DIF <  \item Why adaptive beamformers struggle in active sonar systems. Correlated noise, robustification kills the adaptive potential. Quite computationally intensive. Constraints must be applied in one way or another - parameters must be tuned.
%DIF <  \item The three main ways to implement Capon \cite{Capon1969} is \todo{not entirely true?}
%DIF <  \begin{itemize}
%DIF <  \item Traditional way: Build $\eR$ and solve $\w = \frac{\Ri\a}{\a\H\Ri\a}$. 
%DIF <  \item Iterative methods: Woodbury, Conjugated gradients
%DIF <  \item Beamspace
%DIF <  \item LCA (or other methods, as reference)
%DIF <  \end{itemize}
%DIF <  fs = 100kHz*30\%*$\frac{4}{3}$ = 40kHz \\
%DIF <  $N_{range px} = \frac{2\;250m}{1500m/s}40kHz = 13.3kpx$ \\
%DIF <  $\times 32$ beams = 426kpx \\\\
%DIF <  
% PRF = $\frac{1500m/s}{2 250m} = 3Hz$ \\
% TP = 215px 3Hz = 1.28MP/s \\
% 4 arrays: 1.28MP/s * 4 = 5.12MP/s
...
\begin{align}
O_{\underset{\text{initial}}{\text{Build $\eR$}}} &= \underbrace{\Om\Nk\Nl L^2}_\text{Multiplications} + \DIFdelbegin %DIFDELCMD < \Oa%%%
\DIFdel{(}%DIFDELCMD < \Nk%%%
\DIFdelend \DIFaddbegin \underbrace{\Oa(\Nk+\Nl-2)L^2}\DIFadd{_\text{Additions} }\DIFaddend + \DIFdelbegin %DIFDELCMD < \Nl%%%
\DIFdel{-2)L^2 + }\DIFdelend O_\text{dload},
\end{align}
...
The Quadro 6000 has 32 CUDA cores per SM, each operating at a rate of 1148\;MHz and being able to perform 2 floating point operations (flop) per clock cycle when multiply-add instructions are used. \DIFdelbegin \DIFdel{Theoretical }\DIFdelend \DIFaddbegin \DIFadd{The theoretical }\DIFaddend peak arithmetic throughput is then given as:
\begin{align}
\text{B}_\text{arith} &= 2\cdot1148\;\text{\DIFdelbegin \DIFdel{flop}\DIFdelend \DIFaddbegin \DIFadd{Mflop}\DIFaddend /s/core}\cdot 32\;\text{cores/SM}\cdot 14\;\text{SMs}\nn
...
%DIF < In such cases instruction level parallelism can be exploited to move data from shared memory into the even faster register memory~\cite{Vasilyy}, but this is outside the scope of this article.
...
The authors would like to thank Kongsberg Maritime and the Norwegian Defence Research Establishment (FFI) for providing experimental data, and thank Nvidia for providing support \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend running the batched linear equation solver \DIFdelbegin \DIFdel{as well as }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend for granting us a testdrive at the Boston HPC center.
...
\DIFdelbegin %DIFDELCMD < \bibliography{../../Library/library}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{library.bib}
\DIFaddend 
