...
\ai{This is a good point. The Oxford dictionary lists "MP" as the correct abbreviation for "megapixel", so we substituted "Mpx/s" with "MP/s" and included a proper definition.}
\qi{"Matlab" should be all caps: "MATLAB" throughout the paper.}
\ai{Thank you, we were not aware of this. Corrections applied.}
%
\hi{Abstract}
...
\ai{You are right that subarray averaging is important in our study. But we believe that "subarray MVDR" would be misleading, as our use of subarrays is strictly due to the application of MVDR in an active system to decorrelate signal and interference. TODO: Citation }
%
...
\ai{Thank you for this remark. We agree fully, and will state this better up front in the abstract. TODO}
%
\hi{Section I}
...
\ai{Thank you providing us with this interesting reference. However, we can not see that the technique described by Chapman is relevant for us. He is not using overlapping subarrays like we do, and obtains a sparse array with associated grating lobe problems. We use of subarrays to (1) deal with the coherency problem in active sonar, and (2) to provide robustness to our covariance estimate. The latter is particularly important in our near single snapshot application. We attempt to emphasize this further in the report, and suggest [9] Kailath and Shan, "Adaptive beamforming for coherent signals and interference", as well as R.T.Hoctor and S.A.Kassam, "The unifying role of the coarray in aperture synthesis for coherent and incoherent imaging" as good reads on the subject. }
%
\hi{Section II}
\qi{Should reference the literature (from above comment 6) on the subarray approximation in paragraph 3.}
\ai{Certainly. Reference added.}
\hi{Section III (A-C)}
...
\ai{Yes, the initial plots had the total GPU processing time included. But since there are three versions of the covariance building step, there TODO }
%
...
\qi{On page 4, the authors talk about avoiding global memory, without get-ting into "coalesced" memory access patterns. This might merit a mention - for example, is at best only able to move one float for every 30 floats processed, and potentially much worse if care is not taken to use "coa-lesced memory access patterns (that avoid memory bank conflicts)". On page 5, the authors talk about "a collective access pattern that maximizes global memory bandwidth". Here too, the authors should perhaps use the "coalesced" term, since this is the term used in all of the GPGPU documentation.}
\end{enumerate}
...
\qi{Paragraph after equation (2) mentions "spatial averaging to avoid signal cancellation" - this problem might be better identified for readers by cit-ing the Kailath reference and using "signal cancellation due to coherent multipath".}
...
\item Fig. 8 - Abbreviating megapixels as "MP" in stead of "MPx".
\item Fig. 9 - Abbreviating megapixels as "MP" in stead of "MPx".
\item Fig. 11 - Abbreviating megapixels as "MP" in stead of "MPx".
\end{enumerate}
