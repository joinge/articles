...
\ai{Crude grammar correction applied.}
%
...
\ai{We agree that this should should be clarified, but since the transfer can happen in parallel without adding to the computation time we do not believe this point belongs in the introduction. An elaboration on the topic is included in the last paragraph in section IV. }
%
\hi{Section V}
...
\ans{We thank the reviewer for these remarks, it seems we should state this better upfront. In the 5th paragraph of the introduction we now explain that our system is a well sampled wideband sonar and that we operate in near field conditions.}
...
\end{align}
\ai{The reviewer made us realize that we should clarify this. The 32 elements are evenly spaced along the 1.2m array, so:
\begin{align}
M &= 32 \\
...
\ai{Yes, we are pre-steering the array for every angle. We have clarified this further (see 3.), and added a remark saying that while we can not use it here (due to processing a wideband signal) a phase steered version of MVDR can be obtained by introducing a steering vector $\vec s$.}
%
...
\ans{The reviewer is absolutely right. The copy will be serialized due to divergent branching. It is unfortunate that to the best of our knowledge no Cholesky based batch linear equation solver exist, which would have eliminated the need for the lower triangular. The paragraph has been edited to explain this better.}
...
\ai{We indeed presteer all the elements to deal with with our wideband signal. To make this clearer we have now stated it again after equation (2), and explained that in a narrowband case a generalization is straight forward by substituting $\1$ with a steering vector.}
%
...
\ai{The explanation is provided in the section V, the discussion. We have now rewritten the relevant paragraphs to include answers to these and other comments. Regarding the discussion of the FLOPS these were never intended to be an accurate representation of the run-times but rather to provide an indication of how efficiently we are able to harness the GPU's resources. We have reworded the paragraph slightly to emphasize this.}
%
