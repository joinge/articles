...
This chapter briefly describes the motivation, methods and results of each article. \Fig{3_fig_article_relation} (same as \Fig{2_fig_article_relation}) offers visual context.
...
%Element-space MVDR beamforming on GPUs in active sonar imaging
...
Beam-space MVDR beamforming on GPUs in clinical ultrasound imaging
...
LCA beamforming versus MVDR in active sonar imaging
...
Real-time sonar image synthesis on GPUs for automatic target recognition
...
\item Demonstrates superior classification accuracy on experimental data of a few thousand objects from Jesus Bay, Norway, at false positive rates (FPR) below 20, similar\todo{???} %, and tied with it above. 
\end{enumerate}
