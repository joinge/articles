...
Any wave, regardless of its nature, are governed by a partial differential equation known as the \emph{wave equation}:
%
\begin{align}
\nabla^2 f 
...
A few solutions to the wave equation are of interest, in particular that of a plane and spherical wave:
%
\begin{align}
...
\end{figure}
%
The basic idea behind MVDR is to compute the set of weights that minimizes noise and signal power accumulated by the array, while seeking unit gain for signals from the pixel of interest. With pre-delayed data this can be formulated as follows:
%
\begin{gather}
