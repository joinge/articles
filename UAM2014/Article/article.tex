...
Template matching is a common technique for classification of objects in synthetic aperture sonar (SAS) images. The principle is to isolate an image segment containing the object of interest, correlate it with a set of template images of the relevant object classes, and assign it to the class of the template yielding the highest correlation coefficient. The challenge is to come up with a representative set of template images covering the actual configurations of object and seabed\todo{objects? seabeds?}. The use of a static, predetermined template library may be impractical, yield inaccurate results, or both~\cite{Midelfart2010}. 
...
\caption{Standard template. Predefine template's cylinder length, alignment and burial depth do no match. Shadow and highlight regions to not correspond well.}\label{overlay_cylinder}%
\end{subfigure}\hspace{\imgspacing}%
\begin{subfigure}[t]{.5\linewidth-\imgspacing/2}
\graphicsAI[width=\linewidth]{gfx/overlay_cylinder_submerged_adaptive.svg}%
\caption{Adapted template. Template simulated with adapted parameters. Good fit in both highlight and shadow regions.}\label{overlay_cylinder_adaptive}%
\end{subfigure}
...
One way to automatically classify objects in SAS images is compare the imaged objects with a predefined set of templates. These are typically simple simulations of common objects with various sizes and shapes. With luck one might be a close match, but 

Template matching is a common technique for automatic classification of objects in synthetic aperture sonar (SAS) images.

We have implemented a SAS simulator that creates object templates in delayed real-time based on parameters estimated from the current scene. Compared to the conventional method  having a predefined set of object templates 


, and not just a  on-the-fly based on adapted parameters, and not just a predefined set. 
Modern GPUs are customized to perform computer graphics. Their theoretical peak performance is typically and order of magnitude higher than a CPU in a comparable price range. While it is hard to obtain this performance in practice, one can get very close by using a well matured and highly optimized graphics processing API such as OpenGL. Because of this we use OpenGL for simulating a scene with 3D models of the seabed and objects.
