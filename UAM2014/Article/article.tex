...
We have developed fast a synthetic aperture sonar (SAS) simulator that run on graphics processing units (GPUs)  using a combination of OpenGL for graphics processing and OpenCL for post-processing. This implementation resembles the SIGMAS+ simulator developed at the NATO Undersea Research Centre (NURC). They both take advantage of graphics processing to render a 2D image from 3D models and assume a Lambertian scattering model. 
...
The basic idea behind using a graphics 

OpenGL is a multi-platform application programming interface (API) for rendering 2D and 3D vector graphics. 
