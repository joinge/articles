...
Modern GPUs are customized to perform computer graphics. Their theoretical peak performance is typically and order of magnitude higher than a CPU in a comparable price range. This performance can not always be achieved in practice, unless the algorithm can be adapted to use a well matured and optimized graphics processing API such as OpenGL. Our simulator uses OpenGL for most of the , then it makes little 

Whether this performance can be achieved in practice 
Whenever conventional computer graphics processing can be used to 
GPUs are designed to provide the best possible graphics processing performance, and there large ecosystem of tools and libraries  available 
Using a GPU to drive a simulator is advantageous since
