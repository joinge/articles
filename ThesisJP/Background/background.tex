...
\caption{Schematic overview of CPU (Intel Haswell) and GPU (Nvidia Kepler) architecture. Note how more space is dedicated for compute cores on a GPU, and how more space is dedicated to control and caching on a CPU.}
\label{fig:cpu_gpu}
\end{figure}
...
\end{equation}
in extra processing power to the CPU chip.
...
Both CUDA and OpenCL is centered around a kernel function which are executed be a large group of threads. The kernel function, for both CUDA and OpenCL, has a C-like syntax with some additional specifiers. A CUDA kernel is depicted in Fig.\,\ref{fig:gpu_kernel}. The \texttt{\_\_global\_\_} specifier tells the CUDA compiler that this function is a kernel function, and, in CUDA, this kernel function is launched by using a CUDA specific syntax:
\begin{align*}
\texttt{square<<<grid, block>>>(a, N)}
\end{align*}
where \texttt{grid} and \texttt{block} is a structure containing the grid and block size as depicted in Fig.\,\ref{fig:gpu_grid}, and \texttt{a} is a pointer to a memory buffer located on the GPU. When launched, a copy of the kernel is executed for each thread in each block. Each kernel get as input its location in the compute grid (\texttt{blockIdx} and \texttt{threadIdx}).
