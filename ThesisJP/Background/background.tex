...
\end{align}
where \nom{$\vec{w}$}{Array weight vector (apodization)} is know as the array weight vector or apodization, and $z[n]$ is the output A-mode image.  If $x_o$ is the phase center of our array and $p$ is the point where we want to steer and focus the ultrasound beam, we have to use the following delays:
\begin{align}
\Delta_m[n] =\frac{|x_m+p| - |x_o+p|}{c},
\end{align}
where \nom{$c$}{Speed of sound} is the speed of sound in human tissue. On transmit the inverse set of delays have to be used. Also note that on transmit we have to decide where to steer and focus prior to transmitting a pulse, transmit delays are therefore not time varying.
