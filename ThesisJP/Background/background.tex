...
N = \frac{2r}{c}B,
\end{align}
\nomi{$N$}{Number of samples in range}where \nom{$B$}{Frequency bandwidth} is the bandwidth, $B = B_wf_c$ (where \nom{$B_w$}{Relative bandwidth} is the relative bandwidth and \nom{$f_c$}{Center frequency of ultrasound pulse} is the center frequency of the transmitted pulse). Note that after this process the ultrasound data will be complex values. The total number of samples in an B-mode image is then 
\begin{align}
...
For the low complexity data-dependent beamformer all the shifted windows are shifted less than the mainlobe width.
