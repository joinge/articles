...
%DIF DEL lca.tex        Mon Feb 22 15:49:56 2016
%DIF ADD lca_rev1.tex   Mon Oct 12 11:48:37 2015
%DIF < \synctex=1
%DIF -------
 %DIF > 
%DIF -------
...
%DIF 343-346d343
%DIF < \ifPeerReview
%DIF < \graphicspath{./submission/submit1}
%DIF < \fi
%DIF < 
%DIF -------
...
%DIF -------
\newcommand\mediaPath{gfx/media} %DIF > 
%DIF < \newcommand\mediaPath{http://folk.uio.no/joibu/media/2015_JOE_LCA}
\newcommand\mediaI{\multimedia{\mediaPath/media1.mp4}{Media Movie 1}} %DIF > 
%DIF < 
\newcommand\mediaII{\multimedia{\mediaPath/media2.mp4}{Media Movie 2}} %DIF > 
...
\title{Low Complexity Adaptive \DIFaddbegin \DIFadd{Beamformer}\\ \DIFadd{for Active }\DIFaddend Sonar Imaging}
...
% 
We have studied the low complexity adaptive (LCA) beamformer in active sonar imaging. LCA can be viewed as either a simplification of the minimum variance distortionless response (MVDR) beamformer, or as an adaptive extension to the delay and sum (DAS) beamformer. While both LCA and MVDR attempt to minimize the power of noise and interference in the image, MVDR \DIFdelbegin \DIFdel{achieves this by computing }\DIFdelend \DIFaddbegin \DIFadd{seeks the }\DIFaddend optimal \DIFdelbegin \DIFdel{array weights from }\DIFdelend \DIFaddbegin \DIFadd{solution by estimating and inverting }\DIFaddend the \DIFaddbegin \DIFadd{array's }\DIFaddend spatial \DIFdelbegin \DIFdel{statistics of the wavefield}\DIFdelend \DIFaddbegin \DIFadd{covariance matrix}\DIFaddend , while LCA selects the best \DIFdelbegin \DIFdel{performing weights out of }\DIFdelend \DIFaddbegin \DIFadd{performer from }\DIFaddend a predefined set \DIFaddbegin \DIFadd{of sensor windows}\DIFaddend .

\DIFdelbegin \DIFdel{To build confidence in the LCA method we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend show that a robust MVDR implementation typically creates \DIFdelbegin \DIFdel{weights spanning }\DIFdelend \DIFaddbegin \DIFadd{responses ranging }\DIFaddend between \DIFaddbegin \DIFadd{that of }\DIFaddend the rectangular and Hamming\DIFdelbegin \DIFdel{window function}\DIFdelend . We let LCA select from a set of Kaiser windows with responses in this \DIFdelbegin \DIFdel{span}\DIFdelend \DIFaddbegin \DIFadd{range}\DIFaddend , and add some steered variations of each. We limit the steering to roughly half the -3\,dB width of the window's amplitude response. Using experimental data from the Kongsberg Maritime HISAS 1030 sonar we find that LCA and MVDR produce nearly identical images of large scenes, both being superior to DAS. On point targets LCA is able to double the resolution compared to DAS, or provide half that of MVDR. This performance is achieved with a total of 6 windows; the rectangular and Kaiser window with $\beta=5$, in unsteered version, and left and right steered to the steering limit. Slightly smoother images are produced if the window count is increased to 15, but past this we observe minimal difference. Finally we show that LCA \DIFdelbegin \DIFdel{works }\DIFdelend \DIFaddbegin \DIFadd{work }\DIFaddend just as well if Kaiser windows are substituted with trigonometric ones.

All our observations and experiences point to LCA being very easy to \DIFdelbegin \DIFdel{understand and }\DIFdelend manage. It simply works, and is surprisingly insensitive to the exact type of window function, steering amount, or number of windows. It can be efficiently implemented on parallel hardware, and handles any scene without the need for parameter adjustments.
...
%DIF <  The promise is already demonstrated in nature.
%DIF <  
%DIF <  The sonar systems of the future 
%DIF <  
%DIF <  Future sonar systems 
%DIF <  
%DIF <  Great promise in adaptivity
%DIF <  
%DIF <  The greatest untapped 
%DIF <  
%DIF <  The sonars of the future might benefit greatly from adaptivity.
%DIF <  
%DIF <  
%DIF <  Future sonars will likely be 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  To make a sonar truly perform better, we need to make it smarter. 
%DIF <  
%DIF <  \IEEEPARstart{F}{uture} sonar systems will merit a much more sophisticated level of adaptivity. 
%DIF <  
%DIF <  \IEEEPARstart{T}{he} next generation sonar systems will be adaptive.
%DIF <  
%DIF <  Making sonars adaptive is the way to take them into the future. 
%DIF <  
%DIF <  The virtue of future sonar systems will be sophisticated adaptivity. 
%DIF <  
%DIF <  Intelligent
%DIF <  
%DIF <  Reactive
%DIF <  
%DIF <  Observe, process, adapt, act. 
%DIF <  
%DIF <  Adaptivity - sense, process, respond
%DIF <  
%DIF <  Faster, smarter
%DIF <  
%DIF <  The best sonars in nature are adaptive. 
%DIF <  Nature allures us to be this bold because 
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \IEEEPARstart{T}{he} best \DIFdelbegin \DIFdel{sonars in existence is perhaps }\DIFdelend \DIFaddbegin \DIFadd{adaptive sonars are }\DIFaddend found in nature. Bats, for instance, use a high frequency sonar to detect, identify and track prey with amazing precision in caves amongst a myriad of other bats. \DIFdelbegin \DIFdel{No man made sonar can currenly match this feat, much due to the challenge of processing and adapting to a complex environment fast enough. While }\DIFdelend \DIFaddbegin \DIFadd{Active }\DIFaddend human made sonars have little to no \DIFdelbegin \DIFdel{adaptivity }\DIFdelend \DIFaddbegin \DIFadd{adaptability }\DIFaddend on transmit, \DIFdelbegin \DIFdel{several }\DIFdelend \DIFaddbegin \DIFadd{but several adaptive }\DIFaddend methods have been investigated \DIFdelbegin \DIFdel{to adaptively }\DIFdelend post process the received data. Perhaps the best studied method for reconstructing a sonar image is the minimum variance distortion-less response (MVDR) beamformer~\cite{Capon1969}. It adapts the sonar array's \DIFdelbegin \DIFdel{spatial }\DIFdelend response to minimize the influence of noise and interference in the final image.

In many cases MVDR can improve the contrast and resolution of active sonar images compared to conventional static methods~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Blomberg2013,Blomberg2012a,Lo2004}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Blomberg2013,Blomberg2012a,Dursun2009,Lo2004}
}%DIFAUXCMD
}\DIFaddend . However, while the computational complexity of conventional beamformers are linear with the number of channels, O($M$), MVDR is at O($M^3$). This \DIFdelbegin \DIFdel{is because MVDR relies on estimating and inverting a covariance matrix. The estimation step dominates the computation in sonar systems with less than 32 channels, but here computational }\DIFdelend \DIFaddbegin \DIFadd{makes MVDR impracticable and sometimes infeasible. We have shown in previous studies that MVDR's }\DIFaddend complexity can be reduced\DIFdelbegin \DIFdel{and the }\DIFdelend \DIFaddbegin \DIFadd{, and its }\DIFaddend implementation accelerated significantly using graphics computing units (GPUs)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Buskenes2014}
}%DIFAUXCMD
. For larger systems the inversion step dominates. This can be dealt with by approximating the full covariance matrixwith a small one using space reduction techniques such as beamspace processing~\mbox{%DIFAUXCMD
\cite{Asen2013}
}%DIFAUXCMD
, or the closely related principal component analysis (PCA) method ~\mbox{%DIFAUXCMD
\cite{Kim2014}
}%DIFAUXCMD
. Another alternative is to assume spatial stationarity to form more easily invertible Toepliz matrices~\mbox{%DIFAUXCMD
\cite{Asl2012}
}%DIFAUXCMD
. However, the mentioned methods are still relatively slow compared to DAS.}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Buskenes2014,Asen2013}
}%DIFAUXCMD
. However, while MVDR relies on estimating and inverting a spatial covariance matrix, there exist a method that avoid this altogether.}\todo{samme avsnitt?}
\DIFaddend 

\DIFdelbegin \DIFdel{A much faster and simpler alternative to MVDR is }\DIFdelend \DIFaddbegin \DIFadd{This method is called }\DIFaddend the Low Complexity Adaptive (LCA) beamformer. \DIFdelbegin \DIFdel{Based on an idea by Vignon~\mbox{%DIFAUXCMD
\cite{Vignon2008}
}%DIFAUXCMD
, it was first introduced }\DIFdelend \DIFaddbegin \DIFadd{It was first proposed }\DIFaddend by Synnev\aa{}g \DIFdelbegin \emph{\DIFdel{et al}}%DIFAUXCMD
\DIFdel{. }\DIFdelend in clinical medical imaging~\cite{Synnevag2008}, who demonstrated its ability to obtain very similar \DIFdelbegin \DIFdel{image quality }\DIFdelend \DIFaddbegin \DIFadd{results }\DIFaddend to MVDR in \DIFdelbegin \DIFdel{systems }\DIFdelend \DIFaddbegin \DIFadd{a system }\DIFaddend with focused transmit beams. LCA applies a set of predefined windows \DIFdelbegin \DIFdel{, or apodization weights, }\DIFdelend and selects the one offering the best \DIFdelbegin \DIFdel{sidelobe }\DIFdelend \DIFaddbegin \DIFadd{noise }\DIFaddend suppression. It relies on the same optimization criterion as MVDR, \DIFdelbegin \DIFdel{thus }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend may be considered a version of MVDR with a reduced window solution space. Alternatively, it may be viewed as a \DIFdelbegin \DIFdel{multi-apodization technique. One such method is described by Stankwitz }\emph{\DIFdel{et al}}%DIFAUXCMD
\DIFdel{. in radar imaging~\mbox{%DIFAUXCMD
\cite{Stankwitz1995}
}%DIFAUXCMD
, where the best out of 2 or 3 windows are selected. However, LCA is different in that it typically selects from a larger pool of windows and allows micro-steered variations of them. This gives LCA an adaptive freedom that is somewhere in between that of traditional multi-apodization techniques and MVDR}\DIFdelend \DIFaddbegin \DIFadd{delay-and-sum (DAS) beamformer with an adaptive extension}\DIFaddend .

Synnev\aa{}g \DIFdelbegin \emph{\DIFdel{et al}}%DIFAUXCMD
\DIFdel{. suggested using }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend a window set comprised of rectangular, Kaiser \DIFdelbegin \DIFdel{(or Kaiser-Harris) }\DIFdelend and inverted Kaiser functions\DIFdelbegin \DIFdel{. In total he }\DIFdelend \DIFaddbegin \footnote{\DIFadd{May also be called Kaiser-Harris windows.}}\DIFadd{. He }\DIFaddend made LCA choose from 12 unique windows, 6 of which were micro-steered variants of a fairly wide Kaiser window\DIFdelbegin \DIFdel{, and with the rectangular window included. Micro-steering is means to phase-shift the array's elements to finetune the azimuth angle the array is steered towards. This was motivated by the desire to use wide windows with maximum sidelobe suppression near the receiver, and narrow windows at greater depths for maximum sensitivity and depth penetration}\DIFdelend . While achieving good results, it was not made clear how \DIFdelbegin \DIFdel{many windows LCA needs, or how they ideally should be steered. Also, it is not apparent how LCA performs in sonar systems where the transmit beam is unfocused}\DIFdelend \DIFaddbegin \DIFadd{LCA performs with more or fewer windows, and how steering affects it all}\DIFaddend . 

\DIFdelbegin \DIFdel{In this study we expand our previous work of applying LCA to active sonar imaging~\mbox{%DIFAUXCMD
\cite{Buskenes2011}
}%DIFAUXCMD
. We use }\DIFdelend \DIFaddbegin \DIFadd{We use LCA with }\DIFaddend variations of the \DIFdelbegin \DIFdel{parametric }\DIFdelend Kaiser window, but \DIFdelbegin \DIFdel{also compare it to the trigonometric window function}\DIFdelend \DIFaddbegin \DIFadd{a comparison to using trigonometric windows will also be provided}\DIFaddend . We extend Synnev\aa{}g's work by steering all our windows and not just some, and \DIFdelbegin \DIFdel{demonstrate how LCA's performance is affected by the type and number of windows included. Our results are obtained using }\DIFdelend \DIFaddbegin \DIFadd{elaborate on the type of Kaiser windows to include and the method of steering. Then the method is applied to }\DIFaddend experimental data from the 32 element Kongsberg Maritime HISAS 1030 synthetic aperture sonar (SAS), \DIFdelbegin \DIFdel{which operates at }\DIFdelend \DIFaddbegin \DIFadd{with an operating frequency of }\DIFaddend 100\,\DIFdelbegin \DIFdel{kHz with an -3}%DIFDELCMD < \,%%%
\DIFdel{dB element opening angle of 23}\DIFdelend \DIFaddbegin \DIFadd{kHz and transmit angle of 25}\DIFaddend $^\circ$. 

Our results show that \DIFdelbegin \DIFdel{when MVDR is modified to work in active sonar imaging, it }\DIFdelend \DIFaddbegin \DIFadd{MVDR }\DIFaddend computes weights with \DIFdelbegin \DIFdel{spatial }\DIFdelend responses that tend to be symmetric and steered within a fraction of the window's -\,3dB resolution. When letting LCA choose from Kaiser windows with similar responses, we obtain images with improved noise suppression and resolution. The resolution gain is made predictable by limiting the maximum steering to a fraction of the window's -3\,dB width. Overall, LCA produces images that are similar to MVDR, but with a resolution in between that of DAS and MVDR. LCA is also inherently robust, easy to implement \DIFdelbegin \DIFdel{, }\DIFdelend and fairly easy to understand.
...
\DIFdelbegin \DIFdel{A sonar image is formed by estimating }\DIFdelend \DIFaddbegin \DIFadd{To form a sonar image we need to estimate }\DIFaddend source locations and amplitudes. For this purpose \DIFaddbegin \DIFadd{we apply }\DIFaddend a spatial bandpass filter \DIFdelbegin \DIFdel{is applied }\DIFdelend to the backscattered wavefield data. The filter is called an array processor\DIFaddbegin \DIFadd{, }\DIFaddend or receive beamformer. The basic principle is to apply delay and weights to the sensor channels before summing them up, chosen such that signals from the location of interest are summed coherently, while other sources sum incoherently.

Assume that the wavefield has been sampled by an $M$ element uniform linear array, and that the signal signature has been removed by a matched filter. Let $x_m[\theta,n]$ be the delayed data from the $m$th channel, where the $\theta$ and $n$ \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend the azimuth angle and range sample of the focus point, respectively. Each angle $\theta$ will be processed independently, so to simplify notation we will assume the dependence on $\theta$ to be implicit from now on. 
...
\end{align}
%
where $w_m$ is the weight factor assigned to channel $m$. A weight set is commonly called a window \DIFdelbegin \DIFdel{, apodization or }\DIFdelend \DIFaddbegin \DIFadd{or a }\DIFaddend taper function. Applying a window that trails off towards the edges improves noise suppression at the cost of reduced resolution~\cite{Harris1978}. Real windows lead to symmetric responses, while complex weights allow asymmetric responses.
...
MVDR seeks to minimize the output \DIFdelbegin \DIFdel{variance }\DIFdelend \DIFaddbegin \DIFadd{power }\DIFaddend of the beamformer, under the constraint of a unity gain in some desired direction $\phi$~\cite{Capon1969}\DIFdelbegin \DIFdel{. With the assumption of zero mean data this can be formulated as}\DIFdelend :
%
\begin{align}
\underset{\w[n]}{\argmin}\, E\{\big|z[n]\big|^2\} &= \underset{\w[n]}{\argmin}\, \w[n]\R[n]\w\H[n]\nn
\text{subject to } \DIFdelbegin %DIFDELCMD < \w%%%
\DIFdel{^H[n]}\DIFdelend \DIFaddbegin \w[n]\DIFaddend \a_\phi &= 1.\label{eq:mvdr_definition}
\end{align}
%
Here $\a_\phi$ is a steering vector, $E\{\cdot\}$ is the expectation operator, and \DIFdelbegin \DIFdel{$\R[n] = E\{\x[n]\x\H[n]\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\R[n] = E\{\x\x\H\}$ }\DIFaddend is the spatial covariance matrix\DIFdelbegin \DIFdel{for the full array}\DIFdelend . This is a convex optimization problem with the solution:
%
\begin{gather}
\vec w[n] = \frac{\Ri[n]\a_\phi}{\a_\phi\T\Ri[n]\a_\phi}\DIFdelbegin \DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \label{eq:mvdr_weights}
\end{gather}
\DIFaddbegin \DIFadd{where $\R=E\{\x[n]\x\H[n]\}\in\mathbb{C}^{M,M}$ is the spatial covariance matrix for the full array. }\DIFaddend The problem lies in estimating and inverting this spatial covariance matrix. We have described the exact steps in~\cite{Buskenes2014}: To avoid signal cancellation we apply spatial averaging by computing a mean covariance matrix from a set of subarrays with length $L$~\cite{Kailath1985}, for true speckle statistics we perform temporal averaging over $N_k = 2K+1$ temporal samples~\cite{Synnevag2009a}, and to improve robustness to parameter errors we add \DIFdelbegin %DIFDELCMD < {\large%%%
\DIFdel{$\epsilon$}%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{$d$ }\DIFaddend percent diagonal loading~\cite{Cox1987,Maksym1979}. These steps are also needed to ensure that the covariance matrix is numerically well conditioned and hence invertible.

\begin{figure}[tbp]\DIFdelbeginFL %DIFDELCMD < \centering%%%
%DIF < 
%DIFDELCMD < \ifPeerReview%%%
\DIFdelendFL %
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.5\linewidth]{gfx/buske1_online.eps}%%%
%DIF < 
%DIFDELCMD < \else%%%
%DIF < 
%DIFDELCMD < \includegraphics[width=\linewidth]{gfx/buske1_online.eps}%%%
%DIF < 
%DIFDELCMD < \fi%%%
...
%DIFDELCMD < \ifPeerReview%%%
\DIFdelendFL %
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.5\linewidth]{gfx/buske2_online.png}%%%
%DIF < 
%DIFDELCMD < \else%%%
%DIF < 
%DIFDELCMD < \includegraphics[width=\linewidth]{gfx/buske2_online.png}%%%
%DIF < 
%DIFDELCMD < \fi%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{gfx/cross_tagged.png}\DIFaddendFL %
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth]{gfx/buske3_online.pdf}%%%
...
\begin{figure*}[t]%
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{gfx/buske4_online.pdf}%%%
...
\begin{figure*}[t]\centering
\DIFdelbeginFL %DIFDELCMD < \subfloat[$\beta$-values of the Kaiser window chosen for each image pixel. Note how LCA prefers a a narrow response ($\beta=0$) on the anchor, and on other strong sources with little lateral interference. On the sources where lateral interference is present it chooses the widest response with the best sidelobe suppression.]{\includegraphics[width=\textwidth]{gfx/buske5a_online.pdf}%%
%DIFDELCMD < \label{oversampling_mosaic_bounds_beta}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[$\beta$-values of the Kaiser window chosen for each image pixel. Note how LCA prefers a a narrow response ($\beta=0$) on the anchor, and on other strong sources with little lateral interference. On the sources where lateral interference is present it chooses the widest response with the best sidelobe suppression.]{\includegraphics[width=\textwidth]{gfx/oversampling_mosaic_bounds_lca_windows_beta.pdf}%%
\label{oversampling_mosaic_bounds_beta}}
\DIFaddendFL \hfil
\DIFdelbeginFL %DIFDELCMD < \subfloat[$\phi$-values of the Kaiser window chosen for each image pixel. On strong sources LCA selects windows that are steered away from the source. This is what improves the FWHM measurement in \Fig{oversampling_mosaic_bounds}.]{\includegraphics[width=\textwidth]{gfx/buske5b_online.pdf}%
%DIFDELCMD < \label{oversampling_mosaic_bounds_phi}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[$\phi$-values of the Kaiser window chosen for each image pixel. On strong sources LCA selects windows that are steered away from the source. This is what improves the FWHM measurement in \Fig{oversampling_mosaic_bounds}.]{\includegraphics[width=\textwidth]{gfx/oversampling_mosaic_bounds_lca_windows_phi.pdf}%
\label{oversampling_mosaic_bounds_phi}}
\DIFaddendFL \caption{Kaiser windows chosen for each pixel in the image. The underlying image is the one shown in \Fig{oversampling_mosaic_bounds}, with the same parameters. The location of the anchor, cut line and main scatter locations of the cross is marked in red.}
\label{fig_sim}
\end{figure*}

\begin{figure*}[t]%\centering%
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{gfx/buske6_online.pdf}%%%
...
\begin{figure*}[t]\centering
\DIFdelbeginFL %DIFDELCMD < \subfloat[\emph{Image quality of approximate point scatterers found in the resolution test cross scene}. Two lateral image cuts are shown in the leftmost figures, along with the response of a rectangularly weighted DAS. Note how the FWHM of LCA sits in between that of DAS and MVDR. Also note how LCA is insensitive to the window type; its performance is near identical whether it uses Kasier or trigonometric windows.]{\includegraphics[width=\textwidth]{gfx/buske7a_online.pdf}%
%DIFDELCMD < \label{beamformer_comparison_cross}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[\emph{Image quality of approximate point scatterers found in the cross scene}. Two lateral image cuts are shown in the leftmost figures, along with the response of a rectangularly weighted DAS. Note how the FWHM of LCA sits in between that of DAS and MVDR. Also note how LCA is insensitive to the window type; its performance is near identical whether it uses Kasier or trigonometric windows.]{\includegraphics[width=\textwidth]{gfx/beamformer_comparison_cross.pdf}%
\label{beamformer_comparison_cross}}
\DIFaddendFL \hfil
\DIFdelbeginFL %DIFDELCMD < \subfloat[\emph{Image quality of the full sector Holmengraa scene}. Compared to DAS the adaptive beamformers produce deeper shadows, sharper edges and a higher detail level. The adaptive methods produce nearly identical images.]{\includegraphics[width=\textwidth]{gfx/buske7b_online.pdf}%%
%DIFDELCMD < \label{beamformer_comparison_holmengraa}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[\emph{Image quality of the full sector Holmengraa scene}. Compared to DAS the adaptive beamformers produce deeper shadows, sharper edges and a higher detail level. The adaptive methods produce nearly identical images.]{\includegraphics[width=\textwidth]{gfx/beamformer_comparison_holmengraa.pdf}%%
\label{beamformer_comparison_holmengraa}}
\DIFaddendFL \caption{Comparing image quality of DAS, MVDR and LCA with Kaiser or trigonometric windows.}\label{image_quality}
\end{figure*}
...
In practice, we estimate the beamformer output power \DIFdelbegin \DIFdel{by computing a sample power average $s_z^2$:
}\DIFdelend \DIFaddbegin \DIFadd{as
}\DIFaddend %
\begin{align*}
E\big\{\big|z[n]\big|^2\big\} \DIFdelbegin \DIFdel{\approx s_z^2 }\DIFdelend = \frac{1}{N_k} \sumb{n'=n-K}{n+K} \big| z[n'] \big|^2
\end{align*}
%
which computes the sample power average over $N_k=2K+1$ temporal samples. This is the same temporal averaging method we use for MVDR. In our case the \DIFdelbegin \DIFdel{bandwidth of our }\DIFdelend matched filtered signal is a just few samples long, hence $K=1$ will be used for both MVDR and LCA throughout this work.

\DIFdelbegin \DIFdel{Since pixels are correlated laterally}\DIFdelend \DIFaddbegin \todo{Drop this paragraph altogether?}\DIFadd{If extra flexibility is desired}\DIFaddend , we can \DIFdelbegin \DIFdel{include }\DIFdelend \DIFaddbegin \DIFadd{also estimate the variance by }\DIFaddend a weighted combination of \DIFdelbegin \DIFdel{these as well to improve the variance estimation:
}\DIFdelend \DIFaddbegin \DIFadd{local pixels
}\DIFaddend %
\begin{align}\DIFdelbegin \DIFdel{
s_z}\DIFdelend \DIFaddbegin \DIFadd{
E}\big\{\big|\DIFadd{z}[\DIFadd{x,n}]\big|\DIFaddend ^2\DIFaddbegin \big\} \DIFaddend = \frac{1}{N_x N_k} \sumb{\nmat{\scriptstyle x'=\\[-2mm]\scriptstyle x-X}}{x+X} \sumb{\nmat{\scriptstyle n'=\\[-2mm]\scriptstyle n-K}}{n+K} \DIFdelbegin \DIFdel{\omega[x',n']}\DIFdelend \DIFaddbegin \DIFadd{w}[\DIFadd{x',n'}]\DIFaddend \big| z[x',n'] \big|^2
\end{align}
%
where $N_x = 2X+1$ is the number of azimuth lines to average over, and \DIFdelbegin \DIFdel{$\omega[x',n']$ }\DIFdelend \DIFaddbegin \DIFadd{$w_\text{prf}[x',n']$ }\DIFaddend is a normalized 2 dimensional weight function. 
\DIFdelbegin \DIFdel{However, we observed no visual improvement doing this, and the effect of micro-steered windows is hard to predict. Henceforth no lateral averaging will be applied.
}\DIFdelend % 
% \begin{itemize}
...
\end{align}
%
The Kaiser-Bessel window is near optimal in the sense of having its peak energy concentration around $\theta=0^\circ$, for a given \DIFdelbegin \DIFdel{space-bandwidth product related to the Kaiser }\DIFdelend \DIFaddbegin \DIFadd{time-bandwidth product specified with the }\DIFaddend parameter $\beta$\DIFdelbegin \DIFdel{as}\DIFdelend :
%
\begin{align}
\beta = \frac{TB}{2},
\end{align}
%
where $T$ \DIFdelbegin \DIFdel{in our case is the spatial }\DIFdelend \DIFaddbegin \DIFadd{is the }\DIFaddend extent of the window \DIFaddbegin \DIFadd{in time}\todo{Should perhaps say space-bandwidth product?} \DIFaddend and $B$ is its bandwidth. Adjusting $\beta$ changes the trade-off between mainlobe width and sidelobe level. When $\beta=0$ the window becomes rectangular, while at large values ($\beta>5$) the window converges to a Gaussian both in time and frequency. This class of windows is generally considered well suited for separating closely spaced\DIFaddbegin \todo{better word?} \DIFaddend sources with amplitudes of a high dynamic range~\cite{Harris1978}, they are easy to make, and they are optimal for any value of $\beta$.
...
To test the performance of the MVDR and LCA beamformers, we have processed data acquired by the Kongsberg Maritime HUGIN AUV carrying their 32 element HISAS1030 sonar~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Hansen2011}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Hansen2009}
}%DIFAUXCMD
}\DIFaddend . It is a high resolution synthetic aperture sonar with 1.2\;m array length, 100\;kHz operating frequency, \DIFdelbegin \DIFdel{30}\DIFdelend \DIFaddbegin \DIFadd{40}\DIFaddend \;kHz bandwidth and \DIFdelbegin \DIFdel{23}\DIFdelend \DIFaddbegin \DIFadd{25}\DIFaddend $^\circ$ \DIFdelbegin \DIFdel{receiver element -3}%DIFDELCMD < \,%%%
\DIFdel{dB opening angle }\DIFdelend \DIFaddbegin \DIFadd{opening angle on transmission}\DIFaddend .

Two different scenes will be studied. One of the 1500 DWT oil tanker wreck Holmengraa lying at a slanted seabed at 77\;m depth outside of Horten, Norway~\cite{holmengraa}. It measures 68\;m by 9\;m and fills most of the highlighted sector. This data was collected by the Norwegian Defense Research Establishment and Kongsberg Maritime. The other scene is of an 1\,m by 1\,m iron \DIFdelbegin \DIFdel{resolution test }\DIFdelend cross, connected to a an anchor with 13\,cm diameter (\Fig{cross}). This data was collected by the Bundeswehr Technical Center for Ships and Naval Weapons, Maritime Technology and Research (WTD 71). Both scenes are imaged in sectorscan mode.

In the image reconstruction we have run MVDR with subarray length $L=16$ and \DIFdelbegin \DIFdel{$\text{\large$\epsilon$}=1\%$ diagonal loading}\DIFdelend \DIFaddbegin \DIFadd{$d=1\%$ diagonal loading}\todo{also used for element distance}\DIFaddend . Both MVDR and LCA was run with $K=1$ temporal averaging. This is a fairly aggressive yet stable set of parameters\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{Synnevag2009a}
}%DIFAUXCMD
.}\DIFdelend \DIFaddbegin \DIFadd{.}\todo{suitable ref?}
\DIFaddend 
...
LCA \DIFdelbegin \DIFdel{seeks to improve image resolution and contrast by minimizing the power of }\DIFdelend \DIFaddbegin \DIFadd{iterates through a set of windows in search of the one that minimizes the }\DIFaddend noise and interference \DIFaddbegin \DIFadd{power }\DIFaddend in each image pixel. \DIFdelbegin \DIFdel{This is the same optimization criterion as MVDR uses, but instead of computing an optimal array window like MVDR does, it selects the best window out of a predefined set. Hence, it }\DIFdelend \DIFaddbegin \DIFadd{It }\DIFaddend can be viewed as either a reduced \DIFdelbegin \DIFdel{and discrete window }\DIFdelend \DIFaddbegin \DIFadd{solution }\DIFaddend space version of MVDR, or as an \DIFdelbegin \DIFdel{advanced multi-apodization technique. In this paper we have studied which windows we should ideally let LCA choose from. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We found the LCA to be surprisingly }\DIFdelend \DIFaddbegin \DIFadd{multi-taper extension to DAS. It creates images similar to MVDR, but at a fraction of the computational cost. It is also inherently robust and }\DIFaddend insensitive to the \DIFdelbegin \DIFdel{exact window function it is used with}\DIFdelend \DIFaddbegin \DIFadd{particular set of windows it selects from}\DIFaddend , supported by the observation that \DIFdelbegin \DIFdel{near identical images were produced whether we used }\DIFdelend \DIFaddbegin \DIFadd{LCA can be used with either Kaiser or trigonometric windows and produce near identical images.
}

\DIFadd{When using LCA with }\DIFaddend Kaiser \DIFdelbegin \DIFdel{windows or trigonometric ones. However, LCA must be able to choose between wide and narrow window responses, since the wide ones offer the best sidelobe suppression and the narrow ones offer the best sensitivity.
We achieved good results with 2-3 Kaiser windows in the range $\beta\in[0,5]$, and observed minimal difference when adding more windows or adjusting the upper bound of }\DIFdelend \DIFaddbegin \DIFadd{windows we made their responses similar to those typically chosen by a robust MVDR in key areas in the image. We suggest an upper bound for }\DIFaddend $\beta$ \DIFdelbegin \DIFdel{. For the steering angle we suggest a value in the range $\phi\in[\pm\delta\phi_\text{3dB}(\beta)]$, where $\phi_\text{3dB}(\beta)$ is the bandwidth of the respective window, and $\delta$ is a scaling parameter. Setting $\delta$ to either }\DIFdelend \DIFaddbegin \DIFadd{between 2 and 5, where 5 is slightly better at suppressing sidelobes. For $\phi$ we suggest to steer either to }\DIFaddend 50\% or 100\% \DIFdelbegin \DIFdel{reduces the lateral point target size to }\DIFdelend \DIFaddbegin \DIFadd{of the }\minus{}\DIFadd{3}\,\DIFadd{dB width, which leads to a resolution of }\DIFaddend 73\% or \DIFdelbegin \DIFdel{53\% compared to }\DIFdelend \DIFaddbegin \DIFadd{54\% of }\DIFaddend the rectangular window, respectively. \DIFdelbegin \DIFdel{Hence, this parameter can be used to control the aggressiveness and resolution gain of LCA}\DIFdelend \DIFaddbegin \DIFadd{As lower boundaries we set $\beta=0$ to always include a rectangular window, and $\phi=0$ to always include the centered window}\DIFaddend .

\DIFdelbegin \DIFdel{In summary, LCA is an attractive alternative to other adaptive beamformers due to being very fast, simple to understand, practically parameter-free, inherently robust, and able produce images similar to that of MVDR. Contrary to MVDR, it has a low computational complexity and can be easily and efficiently accelerated using e.g. GPUs. Since LCA performs just as well if we use trigonometric windows instead of Kaiser ones, we can also compute the optimal window parameter analytically for any given steering angle, and avoid the search over the window parameter entirely}\DIFdelend \DIFaddbegin \DIFadd{The tendency of the LCA to perform similarly to the MV beamformer seems to indicate that a full solution space is rarely needed in real world scenarios}\DIFaddend .

%DIF <  The tendency of LCA to produce images of similar quality as MVDR seems to indicate that a full solution space is rarely needed in real world scenarios.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <   It initially very is also very fast, and is very well suited for acceleration using  very fast, easy to implement even on parallel hardware,
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Its computational complexity is only of O($N_wM$), and its data parallel nature makes it very well suited for acceleration using e.g. GPUs. compared to MVDR's O($M^3$).
\DIFdelend % 
...
\DIFdelbegin %DIFDELCMD < \ifBuildBibliography
%DIFDELCMD <    %%%
...
%DIFDELCMD < %%%
\DIFdelend % Generated by IEEEtran.bst, version: 1.13 (2008/09/30)
...
\DIFaddbegin \bibitem{Dursun2009}
\DIFadd{S.~Dursun, A.~Austeng, R.~E. Hansen, and S.~Holm, ``}{\DIFadd{Minimum variance
  beamforming in active sonar imaging}}\DIFadd{,'' in }\emph{\DIFadd{Proceedings of the 3rd
  International Conference }\& \DIFadd{Exhibition on Underwater Acoustic Measurements:
  Technologies and Results}}\DIFadd{, B.~r.~e. }{\DIFadd{John S. Papadakis }\& \DIFadd{Leif}}\DIFadd{, Ed., 2009,
  pp. 1373--1378.
}

\DIFaddend \bibitem{Lo2004}
...
\DIFdelbegin %DIFDELCMD < \bibitem{Kim2014}
%DIFDELCMD < %%%
\DIFdel{K.~Kim, S.~Park, J.~Kim, S.-B. Park, and M.~Bae, ``}%DIFDELCMD < {%%%
\DIFdel{A fast minimum variance
  beamforming method using principal component analysis}%DIFDELCMD < }%%%
\DIFdel{,'' }\emph{\DIFdel{IEEE
  Transactions on Ultrasonics, Ferroelectrics, and Frequency Control}}%DIFAUXCMD
\DIFdel{, vol.~61,
  no.~6, pp. 930--945, Jun. 2014.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Asl2012}
%DIFDELCMD < %%%
\DIFdel{B.~M. Asl and A.~Mahloojifar, ``}%DIFDELCMD < {%%%
\DIFdel{A low-complexity adaptive beamformer for
  ultrasound imaging using structured covariance matrix}%DIFDELCMD < }%%%
\DIFdel{,'' }\emph{\DIFdel{IEEE
  Transactions on Ultrasonics, Ferroelectrics and Frequency Control}}%DIFAUXCMD
\DIFdel{, vol.~59,
...
%DIFDELCMD < \bibitem{Vignon2008}
%DIFDELCMD < %%%
\DIFdel{F.~Vignon and M.~R. Burcher, ``}%DIFDELCMD < {%%%
\DIFdel{Capon beamforming in medical ultrasound imaging
  with focused beams.}%DIFDELCMD < }%%%
\DIFdel{'' }\emph{\DIFdel{IEEE transactions on ultrasonics,
  ferroelectrics, and frequency control}}%DIFAUXCMD
\DIFdel{, vol.~55, no.~3, pp. 619--28, Mar.
  2008.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \bibitem{Synnevag2008}
...
\DIFdelbegin %DIFDELCMD < \bibitem{Stankwitz1995}
%DIFDELCMD < %%%
\DIFdel{H.~Stankwitz, R.~Dallaire, and J.~Fienup, ``}%DIFDELCMD < {%%%
\DIFdel{Nonlinear apodization for sidelobe
  control in SAR imagery}%DIFDELCMD < }%%%
\DIFdel{,'' }\emph{\DIFdel{IEEE Transactions on Aerospace and
  Electronic Systems}}%DIFAUXCMD
\DIFdel{, vol.~31, no.~1, pp. 267--279, Jan. 1995.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Buskenes2011}
%DIFDELCMD < %%%
\DIFdel{J.~I. Buskenes, C.-I.~C. Nilsen, and A.~Austeng, ``}%DIFDELCMD < {%%%
\DIFdel{A Low Complexity Adaptive
  Beamformer for Active Sonar Imaging}%DIFDELCMD < }%%%
\DIFdel{,'' in }\emph{\DIFdel{Proceedings of the
  Underwater Acoustic Measurements: Technologies }%DIFDELCMD < \& %%%
\DIFdel{Results}}%DIFAUXCMD
\DIFdel{.}%DIFDELCMD < \hskip %%%
\DIFdel{1em plus
  0.5em minus 0.4em}%DIFDELCMD < \relax %%%
\DIFdel{Underwater Acoustic Measurements: Technologies }%DIFDELCMD < \&
%DIFDELCMD <   %%%
\DIFdel{Results, 2011.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \bibitem{Harris1978}
...
\DIFdelbegin %DIFDELCMD < \bibitem{Hansen2011}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem{Hansen2009}
\DIFaddend R.~E. Hansen, H.~J. Callow, T.~O. \DIFdelbegin \DIFdel{Sabo, and }\DIFdelend \DIFaddbegin \DIFadd{S\ae~b\o, }\DIFaddend S.~A. \DIFdelbegin \DIFdel{~V. Synnes, }\DIFdelend \DIFaddbegin \DIFadd{Synnes, P.~E. Hagen,
  G.~Fossum, and B.~Langli, }\DIFaddend ``{\DIFdelbegin \DIFdel{Challenges in
  Seafloor Imaging and Mapping With Synthetic Aperture Sonar}\DIFdelend \DIFaddbegin \DIFadd{Synthetic aperture sonar in challenging
  environments: Results from the HISAS 1030.}\DIFaddend }\DIFdelbegin \DIFdel{,'' }\emph{\DIFdel{IEEE
  Transactions on Geoscience and Remote Sensing}}%DIFAUXCMD
\DIFdel{, vol.~49,
  no.~10, pp.3677--3687, Oct. 2011.
}\DIFdelend \DIFaddbegin \DIFadd{'' in }\emph{\DIFadd{Proceedings of the 3rd
  International Conference }\& \DIFadd{Exhibition on Underwater Acoustic Measurements:
  Technologies and Results}}\DIFadd{, 2009.
}\DIFaddend 

\bibitem{holmengraa}
{Article of Holmengraa on dykkepedia.com}.

\DIFdelbegin %DIFDELCMD < \bibitem{Stoica2005}
%DIFDELCMD < %%%
\DIFdel{P.~Stoica and R.~L. Moses, ``}%DIFDELCMD < {%%%
\DIFdel{Nonparametric Methods}%DIFDELCMD < }%%%
\DIFdel{,'' in }\emph{\DIFdel{Spectral
  Analysis of Signals}}%DIFAUXCMD
\DIFdel{.}%DIFDELCMD < \hskip %%%
\DIFdel{1em plus 0.5em minus 0.4em}%DIFDELCMD < \relax %%%
\DIFdel{Prentice Hall,
  2005, ch.~2, pp. 23--89.
}%DIFDELCMD < 
