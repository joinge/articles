...
\ifPhdDoc
   ...
   \includegraphics[width=\linewidth]{gfx/fig1.eps}%
\else
   \begin{figure}[tbp]\centering%
   \ifPeerReview%
...
\ifPhdDoc
   \begin{figure}[tbp]\centering%
   \includegraphics[width=\linewidth]{gfx/fig2.png}%
\else
   \begin{figure}[tbp]\centering%
   \ifPeerReview%
...
\ifPhdDoc
   \begin{figure*}[tbp]\centering%
   \includegraphics[width=\linewidth]{gfx/fig3.pdf}%
\else
...
\ifPhdDoc
   \begin{figure*}[t]\centering
   \subfloat[$\beta$-values of the Kaiser window chosen for each image pixel. Note how LCA prefers a narrow response ($\beta=0$) on the anchor, and on other strong sources with little lateral interference. On the sources where lateral interference is present it chooses the widest response with the best sidelobe suppression.]{\includegraphics[width=\textwidth]{gfx/fig5a.pdf}%%
   \label{oversampling_mosaic_bounds_beta}}
   \hfil
   \subfloat[$\phi$-values of the Kaiser window chosen for each image pixel. On strong sources LCA selects windows that are steered away from the source. This is what improves the FWHM measurement in \Fig{oversampling_mosaic_bounds}.]{\includegraphics[width=\textwidth]{gfx/fig5b.pdf}%
...
\ifPhdDoc
   \begin{figure*}[tbp]\centering%
   \includegraphics[width=\linewidth]{gfx/fig6.pdf}%
\else
   \begin{figure*}[t]%\centering%
...
\subsection[Kaiser parameter β and steering Φ]{Kaiser parameter $\beta$ and steering $\phi$}\label{sec:results_window_parameters}
...
\typeout{hohoho}
   ...
   \input{bio/roy_edgar_hansen_new}
