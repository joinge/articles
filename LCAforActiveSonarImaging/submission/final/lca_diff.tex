...
%DIF DEL lca_submit2.tex   Sun May  1 20:46:48 2016
%DIF ADD lca.tex           Sun May  1 20:42:55 2016
%%%%%%%%%%%%%%%%                                ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% CONDITIONALS %
%%%%%%%%%%%%%%%%                                ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Synnev\aa{}g \emph{et al}. suggested using a window set comprised of rectangular, Kaiser (or Kaiser-Harris) and inverted Kaiser functions. In total he \DIFdelbegin \DIFdel{made }\DIFdelend \DIFaddbegin \DIFadd{let }\DIFaddend LCA choose from 12 unique windows, 6 of which were phase-shifted versions of a fairly wide Kaiser window, and one were rectangular. \DIFdelbegin \DIFdel{Phase-shifting the windows has the effect of slighly adjusting the angle the array is steered towards. }\DIFdelend Synnev\aa{}g's composition of windows was motivated by the desire to use wide windows with maximum sidelobe suppression near the receiver, and narrow windows at greater depths for maximum sensitivity and depth penetration. While \DIFdelbegin \DIFdel{achieving }\DIFdelend \DIFaddbegin \DIFadd{he achieved }\DIFaddend good results, it was not made clear how many windows LCA needs, or how they ideally should be steered. Also, it is not apparent how LCA performs in sonar systems where the transmit beam is unfocused.

In this study we expand our previous work of applying LCA to active sonar imaging~\cite{Buskenes2011}. We use variations of the parametric Kaiser window, but also compare it to the trigonometric window function. We extend Synnev\aa{}g's work by steering all our windows and not just some, and demonstrate how LCA's performance is affected by the type and number of windows included. Our results are obtained using experimental data from the 32 element Kongsberg Maritime HISAS1030 sonar, which operates at 100\,kHz with an \minus{}3\,dB element opening angle of 23$^\circ$. Although this system was designed as \DIFaddbegin \DIFadd{a }\DIFaddend synthetic aperture sonar (SAS) where an image is synthesized from several pings of data, we will only create images out of single pings (sectorscan imaging) in this study.

Our results show that when MVDR is \DIFdelbegin \DIFdel{modified }\DIFdelend \DIFaddbegin \DIFadd{robustified }\DIFaddend to work in active sonar imaging, it computes weights with spatial \DIFaddbegin \DIFadd{amplitude }\DIFaddend responses that tend to be symmetric and steered within a fraction of \DIFdelbegin \DIFdel{the window's }\DIFdelend \DIFaddbegin \DIFadd{their }\DIFaddend \minus{}3\,dB \DIFdelbegin \DIFdel{resolution}\DIFdelend \DIFaddbegin \DIFadd{widths}\DIFaddend . When letting LCA choose from Kaiser windows with similar responses, we obtain images with improved noise suppression and resolution. The resolution gain is made predictable by limiting the maximum steering to a fraction of the \DIFdelbegin \DIFdel{window's }\DIFdelend \minus{}3\,dB width \DIFaddbegin \DIFadd{of each window's spatial amplitude response}\DIFaddend . Overall, LCA produces images that are similar to MVDR, but with a resolution in between that of DAS and MVDR. LCA is also inherently robust, easy to implement, and fairly easy to understand.
...
MVDR seeks to minimize the \DIFdelbegin \DIFdel{output variance }\DIFdelend \DIFaddbegin \DIFadd{power of noise and interference in the output }\DIFaddend of the beamformer, under the constraint of \DIFdelbegin \DIFdel{a }\DIFdelend unity gain in some desired direction $\phi$~\cite{Capon1969}. With the assumption of zero mean data this \DIFdelbegin \DIFdel{can be formulated as}\DIFdelend \DIFaddbegin \DIFadd{takes the form of minimizing output variance}\DIFaddend : 
%
\begin{align}
...
\end{align}
%
Here $\a_\phi$ is a steering vector \DIFaddbegin \DIFadd{for an azimuthal steering angle $\phi$}\DIFaddend , $E\{\cdot\}$ is the expectation operator, and $\R[n] = E\{\x[n]\x\H[n]\}$ is the spatial covariance matrix for the full array. This is a convex optimization problem with the \DIFaddbegin \DIFadd{unique }\DIFaddend solution:
%
\begin{gather}
\vec w[n] = \frac{\Ri[n]\a_\phi}{\a_\phi\T\Ri[n]\a_\phi}.\label{eq:mvdr_weights}
\end{gather}
The problem lies in estimating and inverting \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend spatial covariance matrix. We have described the exact steps in~\cite{Buskenes2014}: To avoid signal cancellation we apply spatial averaging by computing a mean covariance matrix from a set of subarrays with length $L$~\cite{Kailath1985}, for true speckle statistics we perform temporal averaging over $N_k = 2K+1$ temporal samples~\cite{Synnevag2009}, and to improve robustness to parameter errors we add {\large$\epsilon$} percent \DIFdelbegin \DIFdel{diagonal loading}\DIFdelend \DIFaddbegin \DIFadd{of the total output power to the matrix's diagonal}\DIFaddend ~\cite{Cox1987,Maksym1979}. These steps are also needed to ensure that the covariance matrix is numerically well conditioned and hence invertible.
...
\fi%
\caption{Each Kaiser window is steered in the interval \DIFdelbeginFL \DIFdelFL{$\phi=[0, \phi_{\mathrm{3\,dB}}(\beta)]$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\phi\in[0, \phi_{\mathrm{3\,dB}}(\beta)]$}\DIFaddendFL . The angle $\phi_{\mathrm{3\,dB}}(\beta)$ is the amount of steering needed for the steered \DIFaddbeginFL \DIFaddFL{amplitude }\DIFaddendFL response to have a \protect\minus{}3\,dB crossing that is exactly half that of the unsteered window. With each window steered this way we expect the resolution gain to remain predictable and independent of $\beta$, and we also effectively constrain the white noise gain of the beamformer.}\label{windows_steering}
\end{figure}
...
\fi%
\caption{\DIFdelbeginFL \DIFdelFL{One of the objects }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The submerged object }\DIFaddendFL used to test \DIFdelbeginFL \DIFdelFL{the beamformers }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{beamforming resolution }\DIFaddendFL is a 1\,m by 1\,m \DIFdelbeginFL \DIFdelFL{resolution }\DIFdelendFL test cross attached to an anchor with a diameter of approximately 13\,cm. Source image curtesy of Bundeswehr Technical Center for Ships and Naval Weapons, Maritime Technology and Research (WTD 71).}\label{cross}
\end{figure}
...
% \emph{Left:}\hfill
% \DIFdelbeginFL \parbox[t]{.95\linewidth}{\DIFdelFL{MVDR sectorscan image of the oiltanker Holmengraa, with the pixel groups are indicated with red boxes.}}%DIFAUXCMD
% \DIFdelendFL \DIFaddbeginFL \parbox[t]{.95\linewidth}{\DIFaddFL{MVDR sectorscan image of the oiltanker Holmengraa, with 40x40 pixel groups for the shadow, highlight and speckle region of the image indicated with red boxes.}}\DIFaddendFL \protect\\\hspace{\textwidth}
...
\emph{Left:}\hfill
\parbox[t]{.95\linewidth}{MVDR sectorscan image of the oiltanker Holmengraa, with \DIFdelFL{the}\DIFaddFL{40x40} pixel groups \DIFdelFL{are}\DIFaddFL{for the shadow, highlight and speckle region of the image} indicated with red boxes.}%DIFAUXCMD
\protect\\\hspace{\textwidth}
\emph{Right:}\hfill
\parbox[t]{.95\linewidth}{MVDR window amplitude and phase responses computed from 40x40 pixel groups.\DIFdelFL{ for the shadow, highlight and speckle region of the image}. The responses are overlayed each other and the amount of overlap is colored using a logarithmic scale. The size of the pixel groups were chosen ad-hoc for the histograms to be \DIFdelFL{near}\DIFaddFL{visually} invariant to a shift in position within the same region. Since each pixel is pre-delayed into focus the unsteered responses all have their center at broadside. The dashed red lines at -13dB and -43dB marks the peak sidelobe levels of an unsteered rectangular and Hamming window, respectively. Note how the responses are more or less symmetric, with very little steering in the shadow, moderate steering in speckle and steering within roughly 3dB in highlight. The phase varies most in the highlight region where we see the highest contrast.} %DIFAUXCMD
 }\label{mvdr_selected_windows}
\end{figure*}
...
\caption{Kaiser windows chosen for each pixel in the image \DIFaddbeginFL \DIFaddFL{of the resolution test cross}\DIFaddendFL . The underlying image is the one shown in \Fig{oversampling_mosaic_bounds}, with the same parameters. The location of the anchor, cut line and main scatter locations of the cross is marked in red.}
\label{fig_sim}
\end{figure*}

\begin{figure*}[t]%\centering%
\includegraphics[width=\textwidth]{gfx/buske6_online.pdf}%
\caption{\emph{LCA images using different window database sizes}. The window boundaries are the same. The upper left image is identical to a rectangularly weighted DAS. Observe that \DIFaddbeginFL \DIFaddFL{finer }\DIFaddendFL sampling \DIFaddbeginFL \DIFaddFL{of }\DIFaddendFL the $\beta$-range \DIFdelbeginFL \DIFdelFL{more finely increases the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{improves }\DIFaddendFL sidelobe suppression but leaves resolution unchanged. Adding more steering-variations improves both sidelobe suppression and resolution. However, using more than  $N_\beta=2$ window types and $N_\phi=3$ steering angles makes minimal difference.}\label{oversampling_mosaic}
\end{figure*}
...
%
Note how closely LCA is related to the MVDR definition in \eq{eq:mvdr_definition}. The optimization criterion and constraint is the same, but LCA has a finite and discrete solution space for the weights. As will be demonstrated in \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{sec:results_lca_window_function}, LCA \DIFdelbegin \DIFdel{will perform }\DIFdelend \DIFaddbegin \DIFadd{performs }\DIFaddend similarly to MVDR because a robustified MVDR implementation seems to be constrained to window functions similar to the ones we let LCA choose from.
...
While we focus on using LCA with \DIFdelbegin \DIFdel{Kaiser windows}\DIFdelend \DIFaddbegin \DIFadd{the Kaiser window function}\DIFaddend , we will for \DIFdelbegin \DIFdel{completeness compare the performance with that of using trigonometric windows. This isdefined as}\DIFdelend \DIFaddbegin \DIFadd{comparison also test it with the trigonometric window function. Its definition is}\DIFaddend :
%
\begin{align}
f_m(\alpha) = \alpha - (1-\alpha)\cos\Big(\frac{2\pi m}{M-1}\Big)\DIFaddbegin \DIFadd{,}\DIFaddend \label{eq:trig_window_function}
\end{align}
%
where we apply the \DIFdelbegin \DIFdel{contraint }\DIFdelend \DIFaddbegin \DIFadd{constraint }\DIFaddend $\alpha\in[0.5,1]$ to avoid windows with negative coefficients. Steering is applied as in (\ref{eq:steered_window}), and the $\alpha$-value that halves the \minus{}3\,dB distance is found using the method described in Appendix \ref{appendix:steering_angles}. 
...
To determine a suitable range for the Kaiser parameter $\beta$ and \DIFaddbegin \DIFadd{phase }\DIFaddend steering $\phi$, we constructed large and equally sized window databases containing Kaiser windows with varied upper boundaries for $\beta$ and $\phi$. The lower boundaries were chosen as $\beta=0$ and $\phi=0^\circ$, which includes the rectangularly weighted DAS image. The resulting images are shown in \Fig{oversampling_mosaic_bounds}. In each image we computed the lateral distance $\Delta x$ between the \minus{}3\,dB points of the anchor, with the reference $\Delta x_\text{ref}$ being the rectangularly weighted DAS image. A fourth order polynomial fit was used in this computation. \DIFdelbegin \DIFdel{This measure }\DIFdelend \DIFaddbegin \DIFadd{The measure $\Delta x$ }\DIFaddend is also commonly called the full width half maximum (FWHM). In the case of a imaging a point scatter, it is closely related to the resolution of \DIFdelbegin \DIFdel{of }\DIFdelend the system. For a rectangular window, for instance, the system resolution is approximately \DIFdelbegin \DIFdel{$\Delta \approx \text{FWHM}/0.89$}\DIFdelend \DIFaddbegin \DIFadd{$\delta x \approx \Delta x/0.89$}\DIFaddend ~\cite{Harris1978}.

The LCA images in \Fig{oversampling_mosaic_bounds} demonstrate the effect of adjusting the upper bound of $\beta$ and $\phi$. As suggested by \Fig{mvdr_selected_windows} we first attempted to use $\beta\in[0,2]$. While this significantly improved sidelobe suppression, a slight further improvement was observed up to $\beta\in[0,5]$. This can likely be explained by the need to compensate for the increased sidelobe levels caused by steering. No noticeable difference were observed for higher values than $\beta=5$. The images also demonstrate that increasing the upper bound for steering $\phi$ improves the resolution of the strong scatterers. For this sonar the Rayleigh resolution is \DIFdelbegin \DIFdel{$\Delta\approx0.0125$ }\DIFdelend \DIFaddbegin \DIFadd{$\delta x\approx0.0125$ }\DIFaddend radians (0.72$^\circ$), so a point scatterer imaged at 19\,m range with a rectangular DAS would have a lateral \DIFdelbegin \DIFdel{$\text{FWHM}\approx0.125\cdot19\,\text{m}\cdot0.89=21\,\text{cm}$}\DIFdelend \DIFaddbegin \DIFadd{$\Delta x\approx0.0125\cdot19\,\text{m}\cdot0.89=21\,\text{cm}$}\DIFaddend . This is close to the FWHM of the anchor in the DAS image in \Fig{oversampling_mosaic_bounds}, which means that the acoustic fingerprint of the anchor is similar to a point source. This \DIFdelbegin \DIFdel{suggests }\DIFdelend \DIFaddbegin \DIFadd{further implies that }\DIFaddend the anchor reflections \DIFdelbegin \DIFdel{to be }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend specular and that we view its rounded side. As we increase steering to either 50\% or 100\% of the \minus{}3\,dB width, the FWHM drops to 73\% or 53\% to that of the rectangular window, respectively.

\Fig{oversampling_mosaic_bounds_beta} and \Fig{oversampling_mosaic_bounds_phi} illustrates which $\beta$ and $\phi$ values LCA prefers for different regions in the image of the resolution test cross. Red marker lines are used to pinpoint the cut line, location of the anchor and strong scatters on the cross. Observe that in the anchor region only narrow responses ($\beta=0$) are used, which is the reason for the anchor FWHM being measured to the same value regardless of the upper bound of $\beta$ in \Fig{oversampling_mosaic_bounds}. On other scatterers with strong nearby lateral interference LCA prefers wider windows to suppress the interference. When allowed LCA prefers to steer windows away from the sources, but \DIFdelbegin \DIFdel{never }\DIFdelend \DIFaddbegin \DIFadd{we never let it }\DIFaddend exceed the $\phi_\mathrm{3\,dB}(\beta)$ limit. When exceeding this bound we observed oscillation artifacts in the image.


\subsection{Window database size}\label{sec:results_database_size}

So far we have found it reasonable to use Kaiser windows in the range $\beta\in[0,5]$, each steered within the range $|\phi|\in[0,\phi_\mathrm{3\,dB}(\beta)]$. What remains is to determine the number of windows needed. We study this in \Fig{oversampling_mosaic}, where we \DIFdelbegin \DIFdel{compared }\DIFdelend \DIFaddbegin \DIFadd{compare }\DIFaddend LCA images made from windows databases of different sizes, but with the same parameter boundaries. \DIFdelbegin \DIFdel{We included }\DIFdelend \DIFaddbegin \DIFadd{Included are }\DIFaddend the results from using a single window ($N_\beta=1$) and steering angle ($N_\phi=1$), which corresponds to an unsteered rectangularly weighted DAS.

Observe that \DIFdelbegin \DIFdel{sampling }\DIFdelend \DIFaddbegin \DIFadd{a finer sampling of }\DIFaddend the $\beta$-range \DIFdelbegin \DIFdel{more finely increases the }\DIFdelend \DIFaddbegin \DIFadd{improves }\DIFaddend sidelobe suppression but leaves resolution unchanged. Adding more steering-variations improves both sidelobe suppression and resolution. We \DIFdelbegin \DIFdel{can observe a }\DIFdelend \DIFaddbegin \DIFadd{observe }\DIFaddend major improvement going to $N_\beta=2$ and $N_\phi=3$, but minimal improvement by adding yet more windows.


\subsection{Image quality}\label{sec:results_image_quality}

We compare LCA image quality to that of DAS and MVDR in \Fig{image_quality}. In \Fig{beamformer_comparison_cross} we display the images computed by LCA using Kaiser windows, LCA using trigonometric windows, and MVDR. Two lateral image cuts through all images are presented in the left plot. The images and their corresponding cuts are nearly identical for the LCA version with Kaiser windows and the one with trigonometric windows. Compared to DAS the LCA produces \DIFdelbegin \DIFdel{targets }\DIFdelend \DIFaddbegin \DIFadd{images }\DIFaddend with an FWHM that lies in between that of DAS and MVDR.
...
The computational complexity of MVDR is generally considered to be of O($M^3$). However, the implementation we are using is optimized so that building the spatial covariance matrix is of O($N_kN_L,L^2$), and inverting it is of O($L^3$). We describe this implementation in \cite{Buskenes2014}, and a beamspace version in \cite{Asen2013}, where we \DIFdelbegin \DIFdel{optimize if for }\DIFdelend \DIFaddbegin \DIFadd{implement it on }\DIFaddend a graphics processing unit (GPU) for nearly two orders of magnitude speed increase compared to a straightforward C implementation. However, MVDR is not ideal for GPUs due to the complex data dependencies present in the covariance computation, and at best we only managed to utilize 10\% of the GPUs \DIFaddbegin \DIFadd{maximum theoretical }\DIFaddend potential.

In comparison, LCA is of O($M N_\beta N_\phi$), and extremely well suited for GPUs. Each pixel depends on only a small subset of data, and the windows can be precomputed and stored in GPU cache for near immediate access. \DIFdelbegin \DIFdel{If we use a }\DIFdelend \DIFaddbegin \DIFadd{Furthermore, if we use the }\DIFaddend trigonometric window function we \DIFaddbegin \DIFadd{can }\DIFaddend solve for the window parameter analytically as described in \DIFdelbegin \DIFdel{appendix }\DIFdelend \DIFaddbegin \DIFadd{Appendix }\DIFaddend \ref{sec:reducing_lca_complexity}. This reduces the complexity to O($M N_\phi$), \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{because }\DIFaddend the only remaining LCA parameter \DIFdelbegin \DIFdel{is the steering amount}\DIFdelend \DIFaddbegin \DIFadd{to create windows for will be the steering angle}\DIFaddend .
...
received the B.Sc. degree in electrical engineering from Gj\o{}vik College University, Norway, in 2007, and the M.Sc. degree in instrumentation for particle physics from the University of Oslo, Norway, in 2010. He is currently pursuing the Ph.D. degree in \DIFaddbegin \DIFadd{acoustic }\DIFaddend image reconstruction and \DIFdelbegin \DIFdel{technology }\DIFdelend \DIFaddbegin \DIFadd{high performance computing }\DIFaddend at the University of Oslo.

His industry experience includes \DIFaddbegin \DIFadd{development of digital electronics at }\DIFaddend the European Organization for Nuclear Research (CERN), Geneva, Switzerland (2007-2008)\DIFdelbegin \DIFdel{, and the Norwegian Defence Research Establishment, Kjeller, Norway (2009)}\DIFdelend . He has lectured in digital signal processing at the Gj\o{}vik College University (2009), and at the University of Oslo (2010-2013). \DIFaddbegin \DIFadd{Current affiliation is with The Norwegian Defence Research Establishment, Kjeller, Norway, for which he is developing radar systems (2015-), and formerly sonar systems (2009, 2013).
}

\DIFaddend His research interests include \DIFdelbegin \DIFdel{adaptive beamforming, digital }\DIFdelend \DIFaddbegin \DIFadd{radar and sonar technology, aptive }\DIFaddend image reconstruction, high performance computing, intelligent detector design and open source software.
\end{IEEEbiography}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The PhD thesis is named Measurements in the Mixed Layer by a Bistatic Multi-CW Doppler Sonar. 
}\DIFaddend From 1992 to 2000, he was with the Norwegian research company TRIAD, working on multistatic sonar, multistatic radar, synthetic aperture radar (SAR), and underwater communications. Since 2000, he has been with the Norwegian Defence Research Establishment (FFI), Kjeller, Norway. 
\DIFdelbegin \DIFdel{He is currently Principal Scientist and team leader }\DIFdelend \DIFaddbegin \DIFadd{In the period 2009-2015, he was research manager }\DIFaddend for the autonomous underwater vehicle development \DIFdelbegin \DIFdel{and the }\DIFdelend \DIFaddbegin \DIFadd{at FFI, including the fields underwater navigation, battery and fuel-cell technologies, decisional autonomy, and }\DIFaddend synthetic aperture sonar\DIFdelbegin \DIFdel{development }\DIFdelend \DIFaddbegin \DIFadd{. 
He is currently principal scientist }\DIFaddend at FFI. 
He is also \DIFdelbegin \DIFdel{Adjunct Associated Professor at the University of Oslo, }\DIFdelend \DIFaddbegin \DIFadd{adjunct associated professor at Department of informatics at University in }\DIFaddend Oslo, Norway\DIFaddbegin \DIFadd{.
His research interests include synthetic aperture sonar and radar, ultrasound imaging, and array signal processing}\DIFaddend .
\end{IEEEbiography}
