...
%DIF DEL lca_rev1.tex   Mon Oct 12 11:48:37 2015
%DIF ADD lca.tex        Fri Oct 16 11:14:14 2015
%DIF < 
%DIF -------
\synctex=1 %DIF > 
%DIF -------
...
\newcommand\mediaPath{http://folk.uio.no/joibu/media/2015_JOE_LCA} %DIF > 
...
}\DIFaddend . However, while the computational complexity of conventional beamformers are linear with the number of channels, O($M$), MVDR is at O($M^3$). This \DIFdelbegin \DIFdel{makes MVDR impracticable and sometimes infeasible. We have shown in previous studies that MVDR's }\DIFdelend \DIFaddbegin \DIFadd{is because MVDR relies on estimating and inverting a covariance matrix. In sonar systems with less than 32 channels the }\DIFaddend complexity can be reduced \DIFdelbegin \DIFdel{, and its }\DIFdelend \DIFaddbegin \DIFadd{and the }\DIFaddend implementation accelerated significantly using graphics computing units (GPUs)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Buskenes2014,Asen2013}
}%DIFAUXCMD
. However, while MVDR relies on estimating and inverting a spatial covariance matrix , there exist a method that avoid this altogether. }%DIFDELCMD < \todo{samme avsnitt?}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Buskenes2014}
}%DIFAUXCMD
. For larger systems the inversion step dominates the computation. This can be dealt with by approximating the full covariance matrix with a small one using space reduction techniques such as beamspace processing~\mbox{%DIFAUXCMD
\cite{Asen2013}
}%DIFAUXCMD
, or the closely related principal component analysis (PCA) method~\mbox{%DIFAUXCMD
\cite{Kim2014}
}%DIFAUXCMD
. Another method is to assume spatial stationarity to form more easily invertible Toepliz matrices~\mbox{%DIFAUXCMD
\cite{Asl2012}
}%DIFAUXCMD
. However, at best the mentioned methods are able to reduce the complexity to O($M^2$).
}\DIFaddend 

\DIFdelbegin \DIFdel{This method is called }\DIFdelend \DIFaddbegin \DIFadd{A much faster and simpler alternative to MVDR is }\DIFaddend the Low Complexity Adaptive (LCA) beamformer. \DIFdelbegin \DIFdel{It was first proposed }\DIFdelend \DIFaddbegin \DIFadd{Based on an idea by Vignon~\mbox{%DIFAUXCMD
\cite{Vignon2008}
}%DIFAUXCMD
, it was first introduced }\DIFaddend by Synnev\aa{}g \DIFaddbegin \emph{\DIFadd{et al}}\DIFadd{. }\DIFaddend in clinical medical imaging~\cite{Synnevag2008}, who demonstrated its ability to obtain very similar \DIFdelbegin \DIFdel{results }\DIFdelend \DIFaddbegin \DIFadd{image quality }\DIFaddend to MVDR in \DIFdelbegin \DIFdel{a system }\DIFdelend \DIFaddbegin \DIFadd{systems }\DIFaddend with focused transmit beams. LCA applies a set of predefined windows\DIFaddbegin \DIFadd{, or tapers, }\DIFaddend and selects the one offering the best \DIFdelbegin \DIFdel{noise }\DIFdelend \DIFaddbegin \DIFadd{sidelobe }\DIFaddend suppression. It relies on the same optimization criterion as MVDR, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{thus }\DIFaddend may be considered a version of MVDR with a reduced window solution space. Alternatively, it may be viewed as a \DIFdelbegin \DIFdel{delay-and-sum (DAS) beamformer with an adaptive extension}\DIFdelend \DIFaddbegin \DIFadd{multi-taper technique. One such method is described by Stankwitz }\emph{\DIFadd{et al}}\DIFadd{. in radar imaging~\mbox{%DIFAUXCMD
\cite{Stankwitz1995}
}%DIFAUXCMD
, where the best out of 2 or 3 windows are selected. However, LCA is different in that it typically selects from more windows and allow micro-steered variations of them. This gives LCA an adaptive freedom that is somewhere in between that of traditional multi-taper techniques and MVDR}\DIFaddend .

Synnev\aa{}g \DIFdelbegin \DIFdel{used }\DIFdelend \DIFaddbegin \emph{\DIFadd{et al}}\DIFadd{. suggested using }\DIFaddend a window set comprised of rectangular, Kaiser \DIFaddbegin \DIFadd{(or Kaiser-Harris) }\DIFaddend and inverted Kaiser functions\DIFdelbegin \footnote{\DIFdel{May also be called Kaiser-Harris windows.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{. He }\DIFdelend \DIFaddbegin \DIFadd{. In total he }\DIFaddend made LCA choose from 12 unique windows, 6 of which were micro-steered variants of a fairly wide Kaiser window\DIFaddbegin \DIFadd{, and with the rectangular window included. This was motivated by the desire to use wide windows with maximum sidelobe suppression near the receiver, and narrow windows at greater depths for maximum sensitivity and depth penetration}\DIFaddend . While achieving good results, it was not made clear how \DIFdelbegin \DIFdel{LCA performs with more or fewer windows , and how steering affects it all}\DIFdelend \DIFaddbegin \DIFadd{how many windows LCA needs, or how they ideally should be steered. Also, it is not apparent how LCA performs in sonar systems where the transmit beam is unfocused}\DIFaddend .

\DIFdelbegin \DIFdel{We use LCA with }\DIFdelend \DIFaddbegin \DIFadd{In this study apply LCA to active sonar imaging. We use }\DIFaddend variations of the \DIFaddbegin \DIFadd{parametric }\DIFaddend Kaiser window, but \DIFdelbegin \DIFdel{a comparison to using trigonometric windows will also be provided}\DIFdelend \DIFaddbegin \DIFadd{also compare it to the trigonometric window function}\DIFaddend . We extend Synnev\aa{}g's work by steering all our windows and not just some, and \DIFdelbegin \DIFdel{elaborate on the type of Kaiser windows to include and the method of steering. Then the method is applied to }\DIFdelend \DIFaddbegin \DIFadd{demonstrate how LCA's performance is affected by type and number of windows included. Our results are obtained using }\DIFaddend experimental data from the 32 element Kongsberg Maritime HISAS 1030 synthetic aperture sonar (SAS), \DIFdelbegin \DIFdel{with an operating frequency of }\DIFdelend \DIFaddbegin \DIFadd{which operates at }\DIFaddend 100\,kHz \DIFdelbegin \DIFdel{and transmit angle of 25}\DIFdelend \DIFaddbegin \DIFadd{with an element beamwidth of 23}\DIFaddend $^\circ$. 
...
MVDR seeks to minimize the output \DIFdelbegin \DIFdel{power }\DIFdelend \DIFaddbegin \DIFadd{variance }\DIFaddend of the beamformer, under the constraint of a unity gain in some desired direction $\phi$~\cite{Capon1969}\DIFaddbegin \DIFadd{. With the assumption of zero mean data this can be formulated as}\DIFaddend : 
%
\begin{align}
\underset{\w[n]}{\argmin}\, E\{\big|z[n]\big|^2\} &= \underset{\w[n]}{\argmin}\, \w[n]\R[n]\w\H[n]\nn
\text{subject to } \DIFdelbegin %DIFDELCMD < \w[n]%%%
\DIFdelend \DIFaddbegin \w\DIFadd{^H[n]}\DIFaddend \a_\phi &= 1.\label{eq:mvdr_definition}
\end{align}
%
Here $\a_\phi$ is a steering vector, $E\{\cdot\}$ is the expectation operator, and \DIFdelbegin \DIFdel{$\R[n] = E\{\x\x\H\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\R[n] = E\{\x[n]\x\H[n]\}$ }\DIFaddend is the spatial covariance matrix \DIFaddbegin \DIFadd{for the full array}\DIFaddend . This is a convex optimization problem with the solution:
%
\begin{gather}
...
\caption{\emph{Determining Kaiser parameter boundaries}: LCA images \DIFaddbeginFL \DIFaddFL{of the cross and anchor }\DIFaddendFL created with a large and equal size window database, but where the upper bounds for $\beta$ and $\phi$ is varied. The upper left image is equal to DAS with a rectangular window. For all images we measure the lateral distance $\Delta x$ between the two -3dB points of the anchor \DIFaddbeginFL \DIFaddFL{(red object)}\DIFaddendFL . These are specified relatively to the reference distance  $\Delta x_\text{ref}$ from the upper left DAS image.
\newline
...
\end{align*}
%
which computes the sample power average over $N_k=2K+1$ temporal samples. This is the same temporal averaging method we use for MVDR. In our case the \DIFaddbegin \DIFadd{bandwidth of our }\DIFaddend matched filtered signal is a just few samples long, hence $K=1$ will be used for both MVDR and LCA throughout this work.

\DIFdelbegin %DIFDELCMD < \todo{Drop this paragraph altogether?}%%%
\DIFdel{If extra flexibility is desired}\DIFdelend \DIFaddbegin \DIFadd{Since pixels are correlated laterally}\DIFaddend , we can \DIFdelbegin \DIFdel{also estimate the variance by }\DIFdelend \DIFaddbegin \DIFadd{include }\DIFaddend a weighted combination of \DIFdelbegin \DIFdel{local pixels
}\DIFdelend \DIFaddbegin \DIFadd{these as well to improve the variance estimation:
}\DIFaddend %
\begin{align}
...
\end{align}
%
where $N_x = 2X+1$ is the number of azimuth lines to average over, and $w_\text{prf}[x',n']$ is a normalized 2 dimensional weight function. \DIFaddbegin \DIFadd{However, we observed no visual improvement doing this, and the effect of micro-steered windows is hard to predict. Henceforth no lateral averaging will be applied.
}\DIFaddend % 
% \begin{itemize}
...
\end{align}
%
The Kaiser-Bessel window is near optimal in the sense of having its peak energy concentration around $\theta=0^\circ$, for a given \DIFdelbegin \DIFdel{time-bandwidth }\DIFdelend \DIFaddbegin \DIFadd{space-bandwidth }\DIFaddend product specified with the parameter $\beta$:
%
\begin{align}
\beta = \frac{TB}{2},
\end{align}
%
where $T$ \DIFdelbegin \DIFdel{is the }\DIFdelend \DIFaddbegin \DIFadd{in our case is the spatial }\DIFaddend extent of the window \DIFdelbegin \DIFdel{in time}%DIFDELCMD < \todo{Should perhaps say space-bandwidth product?} %%%
\DIFdelend and $B$ is its bandwidth. Adjusting $\beta$ changes the trade-off between mainlobe width and sidelobe level. When $\beta=0$ the window becomes rectangular, while at large values ($\beta>5$) the window converges to a Gaussian both in time and frequency. This class of windows is generally considered well suited for separating closely spaced \DIFdelbegin %DIFDELCMD < \todo{better word?} %%%
\DIFdelend sources with amplitudes of a high dynamic range~\cite{Harris1978}, they are easy to make, and they are optimal for any value of $\beta$.
...
}\DIFaddend . It is a high resolution synthetic aperture sonar with 1.2\;m array length, 100\;kHz operating frequency, \DIFdelbegin \DIFdel{40}\DIFdelend \DIFaddbegin \DIFadd{30}\DIFaddend \;kHz bandwidth and \DIFdelbegin \DIFdel{25}\DIFdelend \DIFaddbegin \DIFadd{23}\DIFaddend $^\circ$ \DIFdelbegin \DIFdel{opening angle on transmission}\DIFdelend \DIFaddbegin \DIFadd{receiver element beamwidth}\DIFaddend .
...
In the image reconstruction we have run MVDR with subarray length $L=16$ and \DIFdelbegin \DIFdel{$d=1\%$ diagonal loading}%DIFDELCMD < \todo{also used for element distance}%%%
\DIFdelend \DIFaddbegin \DIFadd{$\text{\large$\epsilon$}=1\%$ diagonal loading}\DIFaddend . Both MVDR and LCA was run with $K=1$ temporal averaging. This is a fairly aggressive yet stable set of parameters\DIFdelbegin \DIFdel{.}%DIFDELCMD < \todo{suitable ref?}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Synnevag2009}
}%DIFAUXCMD
...
When using LCA with Kaiser windows we made their responses similar to those typically chosen by a robust MVDR in key areas in the image. We suggest an upper bound for $\beta$ between 2 and 5, where 5 is slightly better at suppressing sidelobes. For $\phi$ we suggest to steer either to 50\% or 100\% of the \minus{}3\,dB width, which leads to a resolution of 73\% or \DIFdelbegin \DIFdel{54}\DIFdelend \DIFaddbegin \DIFadd{53}\DIFaddend \% of the rectangular window, respectively. As lower boundaries we set $\beta=0$ to always include a rectangular window, and $\phi=0$ to always include the centered window.
...
The authors would like to thank Kongsberg Maritime, the Norwegian Defence Research Establishment (FFI), and Bundeswehr Technical Center for Ships and Naval Weapons, Maritime Technology and Research (WTD 71), \DIFaddbegin \DIFadd{Germany, }\DIFaddend for the equipment and effort needed to acquire data of the \DIFaddbegin \DIFadd{resolution test }\DIFaddend cross and Holmengraa, as well as for allowing us to use it.
