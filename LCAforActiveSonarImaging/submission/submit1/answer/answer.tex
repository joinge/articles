...
\ans{We appreciate the reviewer's }
...
\ai{We appreciate having this pointed out. We are using a rather loose definition of adaptivity that includes any kind of dynamic and data dependent behaviour, hence feel that both beamformers fall in that category. However, as the reviewer pointed out, the method . }
%
\qi{The use of non-SI units like cm is discouraged.}
\ai{We agree that non-SI units should be avoided, however had the belief that cm was a valid unit? In the IEEE Styles Manual this unit is used several times as an example: 
\url{http://www.ieee.org/publications\_standards/publications/journmag/online\_style\_manual-10292015.pdf}} %(on page 24 paragraph ``The En, Em, or Two-Em Dash'', and several times on page 25 paragraph ``Math'' bullet point 16. Should we be adhering to a different style guide?}
%
\end{enumerate}
