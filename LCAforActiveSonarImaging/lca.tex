...
\caption{One of the objects used to test the beamformers is a 1\,m long cross attached to an anchor with a diameter of approximately 13\,cm.}\label{cross}
\end{figure}
...
\hfil
\subfloat[Image of Holmengraa. All the adaptive beamformers offer improvement compared to DAS. The shadows are deeper, the edges better refined and the detail level is higher.]{\includegraphics[width=\textwidth]{gfx/beamformer_comparison_holmengraa.pdf}%%
...
We compare LCA image quality to that of DAS and MVDR in \Fig{image_quality}. \Fig{beamformer_comparison_cross} are of the cross, were we display the images computed by LCA using Kaiser windows, LCA using trigonometric windows, and MVDR.


 \Fig{beamformer_comparison_holmengraa}
...
Throughout this article we have used LCA with variations of the Kaiser window function defined in (\ref{kaiser_window_element}), steered as described in (\ref{steered_window}). However, as shown in \Fig{image_quality} we can obtain similar performance using the trigonometric window function defined in (\ref{trig_window_function} ):
%
\begin{align}
f_m(\alpha) = e^{j\theta_\phi[m]}\Big[\alpha - (1-\alpha)\cos\Big(\frac{2\pi m}{M-1}\Big)\Big]\label{trig_window_function}
\end{align}
%
where $\theta_\phi[m] = \frac{2\pi dm}{\lambda}\sin\phi$ is the $m$th component of the steering vector defined in (\ref{steering_vector}). For this window there is an analytic solution for the optimal value of $\alpha$ in the minimum variance sense. To derive it, we start by inserting the window function (\ref{trig_window_function}) into the beamformer equation (\ref{z}):
%
\begin{align}
\w\H\x &= \sumb{m=0}{M-1} e^{j\theta_\phi[m]}\Big(\alpha - (1-\alpha)\cos\left(\frac{2\pi m}{M-1}\right)\Big) x_m^* \nn
&= \alpha \a_\phi\T\x - (1-\alpha)\b_\phi\T\x
\end{align}
%
where $\a_\phi$ is the steering vector defined in (\ref{steering_vector}), and 
%
\begin{align}
...
\end{align}
%
This is true for any value of $\alpha$ if $\a_\phi\T\1 = 1$ and $\b_\phi\T\1 = 1$. Hence, we can preserve the unity gain constraint as long as we normalize $\a_\phi$ and $\b_\phi$.
...
&= 2\alpha (aa^* + ab^* + a^*b + bb^*) - (ab^* + a^*b + 2bb^*) = 0,
\end{align}
%
