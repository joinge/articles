...
\begin{itemize}
\item Methods.
\item See what happens to Capon's response when means for robustification are added.
...
\begin{align}
z[n] = \sumb{m=0}{M-1} w_m[n]^*x_m[n-\Delta_m] = \w\H[n]\X_\Delta[n]
\end{align}
where
\begin{align}
\w[n] = \bmat{w_0[n]\\w_1[n]\\\vdots\\w_{M-1}[n]} \qquad \text{and} \X_\Delta[n] = 
\end{align}
