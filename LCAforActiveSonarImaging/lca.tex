...
% 
We have implemented and studied the low complexity adaptive (LCA) beamformer in active sonar imaging. LCA can be viewed as either a simplification of the minimum variance distortionless response (MVDR) beamformer, or as an adaptive extension to the delay and sum (DAS) beamformer. Both LCA and MVDR attempt to minimize the power of noise and interference in the image, but while MVDR tries to find the optimal solution by estimating and inverting a spatial covariance matrix, LCA iterates through a predefined set of sensor weights (or window) and selects the closest to optimal one.

We demonstrate that a robust MVDR implementation typically creates responses ranging between that of the rectangular and Hamming, or Kaiser windows with $\beta\in[0,2]$, respectively. We design the LCA window database with variations of Kaiser windows in this range, and add some steered versions of each. 
