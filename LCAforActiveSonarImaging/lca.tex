...
% 
We have implemented and studied the low complexity adaptive (LCA) beamformer in active sonar imaging. LCA is an attractive alternative to the minimum variance distortionless response (MVDR) method for minimizing noise and interference in sonar images. While MVDR estimates and inverts a spatial covariance matrix to compute an optimal window for the sonar array, LCA selects the optimal window from a predefined set using the same criterion as MVDR.

We show that a robust MVDR implementation typically chooses approximately symmetric windows with maximum sidelobe levels between that of the rectangular and Hamming window. We then set up LCA to use Kaiser windows with similar responses to those of MVDR, and apply various amounts of steering. We find that when Steering to -3\,dB width of the window. This lead to a predictable resolution gain of . design LCA's window database with windows that resembles  constrained to work in active sonar imaging the typical window responses LCA is allowed to choose from windows that are typical solutions found by the minimum variance distortionless response (MVDR) method. 
