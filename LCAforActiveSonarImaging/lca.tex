...
Let the receiver be an $M$ element uniform linear array. Call the the data from the $m$th channel $x_m[\theta,n]$, where the azimuth angle $\theta$ and range sample $n$ is the focus point. To simplify notation, we make the dependence on $\theta$ implicit from now on.
