...
\caption{One of the objects used to test the beamformers is a 1\,m long cross attached to an anchor with a diameter of approximately 13\,cm. Source image curtesy of Bundeswehr Technical Center for Ships and Naval Weapons Bundeswehr, Maritime Technology and Research (WTD 71).}\label{cross}
\end{figure}
...
\begin{figure*}[t]\centering
\subfloat[LCA using Kaiser windows, LCA using trigonometric windows, and MVDR. Two lateral image cuts are shown in the leftmost figures, along with the response of a rectangularly weighted DAS. Note how the FWHM of LCA sits in between that of DAS and MVDR. Also note how LCA is insensitive to the window type; its performance is near identical whether it uses Kasier or trigonometric windows.]{\includegraphics[width=\textwidth]{gfx/beamformer_comparison_cross.pdf}%
\label{beamformer_comparison_cross}}
\hfil
\subfloat[Image of Holmengraa. All the adaptive beamformers offer improvement compared to DAS. The shadows are deeper, the edges better defined and the detail level is higher.]{\includegraphics[width=\textwidth]{gfx/beamformer_comparison_holmengraa.pdf}%%
\label{beamformer_comparison_holmengraa}}
\caption{Image quality: Comparing DAS with MVDR and LCA with Kaiser and trigonometric windows.}\label{image_quality}
\end{figure*}
...
\vdots\\
f_{M-1}(\beta)
}\label{kaiser_window_vector}
\end{align}
%
where
%
\begin{align}
f_m(\beta) = \frac{I_0\left(\pi\beta\sqrt{1-\left(\frac{2m}{M-1}-1\right)^2}\right)}{I_0(\pi\beta)}\label{kaiser_window_element}
\end{align}
%
and $I_0$ is the zeroth order modified Bessel function of the first kind:
%
\begin{align}
I_0(x) = \sumb{a=0}{\infty} \left[ \frac{\left(\frac{x}{2}\right)^a}{a!} \right]^2.\label{modified_bessel_first_kind}
\end{align}
%
...
%
\begin{align}
\w_{\beta,\phi} = \frac{\f_\beta\H\diag{(\a_\phi)}}{\f_\beta\H\a_\phi}\label{steered_window}
\end{align}
%
...
\subsection{Steering bounds}\label{steering_bounds}
...
%
\begin{align}
\phi_\text{3dB}(\beta) \approx \minus{}0.002894\beta^3 + 0.02451\beta^2 - 0.01308\beta + 0.2980\label{steering_kaiser_3dB}
\end{align}
%
...
\subsection{Trigonometric windows}
...
%
\begin{align}
f_m(\alpha) = \alpha - (1-\alpha)\cos\Big(\frac{2\pi m}{M-1}\Big)\label{trig_window_function}
\end{align}
%
where we apply the contraint $\alpha\in[0.5,1]$ to avoid windows with negative coefficients. Steering is applied as in (\ref{steered_window}), and the $\alpha$-value that halves the 3\,dB distance is found using the method described in section \ref{steering_bounds}:
%
\begin{align}
...
Observe from \Fig{mvdr_selected_windows} that MVDR seems to prefer symmetric window responses, even if it is free to choose non-symmetric ones. The symmetry is most predominant in shadow and speckle regions, while in highlight the windows are approximately symmetric within the illuminated sector of the seafloor. We have observed this symmetry to be a side-effect of the averaging steps needed to build the sample covariance matrix, which is required to allow MVDR to run in near snapshot mode in an active system. The peak sidelobe levels of the MVDR windows are mostly between -13\,dB and -43\,dB. This corresponds to that of the rectangular and Hamming window, or a Kaiser window with $\beta=0$ and $\beta=2$, respectively.
...
We compare LCA image quality to that of DAS and MVDR in \Fig{image_quality}. In \Fig{beamformer_comparison_cross} we display the images computed by LCA using Kaiser windows, LCA using trigonometric windows, and MVDR. Two selected lateral cuts are presented in the left plot. The images and these cuts show that there is minimal difference to using Kaiser windows compared to trigonometric windows. Compared to DAS the LCA produces targets with an FWHM that lies in between that of DAS and MVDR.

In \Fig{beamformer_comparison_holmengraa} we show full sector images of the Holmengraa wreck. We compare the images produced by a Hamming weidted DAS, MVDR and LCA with Kaiser or trigonometric windows. All the adaptive beamformers produce a sharper and less noisy image than DAS, but the difference between LCA and MVDR is minimal. Also here it is nearly impossible to observe any difference to using Kaiser or trigonometric windows with LCA.
...
In comparison, LCA is of O($M N_\beta N_\phi$), and extremely well suited for GPUs. Each pixel depends on only a small subset of data, and the windows can be precomputed and stored in GPU cache for near immediate access. If we use a trigonometric window function we solve for the window parameter analytically as described in appendix \ref{reducing_lca_complexity}. This reduces the complexity to O($M N_\phi$), and the only remaining LCA parameter is the steering amount.
...
\end{align}
%
where $[\a_\phi]_m$ is the $m$th component of the steering vector $\a_\phi$ defined in (\ref{steering_vector}), and 
%
\begin{align}
...
The authors would like to thank Kongsberg Maritime, the Norwegian Defence Research Establishment (FFI), and Bundeswehr Technical Center for Ships and Naval Weapons Bundeswehr (WTD 71) for the equipment and effort needed to acquire data of the cross and Holmengraa, as well as for allowing us to use it.
