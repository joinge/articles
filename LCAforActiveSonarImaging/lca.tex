...
MVDR was run with a fairly aggressive yet stable set of parameters; subarray size $L=16$, temporal averaging of $K=1$, and $d=1\%$ diagonal loading. Temporal averaging of $K=1$ was also used with LCA.
...
If we look at the peak sidelobe levels of the MVDR windows they are for the most part between -13\,dB and -43\,dB. This corresponds to that of the rectangular and Hamming window, or a Kaiser window with $\beta=0$ and $\beta=2$. Choosing another value for $\beta$ will  This suggests This and symmetry suggests 

Since the responses as similar to those 
...
\subsection{Kaiser parameter boundaries}

What is the best set of Kaiser windows to use with LCA, and how much should they be steered?

To investigate this we constructed large and equally sized window databases containing windows with varied upper boundaries for $\beta$ and $\phi$. As lower boundaries we chose $\beta=0$ and $\phi=\pm0^\circ$, which includes the rectangularly weighted DAS image. The images produced are shown in \Fig{oversampling_mosaic_bounds}.
