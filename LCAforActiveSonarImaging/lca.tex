...
In short, our findings show that when applied to experimental data, the weights chosen by a robustified MVDR yield responses that tend to be symmetric and steered within a fraction of the -3dB resolution of the respective window. When mimicking these responses with Kaiser windows, we obtain images with both improved noise suppression and resolution. We resolution gain can be made predictable and depending on the amount of steering we apply to the windows. Overall, the image quality produced by LCA is comparable to MVDR, and sometimes even better. LCA is also inherently robust, easy to implement and fairly easy to understand.
...
\caption{The cross and accompanying anchor used to test the beamformers.}\label{cross}
\end{figure}
...
Assuming that a robust MVDR computes the optimal windows in the minimum variance sense, it seems sensible to create similar ones for LCA. We study this in \Fig{mvdr_selected_windows}, where an MVDR sectorscan image of the Holmengraa wreck is presented, along with the window responses MVDR chose in the shadow, speckle and highlight regions of this image. A fairly aggressive yet stable set of parameters for MVDR where used; subarray size $L=16$, temporal averaging of $K=1$, and $d=1\%$ diagonal loading.

While MVDR is free to choose non-symmetric windows, we can observe from \Fig{mvdr_selected_windows} that it prefers symmetric ones. In shadow and speckle the windows are fully symmetric, while in highlight the windows are approximately symmetric within the illuminated\todo{valid term?} sector of the seafloor. The peak sidelobe levels are between that of the rectangular and Hamming window.

All the responses in \Fig{mvdr_selected_windows} can be approximated with the Kaiser-Bessel function. By adjusting the window parameter $\beta$ we can adjust the trade-off between mainlobe width and sidelobe level. 
To generate windows with different mainlobe width and sidelobe level, 

A parametric window like Kaiser-Bessel allow us to create approximately these and the responses in between.

The Kaiser-Bessel windows s with $\beta$-values of 0 and 2 allows us to approximately trade between these two boundaries. \todo{add the advantages here?}

In our studies 

The Kaiser windows are easy to compute, is fairly insensitive to coefficient inaccuracies, and can create windows with shapes spanning from rectangular to Gaussian.

\Fig{oversampling_mosaic_bounds_beta} illustrates which $\beta$-value LCA prefers for different regions in the image of the cross. Red marker lines are used to pinpoint the cut line and location of the anchor. Observe that in the anchor region only rectangular windows are used, which is the reason for the anchor resolution being measure to the same value regardless of the upper bound of $\beta$ in \Fig{oversampling_mosaic_bounds}. 
...
\subsection{Window parameter boundaries}

Letting LCA choose from Kaiser windows seems like a good choice due to their similarity to the 
Which Kaiser windows should we include in the windows database? The parameter $\beta$ controls the window shape, and $\phi$ the steering. 

Having chosen to fill the window database with steered Kaiser windows, we should  function While having decided to use steered Kaiser windows, we still  To decide what the ideal boundaries for $\beta$ and $\phi$ should be, we constructed large and equally sized window databases containing windows with different parameter boundaries. The images produced are shown in \Fig{oversampling_mosaic_bounds}. To include and compare with a rectangular weighted DAS, we set the lower boundary to 0 for both parameters.

varied upper boundaries for $\beta$ and $\phi$.

 The upper left image has $\beta=0$ and no steering, and is thus equal to a rectangularly weighted DAS. 
...
\subsection{Window database size}
