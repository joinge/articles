...
\item But: Active system $\Rightarrow$ noise and signal correlated $\Rightarrow$ adaptive beamformers no longer robust.
Computationally complex O($M^3$), and although subspace/beamspace methods exist,
\begin{itemize}
...
\item Computationally complex O($M^3$), and although subspace/beamspace methods exist, 
\end{itemize}
\item Subarray averaging decorrelate the noise, but in the process we sacrifice resolution.
\end{itemize}
...
\item Why active sonar make it hard. Correlated noise, robustification kills the adaptive potential. Quite computationally intensive.
...
\item Outline: Start with Capon's response when means for robustification are added.
\end{itemize}
...
% \begin{itemize}
% \item Why beamforming?
% \item Adaptive beamformers
% \item Challenges in active sonar systems. Correlated noise, robustification kills the adaptive potential. Quite computationally intensive.
% \item LCA is like DAS, only we test several windows and choose the one that yield the lowest beamformer output power. Ultrasound \cite{Synnevag2008}, active sonar \cite{Buskenes2011}\cite{Blomberg2011}\cite{Blomberg2012}\todo{who was first?}
% \item Outline: Start with Capon's response when means for robustification are added.
% \end{itemize}
% 
% 
