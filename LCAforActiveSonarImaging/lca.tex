...
In this work we investigate a few questions related to the LCA that remain unanswered. One is what window function we can use, and how many windows we will need. We limit our study to the use of variations of the Kaiser-Harris window, since we observed neglible performance difference to using this function or any of the common trigonometric ones. The Kaiser windows are easy to compute, is fairly insensitive to coefficient errors, and can create windows with shapes spanning from rectangular to Gaussian. windows are needed, and what variations of the windows are necessary? 

Another question is what it takes to achieve double the resolution.

ne is what window functions LCA should be allowed to choose from. This hardly seems to matter,   Our studies show minimal difference between the traditional windows, so the common and reasonable Kaiser-Harris window where the compromize between What windows should the LCA be allowed to choose from? How many is needed?  by Synnev\aa{}g demonstrated the potential of LCA, 
