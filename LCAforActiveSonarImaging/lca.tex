...
Throughout this article we have used LCA with variations of the Kaiser window function defined in (\ref{kaiser_window_element}), steered in . However, as shown in \Fig{image_quality} we can obtain similar performance using the trigonometric window function:

\ref{trig_window_function} \ref{steering_window}

%
\begin{align}
f_m(\alpha) = e^{j\phi_\theta[m]}\Big[\alpha - (1-\alpha)\cos\Big(\frac{2\pi m}{M-1}\Big)\Big]\label{trig_window_function}
\end{align}
%
(\ref{steering_vector})
