...
% 
We have implemented and studied the low complexity adaptive (LCA) beamformer in active sonar imaging. LCA can be viewed as either a simplification of the minimum variance distortionless response (MVDR) beamformer, or as an adaptive extension to the delay and sum (DAS) beamformer. While both LCA and MVDR attempt to minimize the power of noise and interference in the image, MVDR tries to find the optimal solution by estimating and inverting a spatial covariance matrix, while LCA iterates through a predefined set of sensor weights (or window) and selects the closest to optimal one.

We show that a robust MVDR implementation typically creates responses ranging between that of the rectangular and Hamming. We let LCA select from a set of Kaiser windows with responses in this range, and add some steered variations of each. The amount of steering is limited to roughly half the response's -3\,dB width.

Using experimental data we find that LCA and MVDR produces nearly identical images of large scenes. On point targets LCA is able to double the resolution compared to DAS, of provide half that of MVDR. This performance is achieved with a total of 6 windows; the unsteered rectangular and Kaiser window with $\beta=5$, and left and right steered versions out to the steering limit. Adding a few extra windows makes the image slightly smoother. 
