...
\end{align}
%
where $N_x = 2X+1$ is the number of azimuth lines to average over, and $\omega[x',n']$ is a normalized 2 dimensional weight function. Since we oversample slightly laterally when we delay each pixel (see Section \ref{sec:lca_oversampling}), we thought the lateral correlation to be sufficient to benefit from this. However, we observed no visual improvement applying this averaging in our case. Also, since the effect of phase-steering windows is hard to predict. Henceforth no lateral averaging will be applied.
% 
% \begin{itemize}
