...
We propose using a light weight alternative to MVDR called the Low Complexity Adaptive (LCA) beamformer. It was first introduced by Synnev\aa{}g in clinical medical imaging~\cite{Synnevag2008}, who demonstrated its ability to obtain very similar results to MVDR in a system with focused transmit beams. LCA applies a set of predefined windows, or tapers, and selects the one offering the best noise suppression. It relies on the same optimization criterion as MVDR, thus may be considered a version of MVDR with a reduced window solution space. Alternatively, it may be viewed as a delay-and-sum (DAS) beamformer with an adaptive multi-taper extension. Such extensions have been studied in e.g. radar imaging up to \cite{Stankwitz1995}, but unlike these 
