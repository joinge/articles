...
\end{align}
where $w_m$ is the weight factor assigned to channel $m$. We will refer to a set of weights as a window, but it is also commonly called a taper function. Conventional beamformers all have static weights, and the reference here is the delay-and-sum (DAS) beamformer, or the backprojection algortithm. As the name implies it delays each pixel into focus, then applies a suitable window, and finally sums the data. 
