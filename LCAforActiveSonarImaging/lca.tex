...
Since LCA operates with the same optimization criterion, we will be using approximately a factor 8 lateral oversampling for both beamformers. The images appear more visually pleasing and detailed up to 8, but we consider an absolute minimum to be 2. In \mediaII{} we demonstrate this with LCA images changing as a function of a lateral oversampling factor. The displayed images are all upinterpolated to the same size, but the detail level varies. 
% 
...
\end{align}
%
where $\phi_\theta[m] = \frac{2\pi}{\lambda}dm\sin\theta$ is the steering angle. Inserting this into the beamformer equation yields:
%
\begin{align}
...
\end{align}
%
This is true for any value of $\alpha$ if $\a\T\1 = 1$ and $\b\T\1 = 1$. Hence, as long as we normalize $\a$ and $\b$ the unity gain constraint is preserved.
...
\end{align}
%
Differentiating this with respect to $\alpha$ yields:
%
\begin{align}
...
\end{align}
%
In this computation there are only 4 unique additions and multiplications. If we used this to analytically solve for $\alpha$, but perform the search for $\phi$, the LCA's computational complexity would be of O($8MN_\phi)$ instead of O($MN_\alpha N_\phi)$. The value for $\alpha$ would also be the optimal.
