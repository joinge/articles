...
In many cases MVDR can improve the contrast and resolution of active sonar images compared to conventional static methods~\cite{Blomberg2013,Blomberg2012a,Dursun2009,Lo2004}. However, while the computational complexity of conventional beamformers are linear with the number of channels, O($M$), MVDR is at O($M^3$). This is because MVDR relies on estimating and inverting a covariance matrix. We have shown that in systems with less than $\approx$32 channels the complexity can be reduced and the implementation accelerated significantly using graphics computing units (GPUs)~\cite{Buskenes2014}. For larger systems the inversion step is the bottleneck and space reduction techniques are desirable. In clinical ultrasound, where the transmit beams are focused, we adapted our GPU implementation to operate in beamspace where the spatial sparsity allows much smaller matrices to be inverted~\cite{Asen2013}. Two closely related dimensionality reducing techniques used with MVDR includes using principal component analysis (PCA)~\cite{Kim2014}, and to assume spatial stationarity to form more easily invertible Toepliz matrices~\cite{Asl2012}. At best the mentioned methods are able to reduce the complexity to O($M^2$).

A much faster and simpler alternative to MVDR is the Low Complexity Adaptive (LCA) beamformer. Based on an idea by Vignon~\cite{Vignon2008}, it was first introduced by Synnev\aa{}g in clinical medical imaging~\cite{Synnevag2008}, who demonstrated its ability to obtain very similar image quality to MVDR in systems with focused transmit beams\todo{ha med at i US \o{}nsker man ulike vindus-typer p\aa{} forskjellig range?}. LCA applies a set of predefined windows, or tapers, and selects the one offering the best sidelobe suppression. It relies on the same optimization criterion as MVDR, thus may be considered a version of MVDR with a reduced window solution space. Alternatively, it may be viewed as a an multi-taper technique. One such method is described by Stankwitz in radar imaging~\cite{Stankwitz1995}. Here the best of 3 windows are selected, but no micro-steered variations are used like LCA does.
