...
Most modern active sonar imaging systems are usually far better at resolving details in range than in the angular direction. This is because it is relatively cheap and easy to collect a long and well sampled temporal representation of the incoming wavefield, but hard to do so spatially. It would require a very large array with a lot of sensors, which is costly, complex and impractical. Alternatively, a larger array can be synthesized from several successive pings recorded by a smaller physical array using a technique called synthetic aperture sonar (SAS). Unfortunately, not many of these systems exist due to the difficulty of navigating with sufficiently high precision under water.


It would be more convenient to avoid 
