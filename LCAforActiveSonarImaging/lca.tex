...
We show that a robust MVDR implementation typically creates responses ranging between that of the rectangular and Hamming, or Kaiser windows with $\beta\in[0,2]$, respectively. We let LCA select from a set of Kaiser windows in this range, and add some variations of steering to each window. When up to their -3dB width.  

each with a little steering added.  


 We find that when Steering to -3\,dB width of the window. This lead to a predictable resolution gain of . design LCA's window database with windows that resembles  constrained to work in active sonar imaging the typical window responses LCA is allowed to choose from windows that are typical solutions found by the minimum variance distortionless response (MVDR) method. 
