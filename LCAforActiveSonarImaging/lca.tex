...
Due to the limitations imposed on hardware, a lot of research is put into improving the part of the digital image formation process. A core component here is the beamformer, a spatial band-pass filter used on the wave field to estimate each pixel value. To allow the use of wide-band signals a coarse steering is first applied by delaying the sensor channels, followed by an individual weighting of them. Using complex weights, this implies fine adjustment of both the amplitude and the phase of the spatial filter. While conventional beamformers apply static weight functions, adaptive methods tailor the weights to the wave field.

One of the best studied adaptive methods is the minimum variance distortion-less response (MVDR) beamformer. It solves an optimization problem to come up with a weight set that yields unity gain in the desired direction, while minimizing the effect of noise and interference from other directions. The result is often good looking non-linear image with improved contrast and detail resolution. However, MVDR is inherently neither statistically nor numerically robust, and typically needs to average data in space and time, and regularization. This constrains the solution space of the weights.

We will show that when applied to experimental data, the weights typically chosen by a robustified MVDR yield responses that are typically symmetric and steered within a fraction of the -3dB resolution of the respective window. Then we make Kaiser windows of different types, and with varying degree of steering, that combined discretizes the MVDR solution space. 

The LCA method was first introduced in clinical ultrasound imaging, where it was shown to improve image quality compared to conventional static beamforming methods \cite{Synnevag2007}. 


MVDR is a fully adaptive method, meaning an optimal set of weights are produced given its optimization problem. Its only constraint is that the weights must sum to one. 

From our work with MVDR it 


 tailor the weights to the incoming wavefield. 
...
than itof to collect information from an extended part of the incoming wave in time it is easier to sample the incoming wavefield both the wavefield is better the angular resolution is usually far inferior to the The quality of active sonar imaging systems are 

Improving the angular resulution of active sonar systems are 
In general the 
In modern active sonar imaging systems it is generally harder to obtain sufficient is to transmit and receive wideband acustic pulses using multi-sensor arrays. and process their record the backscattered wavefield using an array of hydrophones.  Through a coherent combination of the sensor data these 
