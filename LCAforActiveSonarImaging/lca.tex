...
\end{align}
where $w_m$ is the weight factor assigned to channel $m$. We will refer to a set of weights as a window, but it is also commonly called a taper function. Applying a window that trails off towards the edges improves noise suppression at the cost of reduced resolution. Real windows lead to symmetric responses, while complex weights allow asymmetric responses.
...
Adaptive beamformers are generally not so simple. They allow the weights/delays to change to adjust the array response to better fit the incoming wavefield. Of these methods the minimum variance distortionless response (MVDR) beamformer is perhaps the best studied and well known. It seeks to minimize the power of the beamformer, under the constraint of a unity gain in the desired direction $\a$~\cite{Capon1969}:
%
\begin{align}
\underset{\w[n]}{\argmin}\, E\{|z[n]|^2\} &= \underset{\w[n]}{\argmin}\, \w[n]\x\x\H\w\H[n]\\
\text{subject to } \w[n]\a &= 1
\end{align}
%
where $\a$ is a steering vector. This is a convex optimization problem with the solution:
%
\begin{gather}
\vec w[n] = \frac{\Ri[n]\a}{\a\T\Ri[n]\a},\label{weights}
\end{gather}
where $\R=E\{\x[n]\x\H[n]\}\in\mathbb{C}^{M,M}$ is the spatial covariance matrix for the full array. Estimating and inverting this spatial covariance matrix is a non trivial matter. Spatial averaging is needed to avoid signal cancellation~\cite{Kailath1985}, temporal averaging is needed to obtain true speckle statistics~\cite{Synnevag2009a}, and diagonal loading is needed to improve robustness to parameter errors~\cite{Cox1987,Maksym1979}.

See section \ref{kaiser_windows}.

. A large variety of weighting functions exists here for trading lateral resolution for improved noise suppression (contrast), but one always ends up with a compromise between the two~\cite{Harris1978}.
...
\section{Kaiser windows}\label{kaiser_windows}
...
\begin{align}
f_m = \begin{cases}
\frac{I_0\left(\pi\beta\sqrt{1-\left(\frac{2m}{M-1}-1\right)^2}\right)}{I_0(\pi\beta)}, & 0\le m\le M-1 \\
0 & \text{otherwise},
\end{cases}
\end{align}
%
