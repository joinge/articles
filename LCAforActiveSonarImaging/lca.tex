...
\end{align}
%
The Kaiser-Bessel window is near optimal in the sense of having its peak energy concentration around $\theta=0^\circ$, for a time-bandwidth product specified with the parameter $\beta$:
%
\begin{align}
\beta = \frac{TB}{2},
\end{align}
...
Note that the normalization factor $\f\T\1$ is the reciprocal of the window's coherent gain. Since the signal-to-noise ratio is constant for a given window, the effect of gaining the coherent signal by also allowing an equivalent incoherent noise gain.  
