...
{A GPU Implementation of Capon for an Active Sonar System}
...
\ifPeerReview\else
\begin{IEEEkeywords}
Beamforming, adaptive beamforming, sonar, active, complexity. \todo{not entirely true?}
\end{IEEEkeywords}
\fi}
% \fi
...
The use of phased arrays in modern sonar imaging systems allow each sensor to be independently delayed and weighted 
...
Most modern active sonar imaging systems make use of phased arrays to transmit and receive wavefields. On reception, each pixel in the image is estimated by digitally steering the arrays response to the corresponding region in space, a concept called beamforming. This is achieved by applying a suitable set of delays and weights to each of the sensor channels to ensure that signals coming from the region of interest add constructively, while attempting to make sure noise and interference from all other regions cancel out. 

If a beamformer only employ a static set of weights and delays, it is referred to as a conventional one. Adaptive beamformers, on the other hand, attempts to find an optimal spatial response that best fits the incoming wavefield.  The most common approach is perform a rough adjustment using a static set of delays, and compute a new set of weights at each time instant. 

Conventional beamformers employ a static set of weights and delays, 
