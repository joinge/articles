...
def barBenchmarkGPU(filename, M, Nx, Ny, fromL, toL, stepL=1):
   ...
   width = stepL
   x_range = arange(min_L, max_L+1, stepL)
   ...
   cax2 = ax.bar(x_range + (1/3.0*width), yavg2[:,0], width/3.0, bottom=(yavg1[:,1] + yavg1[:,2]), color='0.75', hatch='/')
   cax3 = ax.bar(x_range + (2/3.0*width), yavg3[:,0], width/3.0, bottom=(yavg1[:,1] + yavg1[:,2]), color='0.75', hatch='o')
   
   # calc acquisition rate for the given number of beams if depth equals 15.4cm.
   acq_speed = 0.2*Nx
   cax6 = ax.plot([0, max_L+stepL], [acq_speed, acq_speed], '-', color='0.2')
