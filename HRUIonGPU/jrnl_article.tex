...
\begin{abstract}
%\boldmath
The abstract goes here. The abstract goes here. The abstract goes here. The abstract goes here. The abstract goes here. 
The abstract goes here. The abstract goes here. The abstract goes here. The abstract goes here. The abstract goes here. 
The abstract goes here. The abstract goes here. The abstract goes here. The abstract goes here. The abstract goes here. 
The abstract goes here. The abstract goes here. The abstract goes here. The abstract goes here. The abstract goes here.  
The abstract goes here. The abstract goes here. The abstract goes here. The abstract goes here. The abstract goes here. 
...
For medical ultrasound, Chen et al. \cite{Chen2011a, Chen2011} have implemented a Capon beamformer on the GPU that produced real apodization weights from real data. Since a real function is symmetric in frequency space the Capon beamformer is not allowed to shift the main lobe under this constraint, and therefore the full potential is not reached. Shifting the main lobe is an important property of the Capon beamformer in order for it to improve edge definitions and contrast in an ultrasound image. Another aspect of using complex data is the benefits of a lowered sampling rate, keeping the number of samples for a complete image at a minimum. All results in this paper is reported with the use of baseband data.
...
In previous work (cite JOE) we have applied the GPU-based Capon beamformer in an active sonar system. For sonar the real time requirement is much lower than for ultrasound, which means that most array configurations can be handled by a straight forward implementation.
%(Referer til JOE-artikkelen, få med beskrivelse av hva som blir anerledes for ultralyd.)

In the next section we present background information about the Capon beamformer and the GPU computing model. In Section \ref{sec:meth} we go in detail about the parallel implementation of the Capon beamformer, before in Section \ref{sec:meth2} we introduce methods to apply as the problem size increases. Benchmark results and resulting images are presented in Section \ref{sec:res}, and finally we discuss the results in Section \ref{sec:dis}, and draw conclusions in Section \ref{sec:con}. 

\section{Background}

\subsection{Capon Beamforming}

Bruk riktig terminologi. $N_{avg}$ er K. Antall sub-arrayer må hete noe annet.

Presis på hvilke formler vi skal implementere.

De skal forstå MV etter denne seksjonen.

Ton ned introduksjonen, og sett opp akkurat de ligningene vi implementerer.

(Skriv om den introduserende setningen, start rett på linje to, Kombinere data fra mange sensorer, Leseren kan Ultralyd ikke gpu)


The conventional form of beamforming is delay-and-sum (DAS), where the signal recorded on each element, $x_m[n]$, is delayed to broadside with an appropriate delay $\Delta_m[n]$. The beamformer output, $z[n]$, is then the sum of all $M$ channels weighted with a factor $w_m$,
\begin{align}
z[n] = \sum_{m = 0}^{M-1}w_m^*x_m[n - \Delta_m[n]] = \vec{w}^H\vec{x}[n]. \label{eq:z}
\end{align}
...
The basic idea of an adaptive beamformer, as shown in Figure \ref{fig:mvbf}, is to calculate the weights on-the-fly, based on the data at hand, and on a selected optimization criteria in such a way that we minimize interference impinging from other directions than the point of focus. For a complex data vector $\vec{x}[n] \in \mathbb{C}^M$, we have for an adaptive beamformer a data-dependent set of weights, $\vec{w}(\vec{x}[n])$, that when applied on $\vec{x}[n]$ is the best approximation to achieving the selected criteria. 

(Vi skal implementer dette. vis bare vektene. Dropp ligning 2, siter, bruk R-hat med en gang. Få med den endelige feilen )

\begin{figure}
\centerline{\includegraphics[width=3in]{gfx/beamforming_mv_lowres.png}}
\caption{Beamforming with adaptive weights $\vec{w}[n]$. Impinging signal is demodulated and aligned in phase by applying steering and focusing delays $\Delta_m[n]$. Capon adaptive weights are then calculated based on the in-phase data, and finally the output is formed by a weighted coherent sum.}
\label{fig:mvbf}
\end{figure}

With the Capon beamformer, adaptive weights are found by solving the following minimization problem \cite{Capon1969}:
\begin{align}
...
In practice, the covariance matrix has to be approximated by the sample covariance matrix, 
\begin{align}
...
\begin{figure}
\centerline{\includegraphics[width=3in]{gfx/gpu_layout_lowres.png}}
...
(Ta med mer banale ting, dette er GPU for ultralydfolk)

Etter denne så må de forstå GPU og hvorfor MV passer.
