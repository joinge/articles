...
\vec{b}[n] = \mat{\hat{R}}[n]^{-1}\vec{1}_L,
\end{align}
where $\vec{1}_L = [1_0, 1_1, ..., 1_{L-1}]^T$. Finally the Capon weight vector is formed based on (\ref{eq:b}) as
...
The beamspace version of the Capon beamformer is derived by replacing the covariance matrix in (\ref{eq:w}) with the beamspace covariance matrix $\mat{\hat{R}}_{BS} = f(\vec{x}_{BS}) = f(\mat{B}\vec{x})$, where $\mat{B} \in \mathbb{C}^{N_b,L}$, and $N_b$ is the number of selected beams. Note that when subarray averaging is applied, the transformation must be done per subarray, yielding subarrays that no longer overlap in space. If we want to apply the transformation before the covariance matrix is constructing, the amount of data is change from $M$ to $N_LN_b$ per sample, however all remaining steps will get lower complexity.

Another choice is to transform the element space covariance matrix to beamspace. 
\begin{align}
\mat{\hat{R}}_{BS} &= \mat{B}\mat{\hat{R}}\mat{B}^H
\end{align}
The pros using this approach is that minor changes has to be made to the pipeline described in Section \ref{sec:meth}, but it will only speed up the solving step which is why we have implemented the first approach.
...
\end{align}
For $l=0$ a full computation has to be performed (\ref{eq:bs_formula}). We call this the sliding beamspace transformation.

From (\ref{eq:sliding_bs}) it is obvious that instructions can be saved if we utilize the overlap across subarrays. However, since the number of beams usually is small we will end up with a large memory per thread ratio ($M > N_b$). Using one GPU-thread per beam is therefore not feasible for small $N_b$ large $M$ configurations. The solution is to combine sliding with full computations. As shown in Fig. (???), a number of thread groups target different sets of subarrays which they transform using the sliding beamspace algorithm. In that way, the number of threads per sample is increased and thus the memory per thread ratio is decreased. 

As mentioned the amount of data is increased from $M$ to $N_LN_b$ per sample in beamspace. This means that we no longer can exploit spatial overlap when calculating the sample covariance matrix, but since $N_b$ is small we can now use one thread per element. A kernel similar to the one described by Chen et al. \cite{Chen2011} has been implemented. After solving, the beamspace weight vector is calculated as 
\begin{align}
\vec{w}_{BS} = \frac{\vec{b}_{BS}}{\sqrt{L}\vec{e}_1\vec{b}_{BS}}
\end{align}  
and applied to the transformed data summed across subarrays.
...
The traditional way of applying adaptive diagonal loading is to load the sample covariance matrix with the average energy per element, hence $trace\{\mat{\hat{R}}\}/L$. If adaptive diagonal loading is applied after data has been transformed into beamspace, a high percentage of the total energy is mapped to a small subset of beams. Thus the average energy per beam is usually higher than the average energy per element, especially if the system uses focused transmit beams. This means that less diagonal loading has to be applied in beamspace than in element space ($d_{BS} \ll d_{element}$).
...
\hfil
\subfloat[]{\includegraphics[width=3.5in]{gfx/benchmark_bar_bs_M=64_Nx=80_Ny=400_Nb=3.png}%
\label{fig:benchBS5}}}
