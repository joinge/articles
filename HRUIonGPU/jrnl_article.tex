...
\end{align}
where the steering vector $\vec{a} = \vec{1}_L = [1_0, 1_1, ..., 1_{L-1}]^T$ since $\vec{x}^\Delta$ is pre-delayed. Finally the Capon weight vector is formed from (\ref{eq:b}) as
...
\label{fig:benchBS5}}}
\caption{Benchmark of GPU beamspace Capon beamforming for a cardiac image covering a $70 \degree$ and $15.4$cm deep sector. Execution time is plotted as a function of subarray length $L$. (a) Shows the benchmark for a 64 elements array. (b) Shows the benchmark for a 96 elements array.}
\label{fig:benchBS}
\end{figure*}

\begin{figure*}[!t]
\centerline{\subfloat[]{\includegraphics[width=2in]{gfx/das.png} \label{fig:phantomDAS}}
\hfil
\subfloat[]{\includegraphics[width=2in]{gfx/capon.png} \label{fig:phantomMV}}
\hfil
\subfloat[]{\includegraphics[width=2in]{gfx/capon_bs.png} \label{fig:phantomBS}}}
\caption{Simulated phantom containing one bright and one dark "cyst" and two point targets in speckle. The simulation was done using Field II with a 2.5MHz, $\lambda/2$ and 64 elements array. a) Delay-and-sum with uniform weights. b) Capon ($L=32, K=2, d=0.1$) c) Beamspace Capon ($L=32, K=2, N_b=3, d=0.01$).}
\label{fig:phantom}
\end{figure*}

\begin{figure*}[!t]
\centerline{\subfloat[]{\includegraphics[width=2in]{gfx/das_invivo.png} \label{fig:invivoDAS}}
\hfil
\subfloat[]{\includegraphics[width=2in]{gfx/capon_invivo.png} \label{fig:invivoMV}}
\hfil
\subfloat[]{\includegraphics[width=2in]{gfx/capon_bs_invivo.png} \label{fig:invivoBS}}}
\caption{\textit{In-vivo} harmonic cardiac ultrasound image acquired using a 3.5MHz, 64 elements phased array.  a) Delay-and-sum with uniform weights. b) Capon ($L=32, K=2, d=0.1$) c) Beamspace Capon ($L=32, K=2, N_b=3, d=0.01$).}
\label{fig:invivo}
\end{figure*}
