...
\label{fig:benchBS5}}}
\caption{(Move BS trans below calcR) Benchmark of GPU beamspace Capon beamforming for a cardiac image covering a $70 \degree$ and $15$cm deep sector. Execution time is plotted as a function of subarray length $L$. (a) Shows the benchmark for a 64 elements array. (b) Shows the benchmark for a 96 elements array.}
\label{fig:benchBS}
\end{figure*}
...
\caption{Simulated phantom containing one bright and one dark "cyst" and two point targets in speckle. The simulation was done using Field II with a 2.5MHz, $\lambda/2$ and 64 elements phased array. Dynamic range is -30dB to 30dB with mean value represented as 0dB. a) Delay-and-sum with uniform weights. b) Capon ($L=32, K=2, d=0.1$) c) Beamspace Capon ($L=32, K=2, N_b=3, d=0.01$).}
\label{fig:phantom}
\end{figure*}
...
\caption{\textit{In-vivo} harmonic cardiac ultrasound image acquired using a 3.5MHz, 64 elements phased array. Number of receive beams for the $70\degree$ sector is 98. Dynamic range is -20dB to 20dB with mean value represented as 0dB. a) Delay-and-sum with uniform weights. b) Capon ($L=32, K=2, d=0.1$) c) Beamspace Capon ($L=32, K=2, N_b=3, d=0.01$).}
\label{fig:invivo}
\end{figure*}
...
In Fig. \ref{fig:benchBS} we present similar benchmarks for the beamspace version of the Capon beamformer, but now for a 64 and 96 elements probe. For the latter, number of transmit beams is increased to 120. For all benchmarks there is a line indicating the time it takes to acquire the underlying image. Hence, below this line we are capable of real time processing.

\section{Simulated phantom}
...
\section{\textit{Ion-vivo} cardiac data}
