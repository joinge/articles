...
In the next section we present background information about the Capon beamformer and the GPU computing model. In Section \ref{sec:meth} we go in detail about the parallel implementation of the Capon beamformer, before in Section \ref{sec:bs} we introduce the beamspace method and how it has been implemented on the GPU. Benchmark results and resulting images are presented in Section \ref{sec:bench} and \ref{sec:images}, together with a discussion of trading image quality for speed in Section \ref{sec:trade}.  Finally we discuss the results in Section \ref{sec:dis}, and draw conclusions in Section \ref{sec:con}. 
...
The final output, as shown in (\ref{eq:z_mv}) and (\ref{eq:z_mv2}), can be formed by either reducing the data down to length $L$, or increase the weight vector to length $M$. Since the design is using $L$ threads per output, the data is reduced to length $L$ by letting thread $r$ calculate 
...
\end{align}
where $\mat{B} \in \mathbb{C}^{N_b,L}$, and $N_b$ is the number of selected beams. If the transformation is applied before the covariance matrix is constructed, one transformation must be performed per subarray, yielding subarrays that no longer overlap in space. The amount of input data is then change from $M$ to $N_LN_b$ elements per sample, however all remaining steps will get lower complexity when $N_b < L$.
...
Equation (\ref{eq:sliding_bs}) shows that instructions are spared if we utilize the overlap across subarrays, but with an increase in task parallel granularity (As discussed in Section \ref{sec:calcR}). The most obvious is to use one thread per beam (index $k$) that calculates this beam for all subarrays. However, since the number of selected beams usually is small ($N_b \ll M$) we will end up with a large memory per thread ratio. Using one thread per beam is therefore not feasible for small $N_b$ large $M$ configurations. The solution is to combine SBS with full computations. As shown in Fig. (???), a number of thread groups target different sets of subarrays which they transform using the SBS algorithm. In that way, the number of threads per sample is increased and thus the memory per thread ratio is decreased. 

As mentioned, the amount of channel data is increased from $M$ to $N_LN_b$ per sample when using SBS. This means that the spatial overlap exploited when calculating the sample covariance matrix in element space is no longer available. On the other hand, since $N_b$ should be small ($N_b < 32$), one thread per element in $\mat{\hat{R}}_{BS}$ is now doable. A kernel similar to the one described in \cite{Chen2011} has therefore been implemented. In addition, for small $N_b$ values, several covariance matrices are calculated per thread block to achieve high occupancy of the GPU.
...
\end{align}  
using on thread per beam, and applied to the beamspace data summed across subarrays,
\begin{align}
z[n] = \vec{w}_{BS}^H\sum_{l=0}^{L-1}\vec{x}_{BS,l}. 
\end{align}
And again are several pixels calculated per thread block for small values of $N_b$ in order to achieve high GPU occupancy. The beamspace data summed across subarrays are calculated and outputted as part of the SBS kernel.   
...
In this paper we have presented a parallel implementation of the Capon beamformer in both element- and beam-space for high resolution cardiac ultrasound imaging. The results show that it is now possible to apply BS Capon in real time in cardiac ultrasound imaging utilizing the power of GPU's. According to Fig. \ref{fig:benchBS} two parallel receive lines is also within reach (for both $M=64$ and $M=96$.  $L=M/2$), but at the moment it is unknown if these extra lines is needed for increased sampling or not. Another concern is that lateral oversampling using parallel receive lines will increase the total execution time with a factor equal the amount of oversampling.

For Capon in element space, processing rates for high quality settings is still to high to support real time cardiac ultrasound imaging. From Fig. \ref{fig:benchUS64} we see that for $L = M/2 = 32$ and $K=1$, the total execution time is 100 ms (10 fps). Taking into account that we use complex data, but less samples per image, these results are similar to the performance reported by Chen et al. \cite{Chen2011}. For the 32 element array in Fig. (\ref{fig:benchUS32}) execution time is reduced to 11.5 ms (87 fps) using the same settings ($L=M/2, K = 1$).

A major part of the total execution time for element space Capon comes from solving the system of linear equations. This limitation is targeted by reducing the sample covariance matrix size in beamspace. As shown in Fig. \ref{fig:benchBS}, inverting the three times three beamspace covariance matrix is barely visible. The time taken to construct all covariance matrices is also reduced, but for few beams this is now the major contributor to the total time. If real time is defined as 10 fps \cite{Chen2011} all execution times in both Fig. \ref{fig:bench} and \ref{fig:benchBS} are real time. More natural for cardiac ultrasound imaging is to define real-time as the time it takes to acquire the underlying image. With this requirement the images formed using a 32, 64 and 96 element array has a real time requirement of 125, 62.5 and 41.7 fps respectively using single line acquisition. Each benchmark plot has a line indicating this real time requirement. With beamspace, all processing times for large subarrays are below this line.

This paper further shows how to obtain a parallel Capon beamformer, and how it can be implemented in a GPU framework. The per-pixel granularity suggest that the beamformer is easily dividable across multiple GPUs. Another option to reach real-time execution for all cases presented in this paper is therefore to use several GPUs simultaneously. In that case, processing rates are increased with a factor equal to the number GPUs. However, there is usually a space-cost-energy limitation for how many GPUs that can be fitted in an ultrasound scanner.
 
Turning to image quality we see from Fig. \ref{fig:phantom}, and the attached video of the simulation, that Capon beamforming highly resolves moving point targets both in element- and in a reduced beam-space. Our preliminary finding is that the Capon beamformer is shift invariant as long as the lateral sampling is increased enough. No major improvement in contrast is visually observed in these images. In the cardiac recording (Fig. \ref{fig:invivo}), structures are thinner and better resolved both when the Capon and BS Capon beamformer is applied. However we observe a loss in contrast, especially in the septum (Fig. \ref{fig:slices}). This loss can be partially explained by the present of highly correlated signals and errors in the assumed steering vector. Future work should focus on solving this issue.

Nilsen et. al suggest to do automatic selection of beams in beamspace, hence picking the beams containing most energy. In this paper the center beams are always picked. This seems to work fine, but an adaptive pick of beams should be implemented if interference at high angles is shown to contaminate the image.
...
In this paper we have shown that the Capon beamformer can be accelerated using GPUs. For beamspace Capon beamforming processing rates supporting real time cardiac ultrasound imaging is shown to be possible. Image quality is investigated both in a simulated phantom and in \textit{in-vivo} cardiac data. Results shows that Capon beamforming is feasible for cardiac ultrasound imaging, providing images with highly resolved structures. 
