...
The straight forward sum in (\ref{eq:R}) has complexity $O(L^2N_LN_K) = O(L^3)$ since $N_L$, the number of subarrays, is a function of $L$. This is actually the same as for inverting a $L \times L$ matrix. However little attention has been devoted to this step in the literature before. Note that for a typical array configuration we typically do more spacial then temporal smoothing. Hence, $N_K$ is usually small compared to $N_L$. For short subarrays the construction of $\mat{\hat{R}}$ is dominated by the number of elements $M$ ($N_L = M - L + 1$), but since $L$ is selected in most cases to be between $M/2$ and $M/4$ a large $M$ implies a large $L$.
