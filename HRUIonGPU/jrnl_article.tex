...
In the next section we present background information on Capon beamforming in both element-space and beamspace and the GPU computing model. In Section \ref{sec:meth} we recap our parallel implementation of the Capon beamformer, before the beamspace method, and its GPU implementation, is presented in Section \ref{sec:bs}. Benchmark results and resulting images are presented in Section \ref{sec:bench} and \ref{sec:images}, together with a discussion of trading image quality for speed in Section \ref{sec:trade}.  Finally we discuss the results in Section \ref{sec:dis}, and draw conclusions in Section \ref{sec:con}. 
...
In this section we give a brief discussion of GPU computing, laying the basis for the discussions on parallel implementation strategies. The Capon beamformers have been implemented using the CUDA GPU framework by Nvidia \cite{Nvidia2013}, however the challenges and proposed methods described in this paper are valid for other GPU framworks as well, like e.g. OpenCL. Independent of framework, the key point is that GPUs can accelerate large sets of small concurrent tasks, like beamforming a pixel in a large image, much better than a CPU. While the CPU has few cores and lack logic to process complex threads, the GPU has many cores, but each core needs many (preferably simple) threads to keep it occupied and to thereby achieve maximum throughput.
