...
%\end{align} 
%Typically the sample covariance matrix has to be averaged over several samples in order to ensure numerical stability and statistical robustness. 
...
The beamspace version of the Capon beamformer is derived by replacing the covariance matrix in (\ref{eq:w}) with the beamspace covariance matrix $\mat{\hat{R}}_{BS} = f(\vec{x}_{BS}) = f(\mat{B}\vec{x})$, where $\mat{B} \in \mathbb{C}^{N_b,L}$, and $N_b$ is the number of selected beams. Note that when subarray averaging is applied, the transformation must be done per subarray, yielding subarrays that no longer overlap in space. If we want to apply the transformation before the covariance matrix is constructing, the amount of data is change from $M$ to $N_LN_b$ per sample. The implementation calculating covariance matrices needs to be adjusted to handle non-overlapping subarrays. BuildR complexity is changed to $O(N_b^2L)$, so constructing the covariance matrices will be faster in beamspace as long as $N_b < \sqrt{L}$. However it will involve large changes to the processing pipeline described in Section \ref{sec:meth}. 

In both cases we need to solve using a transformed steering vector
\begin{align}
\vec{a}_{BS} &= \mat{B}\vec{1}_L = \sqrt{L}\vec{e}_1,
\end{align}
where $\vec{e}_i$ has value one in the i$^{th}$ position and is zero everywhere else.

Another choice is to transform the covariance matrix to beamspace. 
\begin{align}
...
In our implementation we have tried to minimize the number of adjustments that had to be done in order to integrate beamspace processing into the GPU-Capon processing pipeline. The beamspace-results presented in this paper is therefore produced by transforming covariance matrices to beamspace, and after solving, transforming the result back to element space.

Derivation of sliding beamspace transformation. Given a $M$ long data vector divided into $N_L$ subarrays, the beamspace transformation of the $l^{th}$ subarray is given as:
\begin{align}
x_{BS,l,k} = \frac{1}{\sqrt{L}}\sum_{p=i}^{l+L-1}x_p e^{-j2\pi k(p-l)/L} , k \in [0, N_b-1].
\end{align}
Manipulating this expression we find that
\begin{align}
x_{BS,l,k} &= \frac{1}{\sqrt{L}}((x_{BS,l-1,k} - x_{l-1})e^{-j2\pi k/L} \\ &+ x_{i+L-1}e^{-j2\pi k(L-1)/L}) \\
&= \frac{1}{\sqrt{L}}(x_{BS,l-1,k} - x_{l-1} + x_{i+L-1})e^{-j2\pi k/L}
\end{align}
For $l=0$ a full computation has to be performed.
Instructions can be saved if we utilize the overlap across subarrays. However, since the number of beams usually is small we will end up with a large memory per thread ratio ($M > N_b$). Using one GPU-thread per beam is therefore not feasible for small $N_b$ large $M$ configurations. The solution is to combine sliding with full computations. A number of thread groups target different sets of subarrays which they transform using the sliding beamspace algorithm. In that way, number of threads per sample is increased and thus the memory per thread ratio is decreased. Notice how subarrays no longer overlap after the beamspace transformation. The amount of data is increased from $M$ to $N_LN_b$ per sample. This means that we no longer can exploit spatial overlap when calculating the sample covariance matrix, but since $N_b$ is small we can now use one thread per element.    
