...
The fundamental problem of sonar imaging is to estimate each pixel's individual contribution to the final recorded echo. If the data is digitally stored, we can perform this task by focusing on one pixel at a time in the post-processing. Since the geometric relationship of the array and image is known, one usually start by selecting delays that mitigates the difference in transmitter-pixel-receiver propagation time. the delays are usually simply chosen to . 
