...
\end{align}
To sum it up, for each range line, and for each sample $n$ in this line we must
\begin{enumerate}
\item Compute the spatial covariance matrix $\eR[n]$ (equation \ref{spatialR}-\ref{finalR}).
\item Compute $\eRi[n]\1$ (from equation \ref{weights}).
\item Insert the result from step (2) into equation \ref{weights} to find the weights.
\item Insert the result from step (3) into equation \ref{z} to acquire the pixel value.
\end{enumerate}
...
Massively parallel hardware such as \glspl{GPU} have a potential for accelerating many signal processing tasks, but fully exploiting their potential is not always trivial. Modern \gls{GPU} chips contain several hundred processing cores, but the amount of memory available locally for each core is very limited. There is also a considerable latency involved when accessing on-board memory, so assessing various memory access schemes is crucial to achieve optimal performance.

Hence, the programming strategy should be to decompose the algorithm into as many threads as possible, each being computationally intense and be light on memory access and consumption. Consecutive threads should also access consecutive memory regions, to enable coalesced data transfers, and have the same line of execution.
