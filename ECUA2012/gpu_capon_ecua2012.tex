...
\end{gather}
where $\1$ is a row vector of ones, and $\R$ is the spatial covariance matrix. Since $\R$ is unknown, it must be estimated. First, we split the segmenting the array into subarrays where all but one element overlap, and call the output vectors
\begin{gather}
\x_l[n] = \bmat{x_l[n-\Delta_l] & x_{l+1}[n-\Delta_l] & \dots & x_{l+L-1}[n-\Delta_{l+L-1}]}\T.
\end{gather}

\begin{gather}
\tilde\R[n] = \frac{1}{M-L+1} \sumb{i=0}{M-L+1} \x_{i+k}[n]\x_{i+k}\H[n]
\end{gather}
\begin{gather}
\tilde\R[n] = \frac{1}{2N_\text{avg}+1} \sumb{n'=n-N_\text{avg}}{n+N_\text{avg}} \x[n']\x\H['n]
\end{gather}
