...
The Minimum Variance Distortionless Response (MVDR) adaptive beamformer has recently been introduced in active sonar imaging. While displaying great potential for creating high quality images, it is computationally intensive and relies on robustification. These are important reasons why the MVDR beamformer is not in widespread use today.

We propose means for adapting the MVDR beamformer and its robustification techniques to fit a modern Graphics Processing Unit (GPU) architecture. These devices have in recent years
...
The MVDR adaptive beamformer (also known as the Minimum Variance or Capon beamformer) is one such technique. While ensuring unit gain in the look direction, it computes the set of weights that minimizes the power accumulated by the array from other directions. Its ability to suppress interference power and improve image quality in active sonar systems has been demonstrated in e.g.~\cite{Blomberg2012a,Blomberg2011,Dursun2009,Lo2004}.
...
\begin{enumerate}
\item \emph{Computing the spatial covariance matrix} $\eR[n]$ (\ref{spatialR}-\ref{finalR}). A group of threads is created that slide along the diagonals of $\eR$. In this way we exploit the fact that entries on the diagonals overlap across subarrays and time, and keep the number of data reads and writes at a minimum.
...
We have tested our \gls{GPU} implementation of the \gls{MVDR} beamformer on two different experimental datasets from the 32 element Kongsberg Maritime HISAS1030 sonar~\cite{Hansen2009}, which was attached to the HUGIN \gls{AUV}. In the first dataset, the sonar was used in sidescan mode to image the 1500 dwt oil tanker wreck Holmengraa (\Fig{holmengraa}). Holmengraa is roughly 68\,m long and 9\,m wide, and lies on a slanted seabed at 77\,m depth. The \gls{MVDR} image was created with the parameters $L=16$, $N_\text{avg}=3$, and $d=1\%$, which proved to be a reasonable selection for this scenario.
...
\newpage
Interesting to note is also that solving $\eRi\1$ remains the main bottleneck in the \gls{GPU} implementation, and the complexity depends on the subarray size $L$'s. This means the overall complexity of the \gls{MVDR} beamformer is actually reduced when we apply subarray averaging.
