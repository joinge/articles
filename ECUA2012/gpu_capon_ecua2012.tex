...
Figure \ref{holmengraa} demonstrates the Capon beamformer's ability to suppress interference and achieve better detail resolution. Compared the \gls{DAS} beamformer, the ship's edges come out as sharper, and the shadows are less noisy. But while \gls{DAS} processed this 1.3Mpx image in milliseconds, our Matlab implementation of Capon needed minutes.

As Figure \ref{benchmarks} illustrates, using a \gls{GPU} improve matters.  and the \gls{} hence the runtimes are seen are   much smaller than the Holmengraa image, but the relative performance increase seen when accelerating the algorithm with a \gls{GPU} is still accurate. 
