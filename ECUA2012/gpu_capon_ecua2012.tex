...
Consider an $M$ element phased array. Let the sample recorded by the $m$'th channel at time instant $n$ be represented as $x_m[n]$, and the accompanying weights and delays be  $w_m[n]$ and $\Delta_m$. The beamformer output $z[n]$ is then given as
\begin{align}
z[n] = \w\H[n]\X[n] = \bmat{w_0[n]\\w_1[n]\\\vdots\\w_{M-1}[n]}^H \bmat{x_0[n-\Delta_0]\\x_1[n-\Delta_1]\\\vdots\\x_{M-1}[n-\Delta_{M-1}]}.
\end{align}
The \gls{MVDR} beamformer \cite{Capon1969} finds the set of weights that minimizes the output power, $\min E\{|z[n]|^2\}$, while ensuring unity gain in the look direction. Assuming predelayed data, the solution to this optimization problem is given as
\begin{gather}
\vec w[n] = \frac{\eRi[n]\1}{\1\T\eRi[n]\1} = \frac{\vec\beta[n]}{\1\T\vec\beta[n]},
\end{gather}
where $\1$ is a row vector of ones, and $\eR$ is an estimate of the spatial covariance matrix. To ensure robustness in an active sonar setting, we perform subarray averaging in order to decorrelate noise from the signal, and some degree of temporal averaging to retain \gls{DAS}-like speckle statistics. The $(i,j)$'th element of the sample covariance matrix, $\hat r_{ij}$, is then given as
\begin{gather}
...
A massively parallel hardware such as \glspl{GPU} have a potential for accelerating a great many signal processing tasks, but fully exploiting this potential is not always trivial. Modern \gls{GPU} chips may contain hundreds of processing cores, but the amount of memory available locally for core is very limited. There is also a considerable latency involved in moving data between the \gls{CPU} and these cores, but as the data transfer is asynchronous this latency is largely hidden as long as the threads running on each core are computationally intensive.

thread = computationally intense and low memory consumption.
consecutive threads should have linear memory access patterns
...
Benchmarks were carried out on a test system comprised of a quad-core Intel Xeon E5620 running at 2.4GHz, 64Gb of \gls{RAM}, and an nVidia Quadro 6000 card. This \gls{GPU}
