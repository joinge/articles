...
By the methods just presented, we observe that for each range line, and for each sample $n$, the following steps must be carried out:
\begin{enumerate}
\item \emph{Compute the spatial covariance matrix} $\eR[n]$ (equation \ref{spatialR}-\ref{finalR}).
\item Compute $\eRi[n]\1$ (from equation \ref{weights}).
\item Substitute the solution for $\eRi[n]\1$ into equation \ref{weights} to find the weights.
\item Apply the weights to each data vector $\x_l[n]$, and sum them all up (as in equation \ref{z}) to acquire the pixel value $z[n]$.
\end{enumerate}
