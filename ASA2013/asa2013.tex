...
If (\ref{spatialR}) were to be implemented directly, then we note that the estimation step would have a complexity of O($N_K N_L L^2$). By comparison, performing a Gauss Jordan inversion is generally of O($L^3$), which implies that whenever $N_K\,N_L = N_K(M-L+1) \gg L$ the estimation step will dominate. This tend to be true in active systems because we often want to set set $L<M/2$ and $N_K\ge>3$ for the MVDR to be robust enough to yield the expected results.

First, we target this limitation arithmetic optimizations and parallel computing power. In particular, we may infer from (\ref{spatialR}) that:
\begin{itemize}
...
As we increase system size the inversion step will become the main bottleneck. Although the dimensionality of the covariance matrix was reduced to $L$ due to subarray averaging, we can reduce it further by moving to the frequency domain. For narrowband  relation between the frequency conent and angles of arrival in the spatial domain; the zero-frequency component will correspond to broadside, and larger frequency components will correspond to increasing .   component corresponding to broadside and increasing frequencies  This is usually handled by reducing the dimensionality of the covariance matrix to 
