...
If (\ref{spatialR}) were to be implemented directly, then we note that the estimation step would have a complexity of O($N_K N_L L^2$). By comparison performing a Gauss Jordan inversion is generally of O($L^3$), which implies that whenever $N_K\,N_L = N_K(M-L+1) \gg L$ then the estimation step will dominate. This tend to be true in active systems because it is usually required to set $L<M/2$ and $N_K\ge>3$ to MVDR yield the expected results.
