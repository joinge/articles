...
The implementation of beamspace MVDR is very similar, but the data access pattern is different. In the time domain subsequent subarrays overlap on all but one element in memory, but once Fourier transformed the data does not overlap at all. Such a seemingly innocent detail can make all the difference in a GPU design, because efficient memory accessing is key to obtaining good performance.
...
The MVDR beamformer is a technique that is able to improve image quality over conventional methods. This can be seen in Figure \ref{holmengraa}, where the MVDR produces images with cleaner shadows and slightly improved edge detection. Contrary to common belief, it can also be made to create these images relatively fast. Through careful optimization work and by running the MVDR on a GPU we have achieved image creation rates of more than 1\,Mpx/s. This is an improvement of 2 orders of magnitude over our reference implementation in C. To obtain these rates we run MVDR in the time-domain for smaller systems, and in the frequency domain for larger systems.
