...
Our GPU implementation consists of three main kernels relating to the computing steps illustrated in Figure \ref{mvdr_implementation}. In the first step we estimate $\eR$, in the second we solve the linear equation $\eRi\1$, and finally we perform some substitutions to obtain the beamformer output, $z$. All pixels are processed before moving on to the next step. Our efforts here were mainly focused on estimating $\eR$, because an official batch linear equation solver is already available from Nvidia, and computing the beamformer output is a rather simple task.
