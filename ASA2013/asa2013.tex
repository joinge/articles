...
\subsection{MVDR beamforming in time domain}
...
Throughout our work with porting the MVDR beamformer to a GPU, one issue has been particularly difficult to solve: The GPU will only perform efficiently when each computing thread is very computationally intense. This should be apparent by studying Table \ref{tabbandwidth}. It compares theoretical peak values for the memory bandwidth and arithmetic throughput for an Nvidia Quadro 6000 GPU. Looking at global memory, we note that it is only able to move 1 float for every 30 floating point numbers that the GPU is able to execute. Now, if we consider the simple equation $C=A+B$, where the ratio is 3 floats read/written for every 1 floating point operation, we realise that global memory should be avoided whenever possible. Using shared memory (L1 cache) and registers is a big step in the right direction, but this e
...
\end{tabular}
\caption{Nvidia Quadro 6000; Memory bandwidths (BW) compared to the peak arithmetic bandwidth of 1.03 TB/s.}\label{tabbandwidth}
\end{table}


\begin{figure}[t]\centering%
\graphicsAI[drawing,width=0.7\linewidth]{gfx/mvdr_implementation.svg}%
