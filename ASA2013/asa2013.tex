...
To arrive at a formulation of how $\eR$ is estimated, we first split the array into subarrays of length $L$ that overlap on all but one sensor. Let $x_l[n]$ represent the datavector from subarray $l$,
\begin{gather}
\x_l[n] = \bmat{x_l[n] & x_{l+1}[n] & \dots & x_{l+L-1}[n]}\T.
\end{gather}
Now the sample covariance matrix formed with temporal and spatial averaging, $\eR$, is given as:
\begin{gather}
...
If (\ref{spatialR}) were to be implemented directly, then we note that the estimation step has a complexity of O($N_K N_L L^2$). By comparison the inversion is generally of O($L^3$), which implies that whenever $N_K\,N_L > L$ then the estimation step will dominate. Note that this inequality will increase as we increase the number of subarrays or temporal samples to perform averaging over.
