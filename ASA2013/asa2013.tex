...
Through a case study of on such technique, the Minimum Variance Distortionless Response (MVDR) beamformer, we identify these challenges and explain how they were overcome. The MVDR technique displays a notable potential for improving image quality over simple conventional methods, but is far more computationally complex. This is because a spatial covariance matrix must be estimated and inverted for each image pixel. These steps take turn in being the most complex depending on system size. With less than 20-30 channels the algorithm spends most of its time with the estimation step, while at larger system sizes the inversion step completely dominates. For large systems one should try to bypass the estimation/inversion step altogether. 

Therefore we suggest two different approaches for     dominates the computation time, while for larger system sizes the the inversion step   %Unlike other studies, w While most studies tend to focus on reducing the dimensionality of the convariance matrix to 
...
\end{figure}
The MVDR beamformer target this limitation by adapting the weights for each pixel to best fit the dynamic nature of the incoming wavefield. It computes the set of weights that minimizes the noise and signal power accumulated by the array, but with the constraint of unity gain for signal coming from the pixel of interest. When the data is delayed this is formulated as follows:
\begin{gather}
...
In terms of computational complexity the two important  of the MVDR method the 
Before If we let $x_l[n]$ represent the datavector from subarray $l$\todo{replace with with a drawing?},
\begin{gather}
