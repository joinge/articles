...
\end{gather}
$\B$ here is usually referred to as a Butler matrix. 
...
We have tested our GPU implementation of the MVDR beamformer experimental datasets from the 32 element Kongsberg Maritime HISAS1030 sonar being attached to the HUGIN AUV~\cite{Hansen2009}. To obtain the studied object, the 1500 dwt oil tanker wreck Holmengraa, the sonar was operated in sidescan mode to image (\Fig{hlmengraa}). Holmengraa is roughly 68\,m long and 9\,m wide, and lies on a slanted seabed at 77\,m depth. The image was processed with MVDR parameters $L=16$, $N_\text{avg}=3$, and $d=1\%$, which proved to be a reasonable selection for this scenario.
