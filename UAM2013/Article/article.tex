...
In active sonar imaging systems the choice of sensor weights will greatly influence image quality. The best results are obtained when the weights are allowed to change to accommodate the dynamic nature of the impinging wavefield. The Minimum Variance Distortionless Response (MVDR) beamformer achieves this by computing the set of weights that minimises the effects of noise and interference on the output image. 

Another technique, the Low Complexity Adaptive (LCA) beamformer, addresses the task in an ad-hoc manner. Instead of computing the weights based on the current wavefield, it iterates through a list of predefined weight sets, applies each in turn, and finally selects the one yielding the most desirable response in a minimum variance sense. This is done locally for each pixel in the image.

In this work we extend the LCA beamformer not only to produce images in a MVDR fashion, but also to enhance other features in the image, such as edges, shadows or highlighted regions. From the statistics of the output from all predefined weights applied to a pixel, we are able to make qualified guesses of whether the pixel belongs to a speckle region, the object under study, or something else. The classification of pixel comes almost without any additional computational cost.

When set up with a diverse set of responses to choose from, the LCA produces images like other adaptive techniques. But we limited the responses to enhance other features in the image, such as edges, shadows or highlighted regions. Then, based on the statistics we got when combining all these images, we were able to make qualified guesses of whether the pixel belonged to a speckle region, the object under study, or something else.
