...
In active sonar imaging systems the choice of sensor weights will greatly influence image quality. The best results are obtained when the weights are allowed to change to accommodate the dynamic nature of the impinging wavefield. The Minimum Variance Distortionless Response (MVDR) beamformer achieves this by computing the set of weights that minimises the effects of noise and interference in the output image. 

Another technique, the Low Complexity Adaptive (LCA) beamformer, addresses this task in an ad-hoc manner. Instead of computing the weights based on the current wavefield, it iterates through a list of predefined weight sets, applies each in turn, and finally selects the one yielding the most desirable response in a minimum variance sense. This is done locally for each pixel in the image. With a diverse set of responses to choose from, the LCA produces images similar to the MVDR, but is much less computationally expensive.

In this work we extend the LCA beamformer to not only produce MVDR-like images, but also to enhance other features in the image, such as edges, shadows or highlighted regions. From the statistics found when comparing these images, we are able to make qualified guesses of whether a pixel belongs to a speckle region, the object under study, or something else. This can be achieved with hardly any additional computational cost.
