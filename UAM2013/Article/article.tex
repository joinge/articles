...
%
\begin{abstract}
Active sonar imaging is typically concerned with extracting intensity and phase information from a backscattered acoustic wavefield. These images can be created by performing receive beamforming on data from a sensor array to digitally focus on one pixel at a time in the image. Image resolution and contrast is then determined by the beamformer's ability to enhance the signal from the the pixel of interest while suppressing noise and interference from other directions. When no prior information is available, this is fully determined by the array's geometrical properties, signal characteristic
