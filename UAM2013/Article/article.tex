...
In active sonar imaging systems based on phased arrays the choice of sensor weights will greatly influence image quality. The best results are obtained when the weights are allowed to change to accomodate the dynamic nature of the impinging wavefield.

The Low Complexity Adaptive (LCA) beamformer addresses this task in an ad-hoc manner. Instead of computing the weights based on the current wavefield, it iterates through a list of predefined weight sets, applies each in turn, and finally selects the one yielding the most desirable response. This is done locally for each pixel in the image.
