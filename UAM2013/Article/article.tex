...
The Low Complexity Adaptive (LCA) beamformer address this task in an ad-hoc manner. Instead of computing the weights based on the current wavefield, it iterates through a list of predefined weight sets, applies each in turn, and finally selects the one yielding the most desirable response. This is done locally for each pixel in the image.

When set up with a diverse set of responses to choose from, the LCA produces images like other adaptive techniques. But we limited the responses to enhance other features in the image, such as edges, shadows or highlighted regions. Then, based on the statistics we got when combining all these images, we were able to determine speckle regions, 
