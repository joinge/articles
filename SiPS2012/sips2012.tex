...
\end{align}
where $\vec{a} = \vec{1}$ when $\vec{x}$ are pre-delayed.

Estimation of $\mat{R}[n]$: $\mat{\hat{R}}[n] = \vec{x}[n]\vec{x}[n]^H$.

In order to get a well conditioned $\mat{\hat{R}}$, avoid signal cancellation and DAS-like speckle, $\mat{\hat{R}}$ has to be averaged over $L$ subarrays and $N_{avg}$ time samples.
\begin{align}
\mat{\hat{R}} = \frac{\sum_{n_{avg}=-N_{avg}}^{N_{avg}} \sum_{l=0}^{M-L+1} \vec{x}_l[n+n_{avg}]\vec{x}_l[n+n_{avg}]^H}{(2N_{avg} + 1)(M-L+1)},
\end{align}
where $\vec{x}_l$ is the $l^\text{th}$ subarray $[x_l[n], \dotso x_{l+L}[n]]$.
...
\subsection{Solving Small Linear Systems}
Gauss Jordan elimination to solve $\mat{R}^{-1}\vec{1}$.
