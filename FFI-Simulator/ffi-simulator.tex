...
\subsection{Using OpenGL only for the simulations?}
...
Our original idea was to use OpenGL to produce depth-maps of the object and (optionally) the seafloor, and then use CUDA to compute how a sonar array would illuminate this scene. However, if we can treat sound as ``light'', we believe it to be better to use OpenGL simulate the entire scene and generate the required 2D views and perspectives of it. The advantage of doing ``everything'' in OpenGL is pretty obvious, we may then support whatever OpenGL supports with little extra effort. The gaming industry demands speed, so such a simulator should also be very fast.
